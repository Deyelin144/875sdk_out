...
# added by gurobot
TOOLCHAIN_DIR="$(gettop)/../xr875_toolchain"
...
function generate_image_header() {
	...
	local REC_DEFCONFIG="${T}/lichee/rtos/projects/${RTOS_TARGET_PROJECT_PATH%_*}_m33/ota_defconfig"
	...
	local ARM_RECOVERY=`grep "CONFIG_PROJECT_BUILD_RECOVERY=" ${REC_DEFCONFIG} > /dev/null && echo 1 || echo 0`
	...
	local RECOVERY_ADDR=`grep "CONFIG_ARCH_RISCV_START_ADDRESS=" ${REC_DEFCONFIG} | awk -F= '{print $NF}'`
	...
	CC_SYMBOLS+="-DRECOVERY_ADDR=${RECOVERY_ADDR} "
...
function etf_menuconfig()
{
    local T="$(gettop)"
    cd ${T}/lichee/rtos; make etf_menuconfig
    cd ${T}
}
...
function ota_mrtos()
{
    ...
    [ -f ${IMG_DIR}/rt_system_xip.bin ] &&
	cp -v ${IMG_DIR}/rt_system_xip.bin ${BIN_DIR}/rtos_xip_${RTOS_TARGET_ARCH}_${RTOS_TARGET_CHIP}-recovery.fex

    [ -f ${IMG_DIR}/rt_system_psram.bin ] &&
	cp -v ${IMG_DIR}/rt_system_psram.bin ${BIN_DIR}/rtos_psram_${RTOS_TARGET_ARCH}_${RTOS_TARGET_CHIP}-recovery.fex

    [ -f ${IMG_DIR}/rt_system_hpsram.bin ] &&
	cp -v ${IMG_DIR}/rt_system_hpsram.bin ${BIN_DIR}/rtos_hpsram_${RTOS_TARGET_ARCH}_${RTOS_TARGET_CHIP}-recovery.fex
...
function etf_mrtos()
{
    ...
    [ -f ${T}/lichee/rtos/.config ] \
        && [ -z "$(diff -u ${T}/lichee/rtos/projects/${RTOS_TARGET_PROJECT_PATH}/etf_defconfig ${T}/lichee/rtos/.config)" ] || {
        echo "Use ${RTOS_TARGET_ROJECT_PATH} default config"
        cp ${T}/lichee/rtos/projects/${RTOS_TARGET_PROJECT_PATH}/etf_defconfig ${T}/lichee/rtos/.config
    }
    ...
    if [ "x$@" == "xmenuconfig" ]; then
            cp ${T}/lichee/rtos/.config ${T}/lichee/rtos/projects/${RTOS_TARGET_PROJECT_PATH}/etf_defconfig
            return
    fi
    ...
    [ -f ${IMG_DIR}/rt_system.bin ] &&
	cp -v ${IMG_DIR}/rt_system.bin ${BIN_DIR}/etf_${RTOS_TARGET_CHIP}.fex
...
function lunch_rtos()
{
    ...
    if [ "$(expr substr $(uname -s) 1 6)" == "CYGWIN" ];then
        ...
        export RTOS_BUILD_ENV="window"
    else
        uncompress_toolchain_linux
        export RTOS_BUILD_ENV="linux"
    fi
...
function ftsymbol()
{
       ...
       \cd $T/lichee/rtos/components/common/aw/ftrace

    if [ -f "$T/lichee/rtos/build/${RTOS_PROJECT_NAME}/img/rt_system.map"  ]; then
               ./sysmap.sh $T/lichee/rtos/build/${RTOS_PROJECT_NAME}/img/rt_system.elf
       else
               echo "Please Compile RTOS."
    fi
...
function install_dsp()
{
    ...
    DSP_BUILD_CONFIG_FILE="${T}/lichee/dsp/.buildconfig"
    if [ -f "$DSP_BUILD_CONFIG_FILE" ]; then
        IC=$(grep -oP '^export LICHEE_IC=\K.*' "$DSP_BUILD_CONFIG_FILE")
        DSP_CORE=$(grep -oP '^export LICHEE_DSP_CORE=\K.*' "$DSP_BUILD_CONFIG_FILE")
        BOARD=$(grep -oP '^export LICHEE_CHIP_BOARD=\K.*' "$DSP_BUILD_CONFIG_FILE")
    else
        echo "Error: .buildconfig file not found!"
    fi

    if [ "x${RTOS_TARGET_CHIP}" == "xsun20iw2p1" ]; then
        if [ -d "$T/board/${RTOS_TARGET_BOARD_PATH%_*}/bin"  ]; then
            cp -vf $T/lichee/dsp/out/${RTOS_PROJECT_NAME%%_*}/${BOARD}/${RTOS_PROJECT_NAME%%_*}_${DSP_CORE}_${BOARD}_raw.bin $T/board/${RTOS_TARGET_BOARD_PATH%_*}/bin/rtos_dsp_${RTOS_TARGET_CHIP}.fex
            if [[ -f  $T/lichee/dsp/out/${RTOS_PROJECT_NAME%%_*}/${BOARD}/${RTOS_PROJECT_NAME%%_*}_${DSP_CORE}_${BOARD}_raw_xip.bin ]]; then
                cp -vf $T/lichee/dsp/out/${RTOS_PROJECT_NAME%%_*}/${BOARD}/${RTOS_PROJECT_NAME%%_*}_${DSP_CORE}_${BOARD}_raw_xip.bin $T/board/${RTOS_TARGET_BOARD_PATH%_*}/bin/rtos_xip_dsp_${RTOS_TARGET_CHIP}.fex
            fi
        fi
    ...
    if [ -d "$T/board/${RTOS_TARGET_BOARD_PATH}/bin"  ]; then
        cp -vf $T/lichee/dsp/out/${RTOS_PROJECT_NAME%%_*}/${BOARD}/${RTOS_PROJECT_NAME%%_*}_${DSP_CORE}_${BOARD}_raw.bin $T/board/${RTOS_TARGET_BOARD_PATH}/bin/rtos_dsp_${RTOS_TARGET_CHIP}.fex
        return
    fi

    cp -vf $T/lichee/dsp/out/${RTOS_PROJECT_NAME%%_*}/${BOARD}/${RTOS_PROJECT_NAME%%_*}_${DSP_CORE}_${BOARD}_raw.bin $T/board/bin/rtos_dsp_${RTOS_TARGET_CHIP}.fex
}
...
function ota_pack()
{
    ...
    # files merge
    echo "-------------------- do merge --------------------"
    if [ "x$(find $OTA_DIR/update -maxdepth 1 -name "merge")" = "x" ]; then
        mkdir -p $OTA_DIR/update/merge
    fi

    cp $OTA_DIR/update/ota-step $OTA_DIR/update/ota-info
    for i in "${image_name[@]}"; do
        image_size=$(stat -c%s $OTA_DIR/update/${i})
        image_md5=$(md5sum $OTA_DIR/update/${i} | awk '{print $1}')
        sed -i "s/image-file=\"${i}\"/image-file=\"${i}\"\n\timage-md5:\"${image_md5}\"\n\timage-size:\"${image_size}\"/g" $OTA_DIR/update/ota-info
    done

    unset image_md5

    if [ "$1" == "-recovery" ]; then
        config_size=`grep "CONFIG_COMPONENTS_AW_OTA_INFO_SIZE=" ${REC_DEFCONFIG} | awk -F= '{print $NF}'`
    else
        config_size=`grep "CONFIG_COMPONENTS_AW_OTA_INFO_SIZE=" ${RV_DEFCONFIG} | awk -F= '{print $NF}'`
    fi
    real_size=$(stat -c%s $OTA_DIR/update/ota-info)
    if [ "$config_size" -gt "$real_size" ]; then
        truncate -s $config_size $OTA_DIR/update/ota-info
        cd $OTA_DIR/update/
        image_name[0]=ota-info
        #echo ${image_name[@]}
        image_md5=$(md5sum ota-info | awk '{print $1}')
        echo -n $image_md5 > md5-temp
        cat md5-temp ${image_name[@]} > merge/ota-package
        cp ota-info merge/
        rm ota-info md5-temp
    else
        echo "Generate ota-package failed. The OTA_INFO_LEN is too small."
        echo "OTA_INFO_LEN:" $info_size "real_size:" $real_size
        return
    fi
    unset image_name
...
if [ "$(expr substr $(uname -s) 1 6)" == "CYGWIN" ];then
    echo "=============== cygwin env ==================="
    USE_GUROBOT_ENV_WAY=1
    if [ $USE_GUROBOT_ENV_WAY -eq 0 ];then
        ...
        echo "use gurobot env"
        TMP_PATH=lichee/rtos/components/common
        if [ ! -d $TMP_PATH ];then
            echo "mkdir $TMP_PATH"
            mkdir $TMP_PATH
        fi
        
        TMP_PATH=$(gettop)/lichee/rtos/components/common/aw
        if [ ! -L $TMP_PATH ];then
            if [ -d $TMP_PATH ];then
                echo "rm dir $TMP_PATH"
                rm $TMP_PATH -rf
            fi
            echo "make mklink aw"
            # mklink /J Link Target
            cmd /c "mklink /J lichee\rtos\components\common\aw lichee\rtos-components\aw"
        fi

        TMP_PATH=$(gettop)/lichee/rtos/components/common/thirdparty
        if [ ! -L $TMP_PATH ];then
            if [ -d $TMP_PATH ];then
                echo "rm dir $TMP_PATH"
                rm $TMP_PATH -rf
            fi
            echo "make mklink thirdparty"
            cmd /c "mklink /J lichee\rtos\components\common\thirdparty lichee\rtos-components\thirdparty"
        fi

        TMP_PATH=$(gettop)/lichee/rtos/drivers/rtos-hal
        if [ ! -L $TMP_PATH ];then
            if [ -d $TMP_PATH ];then
                echo "rm dir $TMP_PATH"
                rm $TMP_PATH -rf
            fi
            echo "make mklink rtos-hal"
            cmd /c "mklink /J lichee\rtos\drivers\rtos-hal lichee\rtos-hal"
        fi
    fi
    ...
    if [ ! -h $TMP_PATH ];then
        echo "ln -sf $TMP_PATH"
        ln -sf $(pwd)/lichee/rtos-components/aw lichee/rtos/components/common/aw
    fi

    TMP_PATH=$(gettop)/lichee/rtos/components/common/thirdparty
    if [ ! -h $TMP_PATH ];then
        echo "ln -sf $TMP_PATH"
        ln -sf $(pwd)/lichee/rtos-components/thirdparty lichee/rtos/components/common/thirdparty
    fi

    TMP_PATH=$(gettop)/lichee/rtos/drivers/rtos-hal
    if [ ! -h $TMP_PATH ];then
        echo "ln -sf $TMP_PATH"
        ln -sf $(pwd)/lichee/rtos-hal lichee/rtos/drivers/rtos-hal
    fi
    echo "=============== copy done ==================="
fi
