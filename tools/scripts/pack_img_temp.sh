...
function do_check_part_size_align()
{
	local align_size part_size
	if [ x"${storage_type}" = x"3" ]; then
		align_size=8
		cp sys_partition_nor.fex ./check_sys_partition.tmp
	else
		align_size=512
		cp sys_partition.fex ./check_sys_partition.tmp
	fi

	sed -i '/mbr/,+5d' ./check_sys_partition.tmp;
	sed -i '/^[\r;]/d' ./check_sys_partition.tmp;
	sed -i '/partition_start/d' ./check_sys_partition.tmp;
	sed -i '/user_type/d' ./check_sys_partition.tmp;
	sed -i 's/\[partition\]/------------------------------------/g' ./check_sys_partition.tmp;
	sed -i 's/[ "\r]//g' ./check_sys_partition.tmp;
	sed -i '/^[;]/d' ./check_sys_partition.tmp;
	sed -i 's/name/partition_name/g' ./check_sys_partition.tmp;
	sed -i 's/size/partition_size/g' ./check_sys_partition.tmp;

	local size_array=$(grep "size" ./check_sys_partition.tmp)
	while read line
	do
		part_size=`echo ${line} | awk -F '=' '{print $2}'`
		if [ $(( ${part_size} % ${align_size} )) -gt 0 ]; then
			pack_error "partition size ${part_size} should align to ${align_size}"
			return 1
		fi
	done <<< "${size_array}"

	rm ./check_sys_partition.tmp
}
...
function do_finish()
{
	...
	#######################################   USER CODE   ##########################################
	#提取版本号
	PROJECT_DIR_NAME=$(dirname ${PACK_PROJECT_PATH})
	RECOVERY_HEADER=${PACK_TOPDIR}/lichee/rtos/projects/${PROJECT_DIR_NAME}/solution_m33/src/recovery/recovery_main.h
	FIRMWARE_HEADER=${PACK_TOPDIR}/lichee/rtos/projects/${PROJECT_DIR_NAME}/solution_c906/src/apps/native_info_manage/native_info_manage.h
	
	echo "DEBUG: RECOVERY_HEADER=${RECOVERY_HEADER}"
	echo "DEBUG: FIRMWARE_HEADER=${FIRMWARE_HEADER}"

	RECOVERY_VER="unknown"
	if [ -f "${RECOVERY_HEADER}" ]; then
		VAL=$(grep '#define RECOVERY_VERSION' "${RECOVERY_HEADER}" | awk -F'"' '{print $2}')
		if [ -n "$VAL" ]; then
			RECOVERY_VER=$VAL
		fi
	fi
	...
	if [ -f "${FIRMWARE_HEADER}" ]; then
		VAL=$(grep '#define FIRMWARE_VERSION' "${FIRMWARE_HEADER}" | awk -F'"' '{print $2}')
		if [ -n "$VAL" ]; then
			FIRMWARE_VER=$VAL
		fi
	fi

	echo "DEBUG: RECOVERY_VER=${RECOVERY_VER}"
	echo "DEBUG: FIRMWARE_VER=${FIRMWARE_VER}"


	echo '**************************************************'
	###############   (COPY 只需要修改这部分）   ##################
	#基地址
	PROJECT_PATH=/home/icommiot/robot
	#导出路径（复制后的路径）
	SHARE_PATH=/mnt/d/file/project/git/tinglixiong/zimg/robot
	# SHARE_PATH=/mnt/d/file/project/git/tinglixiong/zimg/robot/${FIRMWARE_VER}+${RECOVERY_VER}+1.0.1.641	
	##############################################################
	...
	#ELF路径
	C906_ELF_PATH=${PROJECT_PATH}/xr875_single_repository/lichee/rtos/build/xr875s1h10_solution_c906/img/rt_system.elf 
	M33_ELF_PATH=${PROJECT_PATH}/xr875_single_repository/lichee/rtos/build/xr875s1h10_solution_m33/img/rt_system.elf
	#MAP路径
	C906_MAP_PATH=${PROJECT_PATH}/xr875_single_repository/lichee/rtos/build/xr875s1h10_solution_c906/img/rt_system.map
	M33_MAP_PATH=${PROJECT_PATH}/xr875_single_repository/lichee/rtos/build/xr875s1h10_solution_m33/img/rt_system.map
	#OTA_FEX路径
	RTOS_RISCV_FEX=${PROJECT_PATH}/xr875_single_repository/out/xr875s1h10/solution/image/rtos_riscv.fex		#rtos_riscv.fex
	RTOS_ARM_FEX=${PROJECT_PATH}/xr875_single_repository/out/xr875s1h10/solution/image/rtos_arm.fex 			#rtos_arm.fex
	RTOS_DSP_FEX=${PROJECT_PATH}/xr875_single_repository/out/xr875s1h10/solution/image/rtos_dsp.fex 			#rtos_dsp.fex
	RECOVER_FEX=${PROJECT_PATH}/xr875_single_repository/out/xr875s1h10/solution/image/recovery.fex 			#recovery.fex
	#IMG路径
	IMG_PATH=${PROJECT_PATH}/xr875_single_repository/out/xr875s1h10/solution/rtos_freertos_gu206h10_solution_uart0_128Mnand.img

	#复制
	cp_failed=0
	cp ${C906_ELF_PATH} ${SHARE_PATH}/c906_rt_system.elf || { echo "Failed to copy C906 ELF"; cp_failed=1; }
	cp ${M33_ELF_PATH} ${SHARE_PATH}/m33_rt_system.elf || { echo "Failed to copy M33 ELF"; cp_failed=1; }
	cp ${C906_MAP_PATH} ${SHARE_PATH}/c906_rt_system.map || { echo "Failed to copy C906 MAP"; cp_failed=1; }
	cp ${M33_MAP_PATH} ${SHARE_PATH}/m33_rt_system.map || { echo "Failed to copy M33 MAP"; cp_failed=1; }
	cp ${RTOS_ARM_FEX} ${SHARE_PATH} || { echo "Failed to copy RTOS ARM FEX"; cp_failed=1; }
	cp ${RTOS_DSP_FEX} ${SHARE_PATH} || { echo "Failed to copy RTOS DSP FEX"; cp_failed=1; }
	cp ${RTOS_RISCV_FEX} ${SHARE_PATH}/rtos_riscv_${FIRMWARE_VER}.fex || { echo "Failed to copy RTOS RISCV FEX"; cp_failed=1; }
	cp ${RECOVER_FEX} ${SHARE_PATH}/recovery_${RECOVERY_VER}.fex || { echo "Failed to copy RECOVERY FEX"; cp_failed=1; }
	cp ${IMG_PATH} ${SHARE_PATH} || { echo "Failed to copy IMG"; cp_failed=1; }

	#打印提示
	echo -e '\033[0;34m'
	echo '**************************************************'
	echo cp ${C906_ELF_PATH} 
	echo cp ${M33_ELF_PATH} 
	echo cp ${C906_MAP_PATH} 
	echo cp ${M33_MAP_PATH} 
	echo cp ${RTOS_ARM_FEX}
	echo cp ${RTOS_DSP_FEX} 
	echo cp ${RTOS_RISCV_FEX} "->" rtos_riscv_${FIRMWARE_VER}.fex
	echo cp ${RECOVER_FEX} "->" recovery_${RECOVERY_VER}.fex
	echo cp ${IMG_PATH} 
	echo 
	echo to 
	echo 
	...
	if [ $cp_failed -ne 0 ]; then
		...
		echo -e '\033[0;31;1m' 
		echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
		echo "ERROR: COPY FAILED! Please check above errors."
		echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
		echo -e '\033[0m'
	else
		echo -e '\033[0;32m'
...
function do_dragon()
{
	...
	if [ $? -eq 0 ]; then
		if [ -e ${IMG_NAME} ]; then
			...
			if [[ ${IMG_NAME} == *875* ]]; then
                new_img_name=$(echo ${IMG_NAME} | sed 's/xr875s1/gu206/g')
                mv "../${IMG_NAME}" "../${new_img_name}"
                # echo "Renamed image to: ${new_img_name}"
            fi
			echo "----------${new_img_name}----------"
			echo '----------image is at----------'
			echo -e '\033[0;31;1m'
			echo ${ROOT_DIR}/${PACK_PROJECT_PATH}/${new_img_name}
			echo -e '\033[0m'
		fi
	fi
...
function check_config_overload()
{
	# Define three valid address ranges (hex to decimal conversion)
	local CK_SRAM_START=$((0x4000000))      # 67108864
	local CK_SRAM_END=$((0x4100000))        # 68157440
	local CK_LPSRAM_START=$((0x8000000))    # 134217728
	local CK_LPSRAM_END=$((0x9000000))      # 150994944
	local CK_HPSRAM_START=$((0xC000000))    # 201326592
	local CK_HPSRAM_END=$((0xE000000))      # 234881024

	local config_file="config.fex"
	local cfg_path="${ROOT_DIR}/${PACK_PROJECT_PATH}/image/image_header.cfg"

	# Ensure configuration file exists
	if [ ! -f "$cfg_path" ]; then
		pack_error "Configuration file $cfg_path not found!"
		exit 1
	fi

	# Get config.fex start address (sram_offs) - search by filename
	local CONFIG_ADDR=$(grep -oE '\{"[^}]*"bin": "'"$config_file"'"[^}]*\}' "$cfg_path" | grep -o '"sram_offs": "[^"]*"' | cut -d'"' -f4)
	if [ -z "$CONFIG_ADDR" ]; then
		pack_warn "Failed to find $config_file address in $cfg_path"
		return 0
	fi

	# Get config.fex size
	local CONFIG_SIZE=$(ls -nl "$config_file" 2>/dev/null | awk '{print $5}')
	if [ -z "$CONFIG_SIZE" ] || [ "$CONFIG_SIZE" -le 0 ]; then
		pack_error "Failed to get size of $config_file"
		exit 1
	fi

	# Convert to decimal for calculations
	local CONFIG_LONGTH=$((CONFIG_ADDR))
	local CONFIG_END=$((CONFIG_LONGTH + CONFIG_SIZE))

	# Check if config.fex is within valid address ranges
	if ! { [ $CONFIG_LONGTH -ge $CK_SRAM_START ] && [ $CONFIG_LONGTH -lt $CK_SRAM_END ] || \
		  [ $CONFIG_LONGTH -ge $CK_LPSRAM_START ] && [ $CONFIG_LONGTH -lt $CK_LPSRAM_END ] || \
		  [ $CONFIG_LONGTH -ge $CK_HPSRAM_START ] && [ $CONFIG_LONGTH -lt $CK_HPSRAM_END ]; }; then
		pack_error "$config_file (0x$(printf '%x' $CONFIG_LONGTH)) is in an invalid address range!"
		exit 1
	fi

	# Determine which section config.fex is in
	local config_section=""
	local config_section_id=""
	if [ $CONFIG_LONGTH -ge $CK_SRAM_START ] && [ $CONFIG_LONGTH -lt $CK_SRAM_END ]; then
		config_section="CK_SRAM (0x4000000-0x4100000)"
		config_section_id="1"
	elif [ $CONFIG_LONGTH -ge $CK_LPSRAM_START ] && [ $CONFIG_LONGTH -lt $CK_LPSRAM_END ]; then
		config_section="CK_LPSRAM (0x8000000-0x9000000)"
		config_section_id="2"
	else
		config_section="CK_HPSRAM (0xC000000-0xE000000)"
		config_section_id="3"
	fi

	# Get information about all other binary files from config file (excluding config.fex)
	local other_bins=$(grep -oE '\{"[^}]*"bin": "[^"]+"[^}]*\}' "$cfg_path" | grep -v "\"bin\": \"$config_file\"" | grep -v '{}')

	# Check each other binary file for conflicts with config.fex
	while IFS= read -r bin_entry; do
		# Extract filename and address
		local bin_name=$(echo "$bin_entry" | grep -o '"bin": "[^"]*"' | cut -d'"' -f4)
		local bin_addr=$(echo "$bin_entry" | grep -o '"sram_offs": "[^"]*"' | cut -d'"' -f4)
		
		if [ -z "$bin_name" ] || [ -z "$bin_addr" ]; then
			continue  # Skip invalid entries
		fi

		# Convert to decimal
		local bin_longth=$((bin_addr))

		# Check if this binary file is within valid address ranges
		if ! { [ $bin_longth -ge $CK_SRAM_START ] && [ $bin_longth -lt $CK_SRAM_END ] || \
			  [ $bin_longth -ge $CK_LPSRAM_START ] && [ $bin_longth -lt $CK_LPSRAM_END ] || \
			  [ $bin_longth -ge $CK_HPSRAM_START ] && [ $bin_longth -lt $CK_HPSRAM_END ]; }; then
			pack_error "$bin_name (0x$(printf '%x' $bin_longth)) is in an invalid address range!"
			exit 1
		fi

		# Determine which section this binary file is in
		local bin_section_id=""
		if [ $bin_longth -ge $CK_SRAM_START ] && [ $bin_longth -lt $CK_SRAM_END ]; then
			bin_section_id="1"
		elif [ $bin_longth -ge $CK_LPSRAM_START ] && [ $bin_longth -lt $CK_LPSRAM_END ]; then
			bin_section_id="2"
		else
			bin_section_id="3"
		fi

		# Only check for overlap if in the same section
		if [ "$bin_section_id" = "$config_section_id" ]; then
			if [ $CONFIG_END -gt $bin_longth ]; then
				pack_error "$config_file (0x$(printf '%x' $CONFIG_LONGTH)-0x$(printf '%x' $CONFIG_END)) overlaps with $bin_name (0x$(printf '%x' $bin_longth)) in $config_section!"
				pack_error "pack error, please adjust addresses!"
				exit 1
			fi
		fi
	done <<< "$other_bins"
