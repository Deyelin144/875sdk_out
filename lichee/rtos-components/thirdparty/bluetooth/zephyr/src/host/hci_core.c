			 ...
			 struct net_buf **rsp)
{
	...
	if (status
		...
		if (cmd(buf)->state)
			cmd(buf)->state = NULL;
		/* avoid timeout-event release semaphore in later */
		cmd(buf)->sync = NULL;
#endif
		net_buf_unref(buf);
...
#ifdef CONFIG_BT_XR_IMPL
	/**
	 * send the VERSION_IND first as slave role. Solve incompatibility problem
	 * when certain Android phones is below Android6.0(only initating feature
	 * exchange but without version exchange)
	 */
...
static void le_remote_feat_complete(struct net_buf *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_REMOTE_INFO) &&
	    (!IS_ENABLED(CONFIG_BT_REMOTE_VERSION) || conn->role == BT_HCI_ROLE_SLAVE)) {
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	k_thread_create(&tx_thread_data, tx_thread_stack,
			...
			0, K_NO_WAIT);
	k_thread_name_set(&tx_thread_data, "z_core_tx");
	...
	k_thread_create(&rx_thread_data, rx_thread_stack,
			...
			0, K_NO_WAIT);
	k_thread_name_set(&rx_thread_data, "z_core_rx");
#endif
