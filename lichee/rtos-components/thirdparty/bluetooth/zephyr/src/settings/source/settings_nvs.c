...
int32_t settings_nvs_src_deinit(struct settings_nvs *cf)
{
	settings_src_unregister(&cf->cf_store);
}

int32_t settings_nvs_dst_deinit(struct settings_nvs *cf)
{
...
#if defined(CONFIG_BT_DEINIT)
int settings_nvs_backend_deinit(struct settings_nvs *cf)
{
	int rc;
	uint16_t last_name_id;

	cf->last_name_id = NVS_NAMECNT_ID;
	rc = nvs_deinit(&cf->cf_nvs, cf->flash_dev_name);
	if (rc) {
		return rc;
	}
...
{
	settings_nvs_backend_deinit(&default_settings_nvs);
	settings_nvs_src_deinit(&default_settings_nvs);
	settings_nvs_dst_deinit(&default_settings_nvs);
