...
static bool process_disc_flag = false;
...
static BOOLEAN btc_av_state_opening_handler(btc_sm_event_t event, void *p_data)
{
    ...
    case BTA_AV_OPEN_EVT: {
        ...
        if (true == process_disc_flag) {
            BTC_TRACE_WARNING("%s %d process disconnect befor connected\n",__FUNCTION__,__LINE__);
            break;
        }
    ...
    case BTC_AV_DISCONNECT_REQ_EVT:
        BTC_TRACE_WARNING("%s %d disconnect event come befor connected\n",__FUNCTION__,__LINE__);
        process_disc_flag = true;
...
static bt_status_t btc_av_init(int service_id)
{
    ...
    if (btc_av_cb_ptr != NULL) {
        BTC_TRACE_ERROR("%s AV already initiated!", __func__);
...
static bt_status_t connect_init(bt_bdaddr_t *bd_addr, uint16_t uuid)
{
    ...
    BTC_TRACE_DEBUG("%s\n", __FUNCTION__);
#ifdef CONFIG_BT_XR_BUG_FIXED
    if (!g_a2dp_on_init) {
        BTC_TRACE_ERROR("%s AV has not been initiated, shall init first!", __func__);
        return BT_STATUS_NOT_READY;
    }
#endif /* CONFIG_BT_XR_BUG_FIXED */
...
#if A2D_DYNAMIC_MEMORY == TRUE
    // avoid double a2dp_src/snk deinit
    if (btc_av_cb_ptr == NULL) {
        return;
    }
#endif
...
void btc_a2dp_call_handler(btc_msg_t *msg)
{
    ...
    if ((!g_a2dp_on_init)
#if BTC_AV_SINK_INCLUDED
        && (msg->act != BTC_AV_SINK_API_INIT_EVT && msg->act != BTC_AV_SINK_API_REG_DATA_CB_EVT)
#endif /* BTC_AV_SINK_INCLUDED */
#if BTC_AV_SRC_INCLUDED
        && (msg->act != BTC_AV_SRC_API_INIT_EVT && msg->act != BTC_AV_SRC_API_REG_DATA_CB_EVT)
#endif /* BTC_AV_SRC_INCLUDED */
       ) {
        BTC_TRACE_ERROR("%s AV has not been initiated, shall init first! act: %d", __func__, msg->act);
        return;
    }
#endif /* CONFIG_BT_XR_BUG_FIXED */
...
static bt_status_t btc_a2d_sink_connect(bt_bdaddr_t *remote_bda)
{
    ...
    return btc_queue_connect(UUID_SERVCLASS_AUDIO_SINK, remote_bda, connect_init);
}
...
static bt_status_t btc_a2d_src_connect(bt_bdaddr_t *remote_bda)
{
    ...
    return btc_queue_connect(UUID_SERVCLASS_AUDIO_SOURCE, remote_bda, connect_init);
}
