...
static inline void btc_hh_cb_to_app(xr_hidh_cb_event_t event, xr_hidh_cb_param_t *param)
{
    xr_hh_cb_t btc_hh_cb = (xr_hh_cb_t)btc_profile_cb_get(BTC_PID_HH);
    if (btc_hh_cb) {
 ...
 ******************************************************************************/
static inline tBTA_HH_PROTO_MODE proto_mode_change_to_lower_layer(xr_hidh_protocol_mode_t protocol_mode)
{
    tBTA_HH_PROTO_MODE proto_mode = BTA_HH_PROTO_UNKNOWN;

    switch (protocol_mode) {
    case XR_HIDH_REPORT_MODE:
        proto_mode = BTA_HH_PROTO_RPT_MODE;
        break;
    case XR_HIDH_BOOT_MODE:
 ...
 ******************************************************************************/
static inline xr_hidh_protocol_mode_t proto_mode_change_to_upper_layer(tBTA_HH_PROTO_MODE proto_mode)
{
    xr_hidh_protocol_mode_t protocol_mode = XR_HIDH_UNSUPPORTED_MODE;

    switch (proto_mode) {
    case BTA_HH_PROTO_RPT_MODE:
        protocol_mode = XR_HIDH_REPORT_MODE;
        break;
    case BTA_HH_PROTO_BOOT_MODE:
        protocol_mode = XR_HIDH_BOOT_MODE;
        break;
    default:
        break;
    }
...
btc_hh_device_t *btc_hh_find_connected_dev_by_handle(uint8_t handle)
{
    ...
    for (i = 0; i < BTC_HH_MAX_HID; i++) {
        if (btc_hh_cb.devices[i].dev_status == XR_HIDH_CONN_STATE_CONNECTED && btc_hh_cb.devices[i].dev_handle == handle) {
...
static btc_hh_device_t *btc_hh_find_dev_by_bda(BD_ADDR bd_addr)
{
    uint8_t i;
    for (i = 0; i < BTC_HH_MAX_HID; i++) {
        if (btc_hh_cb.devices[i].dev_status != XR_HIDH_CONN_STATE_UNKNOWN &&
...
static btc_hh_device_t *btc_hh_find_connected_dev_by_bda(BD_ADDR bd_addr)
{
    uint32_t i;
    for (i = 0; i < BTC_HH_MAX_HID; i++) {
        if (btc_hh_cb.devices[i].dev_status == XR_HIDH_CONN_STATE_CONNECTED &&
            memcmp(btc_hh_cb.devices[i].bd_addr, bd_addr, BD_ADDR_LEN) == 0) {
            return &btc_hh_cb.devices[i];
        }
    }
...
void btc_hh_timer_timeout(void *data)
{
    ...
    if (p_dev->dev_status != XR_HIDH_CONN_STATE_CONNECTED){
...
void btc_hh_remove_device(BD_ADDR bd_addr)
{
    ...
    p_dev->dev_status = XR_HIDH_CONN_STATE_UNKNOWN;
    ...
    case BTA_HH_GET_RPT_EVT: {
        ...
        if (src_hdr) {
            ...
            if (p_dst_data->hs_data.rsp_data.p_rpt_data == NULL) {
                ...
                p_dst_data->hs_data.status = XR_HIDH_ERR_NO_RES;
                break;
            }
...
static void btc_hh_deinit(void)
{
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_DEINIT_EVT, &param);
    }
}
...
static void btc_hh_connect(btc_hidh_args_t *arg)
{
    ...
    do {
        ...
        param.open.conn_status = XR_HIDH_CONN_STATE_CONNECTING;
        ret = XR_HIDH_OK;
    } while (0);

    if (ret != XR_HIDH_OK) {
        param.open.conn_status = XR_HIDH_CONN_STATE_DISCONNECTED;
    }
    ...
    btc_hh_cb_to_app(XR_HIDH_OPEN_EVT, &param);
}
...
static void btc_hh_disconnect(btc_hidh_args_t *arg)
{
    ...
    do {
        ...
        if (p_dev != NULL) {
            ...
            param.close.conn_status = XR_HIDH_CONN_STATE_DISCONNECTING;
            param.close.handle = p_dev->dev_handle;
        } else {
            ret = XR_HIDH_NO_CONNECTION;
            BTC_TRACE_ERROR("%s Error: device not connected!", __func__);
        }
    ...
    if (ret != XR_HIDH_OK) {
        param.close.conn_status = XR_HIDH_CONN_STATE_DISCONNECTED;
        param.close.handle = BTA_HH_INVALID_HANDLE;
    }
    param.close.status = ret;
    btc_hh_cb_to_app(XR_HIDH_CLOSE_EVT, &param);
}
...
static void btc_hh_virtual_unplug(btc_hidh_args_t *arg)
{
    ...
    do {
        ...
        if ((p_dev != NULL) && (p_dev->dev_status == XR_HIDH_CONN_STATE_CONNECTED) &&
            ...
            param.unplug.conn_status = XR_HIDH_CONN_STATE_DISCONNECTING;
            param.unplug.handle = p_dev->dev_handle;
        } else if ((p_dev != NULL) && (p_dev->dev_status == XR_HIDH_CONN_STATE_CONNECTED)) {
    ...
    if (ret != XR_HIDH_OK) {
        param.unplug.conn_status = XR_HIDH_CONN_STATE_DISCONNECTED;
        param.unplug.handle = BTA_HH_INVALID_HANDLE;
    }
    param.unplug.status = ret;
    btc_hh_cb_to_app(XR_HIDH_VC_UNPLUG_EVT, &param);
}
...
static void btc_hh_set_info(btc_hidh_args_t *arg)
{
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_SET_INFO_EVT, &param);
    }
}
...
static void btc_hh_get_protocol(btc_hidh_args_t *arg)
{
    ...
    if (ret != XR_HIDH_OK) {
        param.get_proto.proto_mode = XR_HIDH_UNSUPPORTED_MODE;
        param.get_proto.handle = BTA_HH_INVALID_HANDLE;
        param.get_proto.status = ret;
        btc_hh_cb_to_app(XR_HIDH_GET_PROTO_EVT, &param);
    }
}
...
static void btc_hh_set_protocol(btc_hidh_args_t *arg)
{
    ...
    do {
        ...
        if (p_dev == NULL) {
            ...
            BTC_TRACE_ERROR("%s Error: device not connected!", __func__);
            break;
        } else if (arg->set_protocol.protocol_mode != XR_HIDH_REPORT_MODE && arg->set_protocol.protocol_mode != XR_HIDH_BOOT_MODE) {
            BTC_TRACE_ERROR("%s: Error, device proto_mode = %d.", __func__, arg->set_protocol.protocol_mode);
            ret = XR_HIDH_HS_INVALID_PARAM;
            break;
        } else {
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_SET_PROTO_EVT, &param);
    }
}
...
static void btc_hh_get_report(btc_hidh_args_t *arg)
{
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_GET_RPT_EVT, &param);
    }
}
...
static void btc_hh_set_report(btc_hidh_args_t *arg)
{
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_SET_RPT_EVT, &param);
    }
}
...
static void btc_hh_send_data(btc_hidh_args_t *arg)
{
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_DATA_EVT, &param);
    }
}
...
static void btc_hh_get_idle_time(btc_hidh_args_t *arg)
{
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_GET_IDLE_EVT, &param);
    }
}
...
static void btc_hh_set_idle_time(btc_hidh_args_t *arg)
{
    ...
    if (ret != XR_HIDH_OK) {
        ...
        btc_hh_cb_to_app(XR_HIDH_SET_IDLE_EVT, &param);
    }
}
...
bool btc_hh_cb_copy_hid_info(xr_hidh_cb_param_t *param, tBTA_HH_DEV_DSCP_INFO *src)
{
    param->dscp.dl_len = 0;
    if (src->descriptor.dl_len > 0) {
...
void btc_hh_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_HH_OPEN_EVT:
        ...
        if (p_data->conn.status == BTA_HH_OK) {
            ...
            if (p_dev == NULL) {
                ...
                p_dev->dev_status = XR_HIDH_CONN_STATE_CONNECTED;

                param.open.status = XR_HIDH_OK;
                param.open.conn_status = XR_HIDH_CONN_STATE_CONNECTED;
            }
        } else {
            p_dev = btc_hh_find_dev_by_bda(p_data->conn.bda);
            if (p_dev != NULL) {
                btc_hh_stop_vup_timer(p_dev->bd_addr);
                p_dev->dev_status = XR_HIDH_CONN_STATE_DISCONNECTED;
            }
            btc_hh_cb.status = (BTC_HH_STATUS)BTC_HH_DEV_DISCONNECTED;
    ...
    case BTA_HH_GET_DSCP_EVT:
        ...
        do {
            ...
            if (p_dev == NULL) {
                ...
                param.dscp.status = XR_HIDH_NO_CONNECTION;
                break;
            }
        ...
        btc_hh_cb_to_app(XR_HIDH_GET_DSCP_EVT, &param);
    ...
    case BTA_HH_CLOSE_EVT:
        ...
        if (p_dev != NULL) {
            ...
            param.close.status = XR_HIDH_NO_CONNECTION;
        }
    ...
    case BTA_HH_VC_UNPLUG_EVT:
        ...
        if (p_dev != NULL) {
            ...
            BTC_TRACE_ERROR("Error: cannot find device with handle %d", p_data->dev_status.handle);
            param.unplug.status = XR_HIDH_NO_CONNECTION;
        }
    ...
    case BTA_HH_ADD_DEV_EVT:
        ...
        if (btc_hh_cb.add_event == BTC_HH_SET_INFO_EVT) {
            ...
            btc_hh_cb_to_app(XR_HIDH_ADD_DEV_EVT, &param);
        }
        break;
    case BTA_HH_RMV_DEV_EVT:
        BTC_TRACE_DEBUG("status = %d, handle = %d", p_data->dev_info.status, p_data->dev_info.handle);
        ...
        btc_hh_cb_to_app(XR_HIDH_RMV_DEV_EVT, &param);
        break;
    case BTA_HH_DATA_IND_EVT:
        ...
        btc_hh_cb_to_app(XR_HIDH_DATA_IND_EVT, &param);
        break;
    case BTA_HH_API_ERR_EVT:
        break;
    default:
...
void btc_hh_arg_deep_copy(btc_msg_t *msg, void *p_dest, void *p_src)
{
    ...
    switch (msg->act) {
    case BTC_HH_SET_INFO_EVT:
        dst->set_info.hid_info = (xr_hidh_hid_info_t *)osi_malloc(sizeof(xr_hidh_hid_info_t));
        if (dst->set_info.hid_info) {
            memcpy(dst->set_info.hid_info, src->set_info.hid_info, sizeof(xr_hidh_hid_info_t));
        } else {
