...
static inline void btc_hd_cb_to_app(xr_hidd_cb_event_t event, xr_hidd_cb_param_t *param)
{
    xr_hd_cb_t btc_hd_cb = (xr_hd_cb_t)btc_profile_cb_get(BTC_PID_HD);
    if (btc_hd_cb) {
...
static void btc_hd_init(void)
{
    ...
    if (ret != XR_HIDD_SUCCESS) {
        xr_hidd_cb_param_t param;
        param.init.status = ret;
        btc_hd_cb_to_app(XR_HIDD_INIT_EVT, &param);
    }
}
...
static void btc_hd_deinit(void)
{
    ...
    if (ret != XR_HIDD_SUCCESS) {
        ...
        btc_hd_cb_to_app(XR_HIDD_DEINIT_EVT, &param);
    }
}
 ...
 *
 ******************************************************************************/
static void btc_hd_register_app(xr_hidd_app_param_t *p_app_param, xr_hidd_qos_param_t *p_in_qos,
                                xr_hidd_qos_param_t *p_out_qos)
{
    BTC_TRACE_API("%s", __func__);
    xr_hidd_status_t ret = XR_HIDD_SUCCESS;
    do {
    ...
    if (ret != XR_HIDD_SUCCESS) {
        ...
        btc_hd_cb_to_app(XR_HIDD_REGISTER_APP_EVT, &param);
    }
    free_app_info_param();
}
...
static void btc_hd_unregister_app(bool need_deinit)
{
    ...
    if (ret != XR_HIDD_SUCCESS) {
        xr_hidd_cb_param_t param = {0};
        param.unregister_app.status = ret;
        btc_hd_cb_to_app(XR_HIDD_UNREGISTER_APP_EVT, &param);
    }
}
...
static void btc_hd_connect(BD_ADDR bd_addr)
{
    ...
    if (ret != XR_HIDD_SUCCESS) {
        ...
        param.open.conn_status = XR_HIDD_CONN_STATE_DISCONNECTED;
        memcpy(param.open.bd_addr, bd_addr, BD_ADDR_LEN);
        btc_hd_cb_to_app(XR_HIDD_OPEN_EVT, &param);
    }
}
...
static void btc_hd_disconnect(void)
{
    ...
    if (ret != XR_HIDD_SUCCESS) {
        ...
        btc_hd_cb_to_app(XR_HIDD_CLOSE_EVT, &param);
    }
}
 ...
 *
 ******************************************************************************/
static void btc_hd_send_report(xr_hidd_report_type_t type, uint8_t id, uint16_t len, uint8_t *p_data)
{
    ...
    do {
        ...
        case BTC_HD_ENABLED:
        case BTC_HD_DISCONNECTED:
            if (type == XR_HIDD_REPORT_TYPE_INTRDATA) {
        ...
        if (type == XR_HIDD_REPORT_TYPE_INTRDATA) {
            report.type = XR_HIDD_REPORT_TYPE_INPUT;
    ...
    if (ret != XR_HIDD_SUCCESS) {
        ...
        btc_hd_cb_to_app(XR_HIDD_SEND_REPORT_EVT, &param);
    }
}
...
static void btc_hd_report_error(uint8_t error)
{
    ...
    if (ret != XR_HIDD_SUCCESS) {
        ...
        btc_hd_cb_to_app(XR_HIDD_REPORT_ERR_EVT, &param);
    }
}
...
static void btc_hd_virtual_cable_unplug(void)
{
    ...
    if (ret != XR_HIDD_SUCCESS) {
        ...
        param.vc_unplug.conn_status = XR_HIDD_CONN_STATE_DISCONNECTED;
        btc_hd_cb_to_app(XR_HIDD_VC_UNPLUG_EVT, &param);
    }
}
...
void btc_hd_cb_handler(btc_msg_t *msg)
{
    ...
    case BTA_HD_GET_REPORT_EVT:
        ...
        btc_hd_cb_to_app(XR_HIDD_GET_REPORT_EVT, &param);
        break;
    case BTA_HD_SET_REPORT_EVT:
        ...
        btc_hd_cb_to_app(XR_HIDD_SET_REPORT_EVT, &param);
        break;
    case BTA_HD_SET_PROTOCOL_EVT:
        switch (p_data->set_protocol) {
        case HID_PAR_PROTOCOL_BOOT_MODE:
            param.set_protocol.protocol_mode = XR_HIDD_BOOT_MODE;
            break;
        case HID_PAR_PROTOCOL_REPORT:
            param.set_protocol.protocol_mode = XR_HIDD_REPORT_MODE;
            break;
        default:
            param.set_protocol.protocol_mode = XR_HIDD_UNSUPPORTED_MODE;
            break;
        }
        btc_hd_cb_to_app(XR_HIDD_SET_PROTOCOL_EVT, &param);
        break;
    case BTA_HD_INTR_DATA_EVT:
        ...
        btc_hd_cb_to_app(XR_HIDD_INTR_DATA_EVT, &param);
        break;
    case BTA_HD_VC_UNPLUG_EVT: {
