...
xr_err_t xr_bt_hid_device_register_app(xr_hidd_app_param_t* app_param, xr_hidd_qos_param_t* in_qos, xr_hidd_qos_param_t* out_qos)
{
    if (xr_bluedroid_get_status() != XR_BLUEDROID_STATUS_ENABLED) {
        return XR_ERR_INVALID_STATE;
    }
...
xr_err_t xr_bt_hid_device_connect(xr_bd_addr_t bd_addr)
{
    ...
    btc_hidd_args_t args;
    memset(&args, 0, sizeof(btc_hidd_args_t));
    memcpy(args.connect.bd_addr, bd_addr, sizeof(xr_bd_addr_t));
