...
static int decoder_resize(DECODER *dec)
{
    ...
    if (image_create(&dec->cur, dec->edged_width, dec->edged_height) ||
        ...
        image_create(&dec->refn[1], dec->edged_width,
                     dec->edged_height)) /* Support B-frame to reference last 2 frame */
    ...
    // dec->qscale = xvid_malloc(sizeof(int) * dec->mb_width * dec->mb_height, CACHE_LINE);
...
int decoder_create(xvid_dec_create_t *create)
{
    ...
    if (create->fourcc ==
        ((int)('X') | ((int)('V') << 8) | ((int)('I') << 16) | ((int)('D') << 24))) { /* XVID */
...
static void decoder_mbintra(DECODER *dec, MACROBLOCK *pMB, const uint32_t x_pos,
                            const uint32_t y_pos, const uint32_t acpred_flag, const uint32_t cbp,
                            Bitstream *bs, const uint32_t quant, const uint32_t intra_dc_threshold,
                            const unsigned int bound)
{
    ...
    for (i = 0; i < 6; i++) {
        ...
        //start_timer();
        predict_acdc(dec->mbs, x_pos, y_pos, dec->mb_width, i, &block[i * 64], iQuant, iDcScaler,
                     predictors, bound);
        if (!acpred_flag) {
...
static void decoder_mb_decode(DECODER *dec, const uint32_t cbp, Bitstream *bs, uint8_t *pY_Cur,
                              uint8_t *pU_Cur, uint8_t *pV_Cur, const MACROBLOCK *pMB)
{
    ...
    typedef void (*get_inter_block_function_t)(Bitstream *bs, int16_t *block, int direction,
                                               const int quant, const uint16_t *matrix);
    // typedef void (*add_residual_function_t)(uint8_t *predicted_block, const int16_t *residual,
    //                                         int stride);

    const get_inter_block_function_t get_inter_block =
            (dec->quant_type == 0) ? (get_inter_block_function_t)get_inter_block_h263 :
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (cbp & (1 << (5 - i))) {
            ...
            //start_timer();
            get_inter_block(bs, &data[0], direction, iQuant,
...
static void __inline validate_vector(VECTOR *mv, unsigned int x_pos, unsigned int y_pos,
                                     const DECODER *dec)
{
...
#define CHECK_MV(mv)                                                                               \
    do {                                                                                           \
        if ((mv).x > xborder_high) {                                                               \
            DPRINTF(XVID_DEBUG_MV, "mv.x > max -- %d > %d, MB %d, %d", (mv).x, xborder_high,       \
                    x_pos, y_pos);                                                                 \
            (mv).x = xborder_high;                                                                 \
        } else if ((mv).x < xborder_low) {                                                         \
            DPRINTF(XVID_DEBUG_MV, "mv.x < min -- %d < %d, MB %d, %d", (mv).x, xborder_low, x_pos, \
                    y_pos);                                                                        \
            (mv).x = xborder_low;                                                                  \
        }                                                                                          \
        if ((mv).y > yborder_high) {                                                               \
            DPRINTF(XVID_DEBUG_MV, "mv.y > max -- %d > %d, MB %d, %d", (mv).y, yborder_high,       \
                    x_pos, y_pos);                                                                 \
            (mv).y = yborder_high;                                                                 \
        } else if ((mv).y < yborder_low) {                                                         \
            DPRINTF(XVID_DEBUG_MV, "mv.y < min -- %d < %d, MB %d, %d", (mv).y, yborder_low, x_pos, \
                    y_pos);                                                                        \
            (mv).y = yborder_low;                                                                  \
        }                                                                                          \
    } while (0)
...
static void decoder_mbinter(DECODER *dec, const MACROBLOCK *pMB, const uint32_t x_pos,
                            ...
                            const uint32_t rounding, const int ref, const int bvop)
{
    ...
    if ((pMB->mode != MODE_INTER4V) || (bvop)) { /* INTER, INTER_Q, NOT_CODED, FORWARD, BACKWARD */
        ...
        //                                 dec->qtmp.y + 128, 16 * x_pos, 16 * y_pos, mv[0].x, mv[0].y,
        //                                 stride, rounding);
        // else
            interpolate16x16_switch(dec->cur.y, dec->refn[ref].y, 16 * x_pos, 16 * y_pos, mv[0].x,
                                    mv[0].y, stride, rounding);
        ...
        //                               stride, rounding);
        //     interpolate8x8_quarterpel(dec->cur.y, dec->refn[0].y, dec->qtmp.y, dec->qtmp.y + 64,
        //                               dec->qtmp.y + 128, 16 * x_pos + 8, 16 * y_pos, mv[1].x,
        //                               mv[1].y, stride, rounding);
        //     interpolate8x8_quarterpel(dec->cur.y, dec->refn[0].y, dec->qtmp.y, dec->qtmp.y + 64,
        //                               dec->qtmp.y + 128, 16 * x_pos, 16 * y_pos + 8, mv[2].x,
        //                               mv[2].y, stride, rounding);
        //     interpolate8x8_quarterpel(dec->cur.y, dec->refn[0].y, dec->qtmp.y, dec->qtmp.y + 64,
        //                               dec->qtmp.y + 128, 16 * x_pos + 8, 16 * y_pos + 8, mv[3].x,
        //                               mv[3].y, stride, rounding);
        // } else {
            interpolate8x8_switch(dec->cur.y, dec->refn[0].y, 16 * x_pos, 16 * y_pos, mv[0].x,
                                  mv[0].y, stride, rounding);
            interpolate8x8_switch(dec->cur.y, dec->refn[0].y, 16 * x_pos + 8, 16 * y_pos, mv[1].x,
                                  mv[1].y, stride, rounding);
            interpolate8x8_switch(dec->cur.y, dec->refn[0].y, 16 * x_pos, 16 * y_pos + 8, mv[2].x,
                                  mv[2].y, stride, rounding);
    ...
    /* chroma */
    interpolate8x8_switch(dec->cur.u, dec->refn[ref].u, 8 * x_pos, 8 * y_pos, uv_dx, uv_dy, stride2,
                          rounding);
    interpolate8x8_switch(dec->cur.v, dec->refn[ref].v, 8 * x_pos, 8 * y_pos, uv_dx, uv_dy, stride2,
                          rounding);
...
static void decoder_mbinter_field(DECODER *dec, const MACROBLOCK *pMB, const uint32_t x_pos,
                                  const uint32_t y_pos, const uint32_t cbp, Bitstream *bs,
                                  const uint32_t rounding, const int ref, const int bvop)
{
    uint32_t stride = dec->edged_width;
    ...
    if ((pMB->mode != MODE_INTER4V) || (bvop)) /* INTER, INTER_Q, NOT_CODED, FORWARD, BACKWARD */
    {
        ...
        if (dec->quarterpel) {
            ...
            interpolate8x8_switch(dec->cur.y, dec->refn[ref].y + pMB->field_for_top * stride,
                                  16 * x_pos, 8 * y_pos, mv[0].x, mv[0].y >> 1, 2 * stride,
                                  rounding);
            /* top field right part */
            interpolate8x8_switch(dec->cur.y, dec->refn[ref].y + pMB->field_for_top * stride,
                                  16 * x_pos + 8, 8 * y_pos, mv[0].x, mv[0].y >> 1, 2 * stride,
                                  rounding);

            /* Interpolate bottom field left part(we use double stride for every 2nd line) */
            interpolate8x8_switch(dec->cur.y + stride,
                                  dec->refn[ref].y + pMB->field_for_bot * stride, 16 * x_pos,
                                  8 * y_pos, mv[1].x, mv[1].y >> 1, 2 * stride, rounding);
            /* Bottom field right part */
            interpolate8x8_switch(dec->cur.y + stride,
                                  dec->refn[ref].y + pMB->field_for_bot * stride, 16 * x_pos + 8,
                                  8 * y_pos, mv[1].x, mv[1].y >> 1, 2 * stride, rounding);

            /* Interpolate field1 U */
            interpolate8x4_switch(dec->cur.u, dec->refn[ref].u + pMB->field_for_top * stride2,
                                  8 * x_pos, 4 * y_pos, uvtop_dx, DIV2ROUND(uvtop_dy), stride,
                                  rounding);
            ...
            interpolate8x4_switch(dec->cur.u + stride2,
                                  dec->refn[ref].u + pMB->field_for_bot * stride2, 8 * x_pos,
                                  4 * y_pos, uvbot_dx, DIV2ROUND(uvbot_dy), stride, rounding);

            /* Interpolate field2 V */
            interpolate8x4_switch(dec->cur.v + stride2,
                                  dec->refn[ref].v + pMB->field_for_bot * stride2, 8 * x_pos,
                                  4 * y_pos, uvbot_dx, DIV2ROUND(uvbot_dy), stride, rounding);
        }
    } else {
...
// static void decoder_mbgmc(DECODER *dec, MACROBLOCK *const pMB, const uint32_t x_pos,
...
//     gmc_data->predict_8x8(gmc_data, dec->cur.u + y_pos * 8 * stride2 + x_pos * 8, dec->refn[0].u,
//                           dec->cur.v + y_pos * 8 * stride2 + x_pos * 8, dec->refn[0].v, stride2,
//                           stride2, x_pos, y_pos, rounding);
    ...
    for (y = 0; y < mb_height; y++) {
        for (x = 0; x < mb_width; x++) {
            ...
            if (check_resync_marker(bs, 0)) {
                bound = read_video_packet_header(bs, dec, 0, &quant, NULL, NULL,
                                                 &intra_dc_threshold);
                x = bound % mb_width;
            ...
            mb->quant = quant;
            mb->mvs[0].x = mb->mvs[0].y = mb->mvs[1].x = mb->mvs[1].y = mb->mvs[2].x =
                    mb->mvs[2].y = mb->mvs[3].x = mb->mvs[3].y = 0;
...
static void get_motion_vector(DECODER *dec, Bitstream *bs, int x, int y, int k, VECTOR *ret_mv,
...
static void get_motion_vector_interlaced(DECODER *dec, Bitstream *bs, int x, int y, int k,
                                         MACROBLOCK *pMB, int fcode, const int bound)
{
    const int scale_fac = 1 << (fcode - 1);
    const int high = (32 * scale_fac) - 1;
    const int low = ((-32) * scale_fac);
...
static void decoder_pframe(DECODER *dec, Bitstream *bs, int rounding, int quant, int fcode,
                           int intra_dc_threshold, const WARPPOINTS *const gmc_warp)
{
    uint32_t x, y;
    uint32_t bound;
    int cp_mb, st_mb;
    ...
    if (!dec->is_edged[0]) {
        //start_timer();
        image_setedges(&dec->refn[0], dec->edged_width, dec->edged_height, dec->width, dec->height,
                       dec->bs_version);
        dec->is_edged[0] = 1;
        //stop_edges_timer();
    }
    ...
    if (gmc_warp) {
        ...
        // generate_GMCparameters(dec->sprite_warping_points, dec->sprite_warping_accuracy, gmc_warp,
    ...
    for (y = 0; y < mb_height; y++) {
        ...
        for (x = 0; x < mb_width; x++) {
            ...
            if (check_resync_marker(bs, fcode - 1)) {
                bound = read_video_packet_header(bs, dec, fcode - 1, &quant, &fcode, NULL,
                                                 &intra_dc_threshold);
                x = bound % mb_width;
                y = MIN((bound / mb_width), (mb_height - 1));
            }
            ...
            if (!(BitstreamGetBit(bs))) { /* block _is_ coded */
                ...
                //     output_slice(&dec->cur, dec->edged_width, dec->width, dec->out_frm, st_mb, y,
                //                  cp_mb);
                //     cp_mb = 0;
                // }
                // st_mb = x + 1;
...
static void get_b_motion_vector(Bitstream *bs, VECTOR *mv, int fcode, const VECTOR pmv,
                                const DECODER *const dec, const int x, const int y)
{
    const int scale_fac = 1 << (fcode - 1);
    const int high = (32 * scale_fac) - 1;
    const int low = ((-32) * scale_fac);
...
static void decoder_bf_interpolate_mbinter(DECODER *dec, IMAGE forward, IMAGE backward,
                                           MACROBLOCK *pMB, const uint32_t x_pos,
                                           const uint32_t y_pos, Bitstream *bs, const int direct)
{
    uint32_t stride = dec->edged_width;
    uint32_t stride2 = stride / 2;
    int uv_dx, uv_dy;
    ...
    if (!direct) {
        ...
        if (dec->quarterpel) { /* for qpel the /2 shall be done before summation. We've done it right in the encoder in the past. */
            ...
            if (dec->bs_version <= BS_VERSION_BUGGY_CHROMA_ROUNDING) {
                ...
                uv_dx = (pMB->mvs[0].x / 2) + (pMB->mvs[1].x / 2) + (pMB->mvs[2].x / 2) +
                        (pMB->mvs[3].x / 2);
                uv_dy = (pMB->mvs[0].y / 2) + (pMB->mvs[1].y / 2) + (pMB->mvs[2].y / 2) +
                        (pMB->mvs[3].y / 2);
                b_uv_dx = (pMB->b_mvs[0].x / 2) + (pMB->b_mvs[1].x / 2) + (pMB->b_mvs[2].x / 2) +
                          (pMB->b_mvs[3].x / 2);
                b_uv_dy = (pMB->b_mvs[0].y / 2) + (pMB->b_mvs[1].y / 2) + (pMB->b_mvs[2].y / 2) +
                          (pMB->b_mvs[3].y / 2);
            }
        } else {
    ...
    //                                     dec->qtmp.y + 128, 16 * x_pos, 16 * y_pos, pMB->mvs[0].x,
    //                                     pMB->mvs[0].y, stride, 0);
    //     } else {
    //         interpolate8x8_quarterpel(dec->cur.y, forward.y, dec->qtmp.y, dec->qtmp.y + 64,
    //                                   dec->qtmp.y + 128, 16 * x_pos, 16 * y_pos, pMB->mvs[0].x,
    //                                   pMB->mvs[0].y, stride, 0);
    //         interpolate8x8_quarterpel(dec->cur.y, forward.y, dec->qtmp.y, dec->qtmp.y + 64,
    //                                   dec->qtmp.y + 128, 16 * x_pos + 8, 16 * y_pos, pMB->mvs[1].x,
    //                                   pMB->mvs[1].y, stride, 0);
    //         interpolate8x8_quarterpel(dec->cur.y, forward.y, dec->qtmp.y, dec->qtmp.y + 64,
    //                                   dec->qtmp.y + 128, 16 * x_pos, 16 * y_pos + 8, pMB->mvs[2].x,
    //                                   pMB->mvs[2].y, stride, 0);
    //         interpolate8x8_quarterpel(dec->cur.y, forward.y, dec->qtmp.y, dec->qtmp.y + 64,
    //                                   dec->qtmp.y + 128, 16 * x_pos + 8, 16 * y_pos + 8,
    //                                   pMB->mvs[3].x, pMB->mvs[3].y, stride, 0);
    //     }
    // } else {
        interpolate8x8_switch(dec->cur.y, forward.y, 16 * x_pos, 16 * y_pos, pMB->mvs[0].x,
                              pMB->mvs[0].y, stride, 0);
        interpolate8x8_switch(dec->cur.y, forward.y, 16 * x_pos + 8, 16 * y_pos, pMB->mvs[1].x,
                              pMB->mvs[1].y, stride, 0);
        interpolate8x8_switch(dec->cur.y, forward.y, 16 * x_pos, 16 * y_pos + 8, pMB->mvs[2].x,
                              pMB->mvs[2].y, stride, 0);
        interpolate8x8_switch(dec->cur.y, forward.y, 16 * x_pos + 8, 16 * y_pos + 8, pMB->mvs[3].x,
                              pMB->mvs[3].y, stride, 0);
    // }
    ...
    //     }
    // } else {
        interpolate8x8_add_switch(dec->cur.y, backward.y, 16 * x_pos, 16 * y_pos, pMB->b_mvs[0].x,
                                  pMB->b_mvs[0].y, stride, 0);
        interpolate8x8_add_switch(dec->cur.y, backward.y, 16 * x_pos + 8, 16 * y_pos,
                                  pMB->b_mvs[1].x, pMB->b_mvs[1].y, stride, 0);
        interpolate8x8_add_switch(dec->cur.y, backward.y, 16 * x_pos, 16 * y_pos + 8,
                                  pMB->b_mvs[2].x, pMB->b_mvs[2].y, stride, 0);
        interpolate8x8_add_switch(dec->cur.y, backward.y, 16 * x_pos + 8, 16 * y_pos + 8,
                                  pMB->b_mvs[3].x, pMB->b_mvs[3].y, stride, 0);
    // }

    interpolate8x8_add_switch(dec->cur.u, backward.u, 8 * x_pos, 8 * y_pos, b_uv_dx, b_uv_dy,
                              stride2, 0);
    interpolate8x8_add_switch(dec->cur.v, backward.v, 8 * x_pos, 8 * y_pos, b_uv_dx, b_uv_dy,
                              stride2, 0);
...
static __inline int32_t get_dbquant(Bitstream *bs)
{
 ...
 * 0001   3
 */
static int32_t __inline get_mbtype(Bitstream *bs)
{
    int32_t mb_type;
...
static int __inline get_resync_len_b(const int fcode_backward, const int fcode_forward)
{
...
static void decoder_bframe(DECODER *dec, Bitstream *bs, int quant, int fcode_forward,
                           int fcode_backward)
{
    uint32_t x, y;
    VECTOR mv;
    ...
    if (!dec->is_edged[1]) {
        //start_timer();
        image_setedges(&dec->refn[1], dec->edged_width, dec->edged_height, dec->width, dec->height,
                       dec->bs_version);
        dec->is_edged[1] = 1;
        //stop_edges_timer();
    }
    ...
    for (y = 0; y < dec->mb_height; y++) {
        ...
        for (x = 0; x < dec->mb_width; x++) {
            ...
            mv = mb->b_mvs[0] = mb->b_mvs[1] = mb->b_mvs[2] = mb->b_mvs[3] = mb->mvs[0] =
                    mb->mvs[1] = mb->mvs[2] = mb->mvs[3] = zeromv;
            mb->quant = quant;
       ...
       */
            ...
            if (check_resync_marker(bs, resync_len)) {
                int bound = read_video_packet_header(bs, dec, resync_len, &quant, &fcode_forward,
            ...
            case MODE_DIRECT_NONE_MV:
                for (i = 0; i < 4; i++) {
                    ...
                    mb->b_mvs[i].x = (mv.x) ? mb->mvs[i].x - last_mb->mvs[i].x :
                                              last_mb->mvs[i].x * (dec->time_bp - dec->time_pp) /
                                                      dec->time_pp;
                    mb->b_mvs[i].y = (mv.y) ? mb->mvs[i].y - last_mb->mvs[i].y :
                                              last_mb->mvs[i].y * (dec->time_bp - dec->time_pp) /
                                                      dec->time_pp;
                }

                decoder_bf_interpolate_mbinter(dec, dec->refn[1], dec->refn[0], mb, x, y, bs, 1);
                break;
            ...
            case MODE_INTERPOLATE:
                ...
                decoder_bf_interpolate_mbinter(dec, dec->refn[1], dec->refn[0], mb, x, y, bs, 0);
                break;
...
static void decoder_output(DECODER *dec, IMAGE *img, MACROBLOCK *mbs, xvid_dec_frame_t *frame,
                           xvid_dec_stats_t *stats, int coding_type, int quant)
{
    ...
    //     image_postproc(&dec->postproc, &dec->tmp, dec->edged_width, mbs, dec->mb_width,
    //                    dec->mb_height, dec->mb_width, frame->general, brightness, dec->frames,
    //                    (coding_type == B_VOP), dec->num_threads);
    //     img = &dec->tmp;
    // }

    if ((frame->output.plane[0] != NULL) && (frame->output.stride[0] >= dec->width)) {
        image_output(img, dec->width, dec->height, dec->edged_width,
                     (uint8_t **)frame->output.plane, frame->output.stride, frame->output.csp,
                     dec->interlacing);
    }
...
int decoder_decode(DECODER *dec, xvid_dec_frame_t *frame, xvid_dec_stats_t *stats)
{
    ...
    if (XVID_VERSION_MAJOR(frame->version) != 1 ||
        (stats && XVID_VERSION_MAJOR(stats->version) != 1)) /* v1.x.x */
        return XVID_ERR_VERSION;
...
repeat:
    ...
    coding_type = BitstreamReadHeaders(&bs, dec, &rounding, &quant, &fcode_forward, &fcode_backward,
                                       &intra_dc_threshold, &gmc_warp);

    DPRINTF(XVID_DEBUG_HEADER,
            "coding_type=%i,  packed=%i,  time=%"
#if defined(_MSC_VER)
            "I64"
#else
            "ll"
    ...
    if (dec->packed_mode && coding_type == N_VOP) {
        ...
        case I_VOP:
            ...
            break;
        case P_VOP:
            decoder_pframe(dec, &bs, rounding, quant, fcode_forward, intra_dc_threshold, NULL);
            break;
        case S_VOP:
            decoder_pframe(dec, &bs, rounding, quant, fcode_forward, intra_dc_threshold, &gmc_warp);
            break;
        case N_VOP:
