...
#include <windows.h>
#elif defined(ARCH_IS_IA32) || defined(ARCH_IS_X86_64) ||                                          \
        (defined(ARCH_IS_PPC) && !defined(__amigaos4__))
#include <signal.h>
#include <setjmp.h>
...
/* detect cpu flags  */
static unsigned int detect_cpu_flags(void)
{
    ...
    if ((cpu_flags & (XVID_CPU_SSE2 | XVID_CPU_SSE3 | XVID_CPU_SSE41)) &&
        sigill_check(sse2_os_trigger))
...
static int xvid_gbl_info(xvid_gbl_info_t *info)
{
...
static int xvid_gbl_convert(xvid_gbl_convert_t *convert)
{
    int width;
    int height;
    int width2;
    ...
    case XVID_CSP_YV12:
        ...
        img.u = (uint8_t *)convert->input.plane[0] + convert->input.stride[0] * height +
                (convert->input.stride[0] / 2) * height2;
        image_output(&img, width, height, width, (uint8_t **)convert->output.plane,
                     convert->output.stride, convert->output.csp, convert->interlacing);
        break;
    ...
    case XVID_CSP_INTERNAL:
        ...
        image_output(&img, width, height, convert->input.stride[0],
                     (uint8_t **)convert->output.plane, convert->output.stride, convert->output.csp,
                     convert->interlacing);
        break;
...
int xvid_decore(void *handle, int opt, void *param1, void *param2)
{
    ...
    case XVID_DEC_DECODE:
        return decoder_decode((DECODER *)handle, (xvid_dec_frame_t *)param1,
                              (xvid_dec_stats_t *)param2);
