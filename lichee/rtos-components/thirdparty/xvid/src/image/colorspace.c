...
#define MAKE_COLORSPACE(NAME, SIZE, PIXELS, VPIXELS, FUNC, C1, C2, C3, C4)                         \
    void NAME(uint8_t *x_ptr, int x_stride, uint8_t *y_ptr, uint8_t *u_ptr, uint8_t *v_ptr,        \
...
#define READ_RGB16_Y(ROW, UVID, C1, C2, C3, C4)                                                    \
    ...
    g##UVID += g = C1##_G(rgb);                                                                    \
    r##UVID += r = C1##_R(rgb);                                                                    \
    y_ptr[(ROW)*y_stride + 0] = (uint8_t)((FIX_IN(Y_R_IN) * r + FIX_IN(Y_G_IN) * g +               \
                                           FIX_IN(Y_B_IN) * b + FIX_ROUND) >>                      \
                                          SCALEBITS_IN) +                                          \
                                Y_ADD_IN;                                                          \
    ...
    g##UVID += g = C1##_G(rgb);                                                                    \
    r##UVID += r = C1##_R(rgb);                                                                    \
    y_ptr[(ROW)*y_stride + 1] = (uint8_t)((FIX_IN(Y_R_IN) * r + FIX_IN(Y_G_IN) * g +               \
                                           FIX_IN(Y_B_IN) * b + FIX_ROUND) >>                      \
                                          SCALEBITS_IN) +                                          \
                                Y_ADD_IN;

#define READ_RGB16_UV(UV_ROW, UVID)                                                                \
    u_ptr[(UV_ROW)*uv_stride] = (uint8_t)((FIX_IN(U_B_IN) * b##UVID + 4 * FIX_ROUND -              \
                                           FIX_IN(U_G_IN) * g##UVID - FIX_IN(U_R_IN) * r##UVID) >> \
                                          (SCALEBITS_IN + 2)) +                                    \
                                U_ADD_IN;                                                          \
    v_ptr[(UV_ROW)*uv_stride] = (uint8_t)((FIX_IN(V_R_IN) * r##UVID - FIX_IN(V_G_IN) * g##UVID -   \
                                           FIX_IN(V_B_IN) * b##UVID + 4 * FIX_ROUND) >>            \
                                          (SCALEBITS_IN + 2)) +                                    \
                                V_ADD_IN;

#define RGB16_TO_YV12_ROW(SIZE, C1, C2, C3, C4) /* nothing */
...
#define RGB16I_TO_YV12_ROW(SIZE, C1, C2, C3, C4) /* nothing */
...
#define READ_RGB_UV(UV_ROW, UVID)                                                                  \
    u_ptr[(UV_ROW)*uv_stride] = (uint8_t)((FIX_IN(U_B_IN) * b##UVID + 4 * FIX_ROUND -              \
                                           FIX_IN(U_R_IN) * r##UVID - FIX_IN(U_G_IN) * g##UVID) >> \
                                          (SCALEBITS_IN + 2)) +                                    \
                                U_ADD_IN;                                                          \
    v_ptr[(UV_ROW)*uv_stride] = (uint8_t)((FIX_IN(V_R_IN) * r##UVID - FIX_IN(V_G_IN) * g##UVID -   \
                                           FIX_IN(V_B_IN) * b##UVID + 4 * FIX_ROUND) >>            \
...
#define RGB_TO_YV12_ROW(SIZE, C1, C2, C3, C4) /* nothing */
...
#define RGBI_TO_YV12_ROW(SIZE, C1, C2, C3, C4) /* nothing */
...
#define READ_YUYV_UV(UV_ROW, ROW1, ROW2, C1, C2, C3, C4)                                           \
    u_ptr[(UV_ROW)*uv_stride] =                                                                    \
            (x_ptr[(ROW1)*x_stride + (C2)] + x_ptr[(ROW2)*x_stride + (C2)] + 1) / 2;               \
    v_ptr[(UV_ROW)*uv_stride] =                                                                    \
            (x_ptr[(ROW1)*x_stride + (C4)] + x_ptr[(ROW2)*x_stride + (C4)] + 1) / 2;

#define YUYV_TO_YV12_ROW(SIZE, C1, C2, C3, C4) /* nothing */
#define YUYV_TO_YV12(SIZE, C1, C2, C3, C4)                                                         \
    READ_YUYV_Y(0, C1, C2, C3, C4)                                                                 \
    READ_YUYV_Y(1, C1, C2, C3, C4)                                                                 \
    READ_YUYV_UV(0, 0, 1, C1, C2, C3, C4)

#define YUYVI_TO_YV12_ROW(SIZE, C1, C2, C3, C4) /* nothing */
...
#define MK_RGB555(R, G, B)                                                                         \
    ((MAX(0, MIN(255, R)) << 7) & 0x7c00) | ((MAX(0, MIN(255, G)) << 2) & 0x03e0) |                \
            ((MAX(0, MIN(255, B)) >> 3) & 0x001f)

#define MK_RGB565(R, G, B)                                                                         \
    ((MAX(0, MIN(255, R)) << 8) & 0xf800) | ((MAX(0, MIN(255, G)) << 3) & 0x07e0) |                \
            ((MAX(0, MIN(255, B)) >> 3) & 0x001f)
...
#define WRITE_RGB(SIZE, ROW, UV_ROW, C1, C2, C3, C4)                                               \
    ...
    x_ptr[(ROW)*x_stride + (SIZE) + (C3)] =                                                        \
            MAX(0, MIN(255, (rgb_y + b_u##UV_ROW) >> SCALEBITS_OUT));                              \
    x_ptr[(ROW)*x_stride + (SIZE) + (C2)] =                                                        \
            MAX(0, MIN(255, (rgb_y - g_uv##UV_ROW) >> SCALEBITS_OUT));                             \
    x_ptr[(ROW)*x_stride + (SIZE) + (C1)] =                                                        \
            MAX(0, MIN(255, (rgb_y + r_v##UV_ROW) >> SCALEBITS_OUT));                              \
    if ((SIZE) > 3)                                                                                \
...
void yv12_to_yv12_c(uint8_t *y_dst, uint8_t *u_dst, uint8_t *v_dst, int y_dst_stride,
                    int uv_dst_stride, uint8_t *y_src, uint8_t *u_src, uint8_t *v_src,
                    int y_src_stride, int uv_src_stride, int width, int height, int vflip)
{
    int width2 = width / 2;
