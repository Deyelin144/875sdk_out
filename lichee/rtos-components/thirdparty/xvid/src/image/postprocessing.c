...
void image_postproc(XVID_POSTPROC *tbls, IMAGE *img, int edged_width, const MACROBLOCK *mbs,
                    int mb_width, int mb_height, int mb_stride, int flags, int brightness,
                    int frame_num, int bvop, int threads)
{
    int k;
#ifndef HAVE_PTHREAD
    int num_threads = 1;
    ...
    for (k = 1; k < num_threads; k++) {
        pthread_create(&data[k].handle, NULL, (void *)stripe_deblock_h, (void *)&data[k]);
    }
#endif
    ...
    /* create threads */
    for (k = 1; k < num_threads; k++) {
        pthread_create(&data[k].handle, NULL, (void *)stripe_deblock_v, (void *)&data[k]);
    }
#endif
    ...
    if ((flags & XVID_FILMEFFECT)) {
        add_noise(tbls, img->y, img->y, edged_width, mb_width * 16, mb_height * 16, frame_num % 3,
                  tbls->prev_quant);
    }
    ...
    if (eq_cnt < THR2) { /* Default mode */                                                        \
        ...
        if (tbls->xvid_abs_tbl[(s[4] - s[5]) + 255] < quant) {                                     \
            ...
            a32 = ((s[5] << 1) - s[6] * 5 + s[7] * 5 - (s[8] << 1));                               \
                                                                                                   \
            diff = (5 * ((SIGN(a30) * MIN(FAST_ABS(a30), MIN(FAST_ABS(a31), FAST_ABS(a32)))) -     \
                         a30) +                                                                    \
                    32) >>                                                                         \
                   6;                                                                              \
            limit = (s[4] - s[5]) / 2;                                                             \
        ...
        if (((max - min)) < 2 * quant) {                                                           \
            ...
            p9 = (tbls->xvid_abs_tbl[(s[8] - s[9]) + 255] < quant) ? s[9] : s[8];                  \
                                                                                                   \
            *v[1] = (uint8_t)((6 * p0 + (s[1] << 2) + (s[2] << 1) + (s[3] << 1) + s[4] + s[5] +    \
                               8) >>                                                               \
                              4);                                                                  \
            *v[2] = (uint8_t)(((p0 << 2) + (s[1] << 1) + (s[2] << 2) + (s[3] << 1) + (s[4] << 1) + \
                               s[5] + s[6] + 8) >>                                                 \
                              4);                                                                  \
            *v[3] = (uint8_t)(((p0 << 1) + (s[1] << 1) + (s[2] << 1) + (s[3] << 2) + (s[4] << 1) + \
                               (s[5] << 1) + s[6] + s[7] + 8) >>                                   \
                              4);                                                                  \
            *v[4] = (uint8_t)((p0 + s[1] + (s[2] << 1) + (s[3] << 1) + (s[4] << 2) + (s[5] << 1) + \
                               (s[6] << 1) + s[7] + s[8] + 8) >>                                   \
                              4);                                                                  \
            *v[5] = (uint8_t)((s[1] + s[2] + (s[3] << 1) + (s[4] << 1) + (s[5] << 2) +             \
                               (s[6] << 1) + (s[7] << 1) + s[8] + p9 + 8) >>                       \
                              4);                                                                  \
            *v[6] = (uint8_t)((s[2] + s[3] + (s[4] << 1) + (s[5] << 1) + (s[6] << 2) +             \
                               (s[7] << 1) + (s[8] << 1) + (p9 << 1) + 8) >>                       \
                              4);                                                                  \
            *v[7] = (uint8_t)((s[3] + s[4] + (s[5] << 1) + (s[6] << 1) + (s[7] << 2) +             \
                               (s[8] << 1) + (p9 << 2) + 8) >>                                     \
                              4);                                                                  \
            *v[8] = (uint8_t)((s[4] + s[5] + (s[6] << 1) + (s[7] << 1) + (s[8] << 2) + 6 * p9 +    \
                               8) >>                                                               \
                              4);                                                                  \
        }                                                                                          \
    }
...
void add_noise(XVID_POSTPROC *tbls, uint8_t *dst, uint8_t *src, int stride, int width, int height,
               int shiftptr, int quant)
{
    int x, y;
    int shift = 0;
    int add = (quant < 5) ? 3 : 0;
    ...
    for (y = 0; y < height; y++) {
        ...
        shift &= ~7;
        for (x = 0; x < width; x++) {
            const int n = tbls->xvid_prev_shift[y][0 + add][x] +
                          tbls->xvid_prev_shift[y][1 + add][x] +
