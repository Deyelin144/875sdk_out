...
static void bs_get_matrix(Bitstream *bs, uint8_t *matrix)
{
    int i = 0;
    int last, value = 0;
 ...
 * returns mbpos
 */
int read_video_packet_header(Bitstream *bs, DECODER *dec, const int addbits, int *quant,
                             int *fcode_forward, int *fcode_backward, int *intra_dc_threshold)
{
    ...
    if (hec) {
        ...
        for (time_base = 0; BitstreamGetBit(bs) != 0; time_base++)
            ; /* modulo_time_base */
        READ_MARKER();
        ...
        if (dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) {
            *intra_dc_threshold = intra_dc_threshold_table[BitstreamGetBits(bs, 3)];
            ...
            if (dec->reduced_resolution_enable && dec->shape == VIDOBJLAY_SHAPE_RECTANGULAR &&
...
static void read_vol_complexity_estimation_header(Bitstream *bs, DECODER *dec)
{
...
static void read_vop_complexity_estimation_header(Bitstream *bs, DECODER *dec, int coding_type)
{
    ESTIMATION *e = &dec->estimation;
    ...
    if (e->method == 0 || e->method == 1) {
        if (coding_type == I_VOP) {
            if (e->opaque)
                BitstreamSkip(bs, 8); /* dcecs_opaque */
            if (e->transparent)
                BitstreamSkip(bs, 8); /* */
            if (e->intra_cae)
                BitstreamSkip(bs, 8); /* */
...
int BitstreamReadHeaders(Bitstream *bs, DECODER *dec, uint32_t *rounding, uint32_t *quant,
                         uint32_t *fcode_forward, uint32_t *fcode_backward,
                         uint32_t *intra_dc_threshold, WARPPOINTS *gmc_warp)
{
    uint32_t vol_ver_id;
    ...
    while ((BitstreamPos(bs) >> 3) + 4 <= bs->length) {
        ...
        if (start_code == VISOBJSEQ_START_CODE) {
            ...
            if (dec->shape != VIDOBJLAY_SHAPE_BINARY_ONLY) {
                ...
                dec->sprite_enable =
                        BitstreamGetBits(bs, (vol_ver_id == 1 ? 1 : 2)); /* sprite_enable */
                ...
                if (dec->sprite_enable == SPRITE_STATIC || dec->sprite_enable == SPRITE_GMC) {
                    ...
                    dec->sprite_warping_points =
                            BitstreamGetBits(bs, 6); /* no_of_sprite_warping_points */
                    dec->sprite_warping_accuracy =
                            BitstreamGetBits(bs, 2); /* sprite_warping_accuracy */
                    dec->sprite_brightness_change = BitstreamGetBits(bs, 1); /* brightness_change */
                    if (dec->sprite_enable != SPRITE_GMC) {
                        low_latency_sprite_enable =
                                BitstreamGetBits(bs, 1); /* low_latency_sprite_enable */
                    }
                }
                ...
                dec->complexity_estimation_disable =
                        BitstreamGetBit(bs); /* complexity estimation disable */
                ...
                if (vol_ver_id != 1) {
                    ...
                    dec->reduced_resolution_enable =
                            BitstreamGetBit(bs); /* reduced_resolution_vop_enable */
            ...
            if (dec->time_inc_bits) {
                time_increment =
                        (BitstreamGetBits(bs, dec->time_inc_bits)); /* vop_time_increment */
            }
            ...
            if ((dec->sprite_enable == SPRITE_STATIC || dec->sprite_enable == SPRITE_GMC) &&
                coding_type == S_VOP) {
                    printf("warmming : all S_VOP is not support!\n");
                }
            //     int i;
...
// static void bs_put_matrix(Bitstream *bs, const uint16_t *matrix)
// {
//     int i, j;
...
void BitstreamWriteUserData(Bitstream *const bs, const char *data, const unsigned int length)
{
    unsigned int i;
