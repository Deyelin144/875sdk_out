...
static void (*Predict_16x16_func)(const NEW_GMC_DATA *const This, uint8_t *dst, const uint8_t *src,
                                  int dststride, int srcstride, int x, int y, int rounding) = 0;
static void (*Predict_8x8_func)(const NEW_GMC_DATA *const This, uint8_t *uDst, const uint8_t *uSrc,
                                uint8_t *vDst, const uint8_t *vSrc, int dststride, int srcstride,
                                int x, int y, int rounding) = 0;
...
static const uint32_t MTab[16] = { MLT(0),  MLT(1),  MLT(2),  MLT(3), MLT(4),  MLT(5),
                                   MLT(6),  MLT(7),  MLT(8),  MLT(9), MLT(10), MLT(11),
                                   MLT(12), MLT(13), MLT(14), MLT(15) };
#undef MLT
...
static void Predict_16x16_C(const NEW_GMC_DATA *const This, uint8_t *dst, const uint8_t *src,
...
static void Predict_8x8_C(const NEW_GMC_DATA *const This, uint8_t *uDst, const uint8_t *uSrc,
                          uint8_t *vDst, const uint8_t *vSrc, int dststride, int srcstride, int x,
                          int y, int rounding)
{
...
static void get_average_mv_C(const NEW_GMC_DATA *const Dsp, VECTOR *const mv, int x, int y,
                             int qpel)
{
    int i, j;
    int vx = 0, vy = 0;
...
static void Predict_1pt_16x16_C(const NEW_GMC_DATA *const This, uint8_t *Dst, const uint8_t *Src,
                                int dststride, int srcstride, int x, int y, int rounding)
{
    const int W = This->sW;
    const int H = This->sH;
...
static void Predict_1pt_8x8_C(const NEW_GMC_DATA *const This, uint8_t *uDst, const uint8_t *uSrc,
                              uint8_t *vDst, const uint8_t *vSrc, int dststride, int srcstride,
                              int x, int y, int rounding)
{
    const int W = This->sW >> 1;
    const int H = This->sH >> 1;
    const int rho = 3 - This->accuracy;
...
static void get_average_mv_1pt_C(const NEW_GMC_DATA *const Dsp, VECTOR *const mv, int x, int y,
                                 int qpel)
{
...
static void (*GMC_Core_Lin_8)(uint8_t *Dst, const uint16_t *Offsets, const uint8_t *const Src0,
                              const int BpS, const int Rounder) = 0;
...
static void GMC_Core_Non_Lin_8(uint8_t *Dst, const uint16_t *Offsets, const uint8_t *const Src0,
                               const int srcstride, const int Rounder)
{
    int i;
    for (i = 0; i < 8; ++i) {
...
static void Predict_16x16_mmx(const NEW_GMC_DATA *const This, uint8_t *dst, const uint8_t *src,
                              int dststride, int srcstride, int x, int y, int rounding)
{
    const int W = This->sW;
    const int H = This->sH;
    ...
    for (j = 16; j > 0; --j) {
        ...
        Uo += dUy;
        Vo += dVy;
        if (W2 > (uint32_t)U && W2 > (uint32_t)(U + 15 * dUx) && H2 > (uint32_t)V &&
            H2 > (uint32_t)(V + 15 * dVx)) {
            uint32_t UV1, UV2;
            for (i = 0; i < 16; ++i) {
                uint32_t u = (U >> 16) << rho;
                uint32_t v = (V >> 16) << rho;
            ...
            if (UV1 + 7 * 16 == UV2)
                GMC_Core_Lin_8(dst, Offsets,
                               src + (Offsets[0] >> 4) + (Offsets[16] >> 4) * srcstride, srcstride,
                               Rounder);
            else
            ...
            if (UV1 + 7 * 16 == UV2)
                GMC_Core_Lin_8(dst + 8, Offsets + 8,
                               src + (Offsets[8] >> 4) + (Offsets[24] >> 4) * srcstride, srcstride,
                               Rounder);
            else
...
static void Predict_8x8_mmx(const NEW_GMC_DATA *const This, uint8_t *uDst, const uint8_t *uSrc,
                            uint8_t *vDst, const uint8_t *vSrc, int dststride, int srcstride, int x,
                            int y, int rounding)
{
    const int W = This->sW >> 1;
...
void init_GMC(const unsigned int cpu_flags)
{
    ...
    if ((cpu_flags & XVID_CPU_MMX) || (cpu_flags & XVID_CPU_MMXEXT) ||
        ...
        (cpu_flags & XVID_CPU_SSE) || (cpu_flags & XVID_CPU_SSE2) || (cpu_flags & XVID_CPU_SSE3) ||
        (cpu_flags & XVID_CPU_SSE41)) {
...
void generate_GMCparameters(int nb_pts, const int accuracy, const WARPPOINTS *const pts,
                            const int width, const int height, NEW_GMC_DATA *const gmc)
{
    gmc->sW = width << 4;
    gmc->sH = height << 4;
    ...
    if (nb_pts < 2 ||
        (pts->duv[2].x == 0 && pts->duv[2].y == 0 && pts->duv[1].x == 0 && pts->duv[1].y == 0)) {
...
void generate_GMCimage(const NEW_GMC_DATA *const gmc_data, /* [input] precalculated data */
                       const IMAGE *const pRef,            /* [input] */
                       const int mb_width, const int mb_height, const int stride, const int stride2,
                       ...
                       IMAGE *const pGMC)                  /* [output] full warped image */
{
    ...
    for (mj = 0; mj < (unsigned int)mb_height; mj++)
        for (mi = 0; mi < (unsigned int)mb_width; mi++) {
            ...
            if (pGMC) {
                gmc_data->predict_16x16(gmc_data, pGMC->y + mj * 16 * stride + mi * 16, pRef->y,
                                        stride, stride, mi, mj, rounding);

                gmc_data->predict_8x8(gmc_data, pGMC->u + mj * 8 * stride2 + mi * 8, pRef->u,
                                      pGMC->v + mj * 8 * stride2 + mi * 8, pRef->v, stride2,
                                      stride2, mi, mj, rounding);
            }
