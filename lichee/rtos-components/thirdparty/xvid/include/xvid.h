...
#define XVID_CSP_INTERNAL                                                                          \
    (1 << 13)                     /* decoder only: 4:2:0 planar, returns ptrs to internal buffers */
...
typedef struct {
    int version;
    unsigned int
            cpu_flags; /* [in:opt] zero = autodetect cpu; XVID_CPU_FORCE|{cpu features} = force cpu features */
...
#define XVID_GBL_INIT                                                                              \
    0 /* initialize xvidcore; must be called before using xvid_decore, or xvid_encore) */
...
#define XVID_REQPSNR                                                                               \
    (1 << 1) /* plugin requires psnr between the uncompressed and compressed image*/
...
typedef struct {
    ...
    xvid_image_t
            original; /* [out] after: points the original (uncompressed) copy of the current frame */
...
#define XVID_ENC_ENCODE                                                                            \
    2                      /* encode a frame: returns number of ouput bytes
...
#define XVID_GLOBAL_FORCE_VOL                                                                      \
    (1 << 4) /* when vol-based parameters are changed, insert an ivop NOT recommended */
#endif
#define XVID_GLOBAL_DIVX5_USERDATA                                                                 \
    (1 << 5) /* write divx5 userdata string 
...
#define XVID_VOL_REDUCED_ENABLE                                                                    \
    (1 << 4) /* enable reduced resolution vops: frames will be checked for rrv suitability */
...
#define XVID_VOP_GREYSCALE                                                                         \
    (1 << 6) /* enable greyscale only mode (even for  color input material chroma is ignored) */
...
#define XVID_VOP_RD_PSNRHVSM                                                                       \
    (1 << 14) /* use PSNR-HVS-M as metric for rate-distortion optimizations */
...
#define XVID_ME_ADVANCEDDIAMOND16                                                                  \
    (1 << 0) /* use advdiamonds instead of diamonds as search pattern */
...
#define XVID_ME_EXTSEARCH_RD                                                                       \
    (1                                                                                             \
     << 18) /* perform RD-based search using square pattern enable XVID_ME_EXTSEARCH8 to do this in 8x8 search as well */
