...
#define SCALED( x )  ( (x) * ( 1L << shift ) - delta )
  ...
  TCell s_convert_glyph_buffer[FT_MAX_GRAY_POOL];
  ...
  gray_convert_glyph( RAS_ARG )
  {
    ...
    memset(s_convert_glyph_buffer, 0, sizeof(s_convert_glyph_buffer));
    /* Initialize the null cell at the end of the poll. */
    ras.cell_null        = s_convert_glyph_buffer + FT_MAX_GRAY_POOL - 1;
    ...
    ras.ycells     = (PCell*)s_convert_glyph_buffer;
    ...
    for ( y = yMin; y < yMax; )
    {
      ...
      do
      {
        ...
        ras.cell_free = s_convert_glyph_buffer + n;
        ras.cell      = ras.cell_null;
        ras.min_ey    = band[1];
        ras.max_ey    = band[0];
        ras.count_ey  = width;
