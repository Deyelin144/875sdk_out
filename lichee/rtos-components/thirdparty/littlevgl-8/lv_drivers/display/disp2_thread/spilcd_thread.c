...
static int lv_spilcd_init(void)
{
    ...
    fbsmem_start = sunxifb_mem_alloc(smem_len, "spilcd_buff");
    ...
    spilcd_info.screen_base = fbsmem_start;
    ...
    // make ococci happy
    extern char* s_rgb565_sec_buffer_ptr;
    s_rgb565_sec_buffer_ptr = fbsmem_start;
...
static int lv_spilcd_flush(void)
{
    sunxifb_mem_flush_cache(fbsmem_start, smem_len);
