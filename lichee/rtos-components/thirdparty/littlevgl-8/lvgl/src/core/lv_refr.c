...
static void draw_buf_rotate(lv_area_t * area, lv_color_t * color_p)
{
    ...
    if(drv->rotated == LV_DISP_ROT_180) {
        ...
        while(row < area_h) {
            ...
            if((row == 0) && (area_h >= area_w)) {
                ...
                if(rot_buf == NULL) {
                    ...
                    unsigned rotate_buff_size = area_w * area_h * sizeof(lv_color_t);
                    rotate_buff_size = UINT32_UP_ALIGN64(rotate_buff_size);
                    rot_buf = sunxifb_mem_alloc(rotate_buff_size , "sw_rotate");
                }
        ...
        if(rot_buf != NULL) sunxifb_mem_free((void *)&rot_buf, "sw_rotate free");
    }
}
...
static void draw_buf_flush(lv_disp_t * disp)
{
    ...
    if(disp->driver->flush_cb) {
        ...
        if (disp->driver->rotated != LV_DISP_ROT_NONE) {
            if ((disp->driver->sw_rotate == 1)
#ifdef LV_USE_SUNXIFB_G2D_ROTATE
                || (disp->driver->rotated == LV_DISP_ROT_180) ||
                (lv_area_get_size(draw_ctx->buf_area) < sunxifb_g2d_get_limit(SUNXI_G2D_LIMIT_ROTATE))
#endif /* LV_USE_SUNXIFB_G2D_ROTATE */
                ) {
            draw_buf_rotate(draw_ctx->buf_area, draw_ctx->buf);
            } else {
#ifdef LV_USE_SUNXIFB_G2D_ROTATE
                unsigned area_size = lv_area_get_size(draw_ctx->buf_area) * LV_COLOR_DEPTH / 8;
                area_size = UINT32_UP_ALIGN64(area_size);
                lv_area_t new_area;
                lv_color_t *g2d_rotate_p = sunxifb_mem_alloc(area_size, "g2d_rotate");
                if (g2d_rotate_p == NULL) {
                    draw_buf_rotate(draw_ctx->buf_area, draw_ctx->buf);
        }
                memset(g2d_rotate_p, 0, area_size);
                hal_dcache_clean((unsigned long)draw_ctx->buf, area_size);
                hal_dcache_clean((unsigned long)g2d_rotate_p, area_size);
                if (sunxifb_g2d_rotate(draw_ctx->buf, draw_ctx->buf_area,
                                g2d_rotate_p, &new_area, disp->driver) < 0) {
                    sunxifb_mem_free((void *)&g2d_rotate_p, "g2d_free");
                    draw_buf_rotate(draw_ctx->buf_area, draw_ctx->buf);
                } else {
                    hal_dcache_invalidate((unsigned long)g2d_rotate_p, area_size);
                    call_flush_cb(disp->driver, &new_area, g2d_rotate_p);
                    sunxifb_mem_free((void *)&g2d_rotate_p, "g2d_free");
                }
#endif /* LV_USE_SUNXIFB_G2D_ROTATE */
            }
