                                                      ...
                                                      const lv_area_t * coords, const void * src)
{
    ...
    /*The whole uncompressed image is not available. Try to read it line-by-line*/
...
#ifdef BIN_DEC_WHOLE_ONCE
        int32_t width = lv_area_get_width(&mask_com);
        int32_t height = lv_area_get_height(&mask_com);
        int32_t line_size = 0;
        if (cf == LV_IMG_CF_TRUE_COLOR_ALPHA) {
            line_size = (width * LV_IMG_PX_SIZE_ALPHA_BYTE);
        } else if (cf == LV_IMG_CF_TRUE_COLOR) {
            line_size = (width * (LV_IMG_PX_SIZE_ALPHA_BYTE - 1));
        }
        uint8_t * buf = sunxifb_mem_alloc(line_size * height, "bin_dec_alloc");
        if (buf == NULL) {
            return LV_RES_INV;
        }
        ...
        for(int32_t row = mask_com.y1; row <= mask_com.y2; row++) {
            read_res = lv_img_decoder_read_line(&cdsc->dec_dsc, x, y, width, buf_offset);
            if(read_res != LV_RES_OK) {
                lv_img_decoder_close(&cdsc->dec_dsc);
                LV_LOG_WARN("Image draw can't read the line");
                sunxifb_mem_free((void**) &buf, "bin_dec_free");
                draw_cleanup(cdsc);
                return LV_RES_INV;
            }
        ...
        sunxifb_mem_flush_cache(buf, line_size * height);
        lv_draw_img_decoded(draw_ctx, draw_dsc, &mask_com, buf, cf);
        sunxifb_mem_free((void**) &buf, "bin_dec_free");
#else
        // int32_t width = lv_area_get_width(&mask_com);
        // uint8_t hm = PER_DEC_LINE;
        // int32_t line_size = (width * (LV_IMG_PX_SIZE_ALPHA_BYTE - 1));
        // // uint8_t  * buf = lv_mem_buf_get(lv_area_get_width(&mask_com) *
        // //                                 LV_IMG_PX_SIZE_ALPHA_BYTE);  /*+1 because of the possible alpha byte*/
        // uint8_t * buf = sunxifb_mem_alloc(width * LV_IMG_PX_SIZE_ALPHA_BYTE * hm, "bin_dec_alloc");
        // if (buf == NULL) {
        //     return LV_RES_INV;
        // }
        ...
        // lv_area_set_height(&line, hm);
        // int32_t x = mask_com.x1 - coords->x1;
        // int32_t y = mask_com.y1 - coords->y1;
        // int32_t row;
        // lv_res_t read_res;
        // for(row = mask_com.y1; row <= mask_com.y2; row+=hm) {
        //     lv_area_t mask_line;
        //     union_ok = _lv_area_intersect(&mask_line, clip_area_ori, &line);
        //     if(union_ok == false) continue;
        //     for (int i = 0; i < hm; i++) {
        //         read_res = lv_img_decoder_read_line(&cdsc->dec_dsc, x, y, width, (buf + (line_size * i)));
        //         y++;
        //     }
        //     if(read_res != LV_RES_OK) {
        //         lv_img_decoder_close(&cdsc->dec_dsc);
