...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    m = 5;
#elif LV_COLOR_DEPTH == 16
#ifdef GIF_DEC_WITHOUT_ALPHA
    m = 3;
#else
    m = 4;
#endif /* GIF_DEC_WITHOUT_ALPHA */
#elif LV_COLOR_DEPTH == 8 || LV_COLOR_DEPTH == 1
    m = 3;
#endif
    gif->canvas = (uint8_t *) sunxifb_mem_alloc(m * width * height, "gif_open");
    if (!gif->canvas) {
        lv_mem_free(gif);
        goto fail;
    }
...
#ifdef GIF_DEC_WITHOUT_ALPHA
                if (opa < LV_OPA_MAX) {
                    printf("rgb565 not support alpha, or close defined GIF_DEC_WITHOUT_ALPHA!!!\n");
                } else {
