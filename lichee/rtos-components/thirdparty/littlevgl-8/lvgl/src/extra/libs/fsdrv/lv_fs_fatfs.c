...
#include "../../../../../rtos/projects/xr875s1h10/solution_m33/src/recovery/src/fs_adapter/fs_adapter.h"
#endif
#if LV_USE_FS_FATFS
#include <dirent.h>
#include "ff.h"
...
static void * fs_open(lv_fs_drv_t * drv, const char * path, lv_fs_mode_t mode)
{
    LV_UNUSED(drv);
#ifdef CONFIG_PROJECT_BUILD_RECOVERY
    fs_open_flags_t flags = 0;

    if(mode == LV_FS_MODE_WR) flags = UNIT_FS_WRONLY;
    else if(mode == LV_FS_MODE_RD) flags = UNIT_FS_RDONLY;
    else if(mode == (LV_FS_MODE_WR | LV_FS_MODE_RD)) flags = UNIT_FS_RDWR | UNIT_FS_CREAT;
    void *fp = fs_adapter()->fs_open(path, flags);
    if (!fp) {
...
#ifdef CONFIG_PROJECT_BUILD_RECOVERY
    int res = fs_adapter()->fs_read(file_p, buf, (unsigned int)btr, (unsigned int*)br);
    if(res == 0) return LV_FS_RES_OK;
...
static lv_fs_res_t fs_write(lv_fs_drv_t * drv, void * file_p, const void * buf, uint32_t btw, uint32_t * bw)
{
    LV_UNUSED(drv);
#ifdef CONFIG_PROJECT_BUILD_RECOVERY
    int res = fs_adapter()->fs_write(file_p, (void *)buf, (unsigned int)btw, (unsigned int*)bw);
    if(res == 0) return LV_FS_RES_OK;
    else return LV_FS_RES_UNKNOWN;
#else
...
static lv_fs_res_t fs_seek(lv_fs_drv_t * drv, void * file_p, uint32_t pos, lv_fs_whence_t whence)
{
    LV_UNUSED(drv);
#ifdef CONFIG_PROJECT_BUILD_RECOVERY
    int ret = fs_adapter()->fs_seek(file_p, (unsigned int)pos, (fs_whence_flags_t)whence);
    if(ret == 0) return LV_FS_RES_OK;
    else return LV_FS_RES_UNKNOWN;
#else
    switch(whence) {
        case LV_FS_SEEK_SET:
...
static lv_fs_res_t fs_tell(lv_fs_drv_t * drv, void * file_p, uint32_t * pos_p)
{
    LV_UNUSED(drv);
#ifdef CONFIG_PROJECT_BUILD_RECOVERY
    int ret = fs_adapter()->fs_tell(file_p, (unsigned int*)pos_p);
    if(ret == 0) return LV_FS_RES_OK;
    else return LV_FS_RES_UNKNOWN;
#else
...
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path)
{
    LV_UNUSED(drv);
#ifdef CONFIG_PROJECT_BUILD_RECOVERY
    void *dir = fs_adapter()->opendir(path);
    if (!dir) {
...
static lv_fs_res_t fs_dir_read(lv_fs_drv_t * drv, void * dir_p, char * fn)
{
    ...
    do {
        res = fs_adapter()->readdir(dir_p, &info);
        if(res != 0) return LV_FS_RES_UNKNOWN;
        info_name = info.name_ptr == NULL ? info.name : info.name_ptr;
        if(info.type & UNIT_FS_DIR) {
            fn[0] = '/';
            strcpy(&fn[1], info_name);
        }
        else strcpy(fn, info_name);
