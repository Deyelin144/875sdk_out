...
#ifdef __CONFIG_SONIC_LITE
#include "sonic/sonic_lite.h"
#include <string.h>
#include <stdio.h>
...
static unsigned int g_sonic_max_period = 0;
static unsigned int g_sonic_min_period = 0;
static unsigned int g_sonic_skip = 0;
static unsigned int g_sonic_input_buffer_size = 0;
static unsigned int g_sonic_enable = 0;
...
// void *sonicMalloc(size_t size)
// {
//     return psram_malloc(size);
// }

// void sonicFree(void *ptr)
// {
//     psram_free(ptr);
// }
// #else
...
// }
// #endif
extern void *sonicMalloc(int size);
...
/* Create a sonic stream.  Return NULL only if we are out of memory and cannot
   allocate the stream. */
int sonicInit(int freq, int sample)
{
    if (g_sonic_enable == 1) {
        printf("sonic is working!\n");
        return -1;
    }
    g_sonic_max_period = (freq / SONIC_MIN_PITCH);
    g_sonic_min_period = freq / SONIC_MAX_PITCH;
    g_sonic_skip = freq / SONIC_AMDF_FREQ;
    g_sonic_input_buffer_size = (2 * g_sonic_max_period + sample);
    sonicStream.inputBuffer = sonicMalloc(2 * g_sonic_input_buffer_size);
    sonicStream.outputBuffer = sonicMalloc(2 * g_sonic_input_buffer_size);
    sonicStream.downSampleBuffer = sonicMalloc(2 * (2 * g_sonic_max_period) / g_sonic_skip);
...
void sonicDeInit(void)
{
    sonicFree(sonicStream.inputBuffer);
    sonicFree(sonicStream.outputBuffer);
    sonicFree(sonicStream.downSampleBuffer);
    g_sonic_enable = 0;
}
...
static int copyInputToOutput(int position)
{
    ...
    if (numSamples > 2 * g_sonic_max_period) {
        numSamples = 2 * g_sonic_max_period;
    }
...
{
    int maxRequired = 2 * g_sonic_max_period;
    ...
    memset(sonicStream.inputBuffer + remainingSamples, 0,
           sizeof(short) * (g_sonic_input_buffer_size - remainingSamples));
...
{
    int numSamples = 2 * g_sonic_max_period / g_sonic_skip;
    int i, j;
    int value;
...
{
    int minPeriod = g_sonic_min_period;
    int maxPeriod = g_sonic_max_period;
    ...
    if (g_sonic_skip == 1) {
        ...
        period = findPitchPeriodInRange(sonicStream.downSampleBuffer, minPeriod / g_sonic_skip,
                                        maxPeriod / g_sonic_skip, &minDiff, &maxDiff);
        period *= g_sonic_skip;
        minPeriod = period - (g_sonic_skip << 2);
        maxPeriod = period + (g_sonic_skip << 2);
        if (minPeriod < g_sonic_min_period) {
            minPeriod = g_sonic_min_period;
        }
        if (maxPeriod > g_sonic_max_period) {
            maxPeriod = g_sonic_max_period;
        }
        period = findPitchPeriodInRange(samples, minPeriod, maxPeriod, &minDiff, &maxDiff);
    }
...
static int skipPitchPeriod(short *samples, float speed, int period)
{
    ...
    overlapAdd(newSamples, sonicStream.outputBuffer + sonicStream.numOutputSamples, samples,
               samples + period);
