...
#define NOPOLL_DELIVER_PONG_FRAME  1
...
{
#if defined(NOPOLL_MBEDTLS)
	// printf("====>nopoll_conn_tls_send, buffer_size=%d\n", buffer_size);
	int ret = mbedtls_ssl_write(conn->ssl, (unsigned char *)buffer, buffer_size);
	if(ret < 0) {
		printf("####nopoll mbedtls_ssl_write failed, ret=%d, errno=%s\n", ret, strerror(errno));
	}
	return ret;
#else
	int         res;
...
{
#if defined(NOPOLL_LWIP)
	int ret = lwip_send(conn->session, buffer, buffer_size, 0);
	if (ret < 0) {
		printf ("lwip_send error: %s!!!!!!\n", strerror(errno));	
	}
	return ret;	
#else
...

{
	...
	while (nopoll_true) {
		...
		if ((errno != 0) || tries > 50) {
			...
			printf ("Found errno=%d (%s) value while trying to bytes to the WebSocket conn-id=%d or max tries reached=%d\n", errno, strerror (errno), conn->id, tries);
			break;
		} /* end if */
		...
		if ((tries > 10 && errno == NOPOLL_EWOULDBLOCK) || (tries > 50)) {
			/* Max retry attempts reached */
			nopoll_log (conn->ctx, NOPOLL_LEVEL_WARNING, "Reached max retry attempts (%d) while trying to send bytes to WebSocket conn-id=%d", tries, conn->id);
			// printf ("Reached max retry attempts (%d) while trying to send bytes to WebSocket conn-id=%d\n", tries, conn->id);
			break;
		}

		/* For EWOULDBLOCK, continue retrying with exponential backoff */
		if (errno != NOPOLL_EWOULDBLOCK && errno != 0) {
			/* Non-recoverable error */
			nopoll_log (conn->ctx, NOPOLL_LEVEL_WARNING, "Found non-recoverable error errno=%d (%s) while trying to send bytes to WebSocket conn-id=%d",
			    errno, strerror (errno), conn->id);
			// printf ("Found non-recoverable error errno=%d (%s) while trying to send bytes to WebSocket conn-id=%d", errno, strerror (errno), conn->id);
			break;
		}

		/* Exponential backoff: 100ms, 200ms, 400ms, ..., up to 1s */
		// long sleep_time = 100000 * (1 << (tries - 1));
		// if (sleep_time > 1000000) {
		// 	sleep_time = 1000000; /* Cap at 1 second */
		// }
		// nopoll_sleep (sleep_time);
		nopoll_sleep (100000);
#endif
	} /* end while */
	...
	*/
	...
	if (bytes_sent == 0 && errno == NOPOLL_EWOULDBLOCK) {
		printf("NOPOLL_EWOULDBLOCK, bytes_sent=%d, errno=%d\n", bytes_sent, errno);
		return -2;
	}
#else
	/* In LWIP, we might get partial writes even with EWOULDBLOCK
		Always report the actual bytes sent if any were sent */
	if (bytes_sent > 0) {
		// printf("LWIP: Partial send successful, bytes_sent=%d\n", bytes_sent);
		return bytes_sent;
	}
	/* Only return -2 for LWIP when truly nothing was sent and we have pending bytes */
	if (bytes_sent == 0 && errno == NOPOLL_EWOULDBLOCK && conn->pending_write_bytes > 0) {
		printf("LWIP: NOPOLL_EWOULDBLOCK, bytes_sent=%d, errno=%d\n", bytes_sent, errno);
		return -2;
	}

	/* Always return the actual bytes sent, even if there are pending bytes
	   and we encountered EWOULDBLOCK. This allows partial sends to be reported
	   to the caller instead of always returning -2 when there's any blocking. */
	printf("bytes_sent=%d\n", bytes_sent);
#endif
