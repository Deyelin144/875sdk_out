...
list_head_t *LITE_json_keys_of(char *src, char *prefix)
{
    ...
    json_object_for_each_kv(src, pos, key, klen, val, vlen, vtype)
    {
        if (key && klen && val && vlen) {
            ...
            list_add_tail_q(&entry->list, &keylist);
