...
ListNode *qcloud_list_find(List *self, void *val)
{
    ...
    if (NULL == (it = qcloud_list_iterator_new(self, QC_LIST_HEAD_E))) {
        return NULL;
    }
...
{
    ListDirection direction = QC_LIST_HEAD_E;

    if (index < 0) {
        direction = QC_LIST_TAIL_E;
        index     = ~index;
    }
...
{
    ListNode *node = direction == QC_LIST_HEAD_E ? list->head : list->tail;
...
ListNode *qcloud_list_iterator_next(ListIterator *self)
{
    ...
    if (curr) {
        self->next = self->direction == QC_LIST_HEAD_E ? curr->next : curr->prev;
    }
    return curr;
}
 ...
 */
ListNode *qcloud_list_node_new(void *val)
{
    ListNode *self;
