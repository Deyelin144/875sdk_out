...
static unsigned int arc4_stir_pid;
...
{
	unsigned char buf[ADD_ENTROPY];
	// ! 这里尝试使用HAL接口生成随机数 屏蔽平台差异
	for(int i = 0; i < ADD_ENTROPY; i++)
	{
		buf[i] = HAL_Random() % 0xff;
	}
	arc4_addrandom(buf, sizeof(buf));
	evutil_memclear_(buf, sizeof(buf));
	return 0;
...
arc4_stir_if_needed(void)
{
	...
	unsigned int pid = (unsigned int)HAL_GetCurrentThreadHandle();
#else
...
// #if !defined( PLATFORM_XR875_RTOS) && !defined(PLATFORM_ESP_RTOS)
// ARC4RANDOM_EXPORT void
