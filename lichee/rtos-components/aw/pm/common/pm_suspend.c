...
extern void printf_lock(void) __attribute__((weak));
extern void printf_unlock(void) __attribute__((weak));
...
static pm_virt_log_stage_t virt_log_stage = PM_VIRT_LOG_BEFORE_DEVICES;
...
{
	// only for ococci!!!
	writel(0, PM_STANDBY_STAGE_DSP_REC_REG);
	writel(0, PM_STANDBY_STAGE_C906_REC_REG);
}
...
void pm_record_stage(uint32_t val, uint8_t cover)
{
	uint32_t rec_reg;
	uint32_t rec_val;
	pm_debug_watchdog_feed();
 	if (!pm_test_standby_recording())
 		return;
#ifdef CONFIG_COMPONENTS_PM_SUBSYS_TASK_SAFE
	pm_log("\033[31m!!!not support record stage again,\
			please close config CONFIG_COMPONENTS_PM_SUBSYS_TASK_SAFE!!!\033[0m\n");
	while(1);
#endif
...
void pm_record_subsys_task_sync(uint32_t val, uint32_t mask)
{
	uint32_t rec_reg;
	uint32_t rec_val;
	uint32_t timeout = 0;
	...
	while(1) {
		rec_val = readl(rec_reg);
		rec_val &= mask;

		if ((rec_val >= val) || ((++timeout) >= 10000))
			return;
		
#ifdef CONFIG_ARCH_DSP
			// DSP udelay is not effect
...
static void pm_virt_log(int virt, pm_virt_log_stage_t stage)
{
	if (stage != virt_log_stage)
		return;

	if (virt) {
		pm_debug_get_starttime (PM_TIME_VIRT_LOG_SUSPEND, hal_gettime_ns());
		pm_log("switch to virtual log, stage: %d\n", virt_log_stage);
#ifdef CONFIG_COMPONENTS_PM_CORE_DSP
		// dsp not support printf tiny
		virt_log_enable(1);
#else
		if (stage != PM_VIRT_LOG_LOCK) {
			virt_log_enable(1);
		} else {
			printf_lock();
		}
#endif
		pm_debug_record_time (PM_TIME_VIRT_LOG_SUSPEND, hal_gettime_ns());
	} else {
		pm_debug_get_starttime (PM_TIME_VIRT_LOG_RESUME, hal_gettime_ns());
#ifdef CONFIG_COMPONENTS_PM_CORE_DSP
		// dsp not support printf tiny
		virt_log_enable(0);
#else
...
#endif
		pm_log("restore from virtual log, stage: %d\n", virt_log_stage);
		pm_debug_record_time (PM_TIME_VIRT_LOG_RESUME, hal_gettime_ns());
	}
}
...
static void pm_suspend_task(void *nouse)
{
	...
	while (1) {
		...
		case PM_MODE_HIBERNATION:
			...
			pm_debug_get_starttime (PM_TIME_RESUME_END, hal_gettime_ns());
		...
		pm_debug_record_time (PM_TIME_RESUME_END, hal_gettime_ns());
	}
...
static int pm_suspend_devices_and_enter(suspend_mode_t src_mode)
{
	...
	pm_virt_log(1, PM_VIRT_LOG_LOCK);
...
#ifdef CONFIG_COMPONENTS_PM_SUBSYS_TASK_SAFE
	/*at usually, pm_task_freeze does not fail,
	 so ignore fail could result another core block long time.*/
	pm_record_subsys_state_set(0x03, 0);
	pm_record_subsys_task_sync(0x03, 0x0f);
#endif
	/*debug*/
...
lable_suspend_again_late:
	...
	pm_virt_log(1, PM_VIRT_LOG_EARLY_STAGE);
	...
	pm_virt_log(1, PM_VIRT_LOG_BEFORE_DEVICES);
...
static int pm_suspend_enter(suspend_mode_t mode)
{
	...
	pm_virt_log(1, PM_VIRT_LOG_LATE_STAGE);

	pm_record_stage(PM_TEST_RECORDING_ENTER | 0xa, 1);

	pm_debug_get_starttime (PM_TIME_DEV_SYSCORE_SUSPEND, hal_gettime_ns());
	ret = pm_syscore_suspend(mode);
	pm_debug_record_time (PM_TIME_DEV_SYSCORE_SUSPEND, hal_gettime_ns());
	if (ret)
		goto lable_dev_resume_noirq;
...
lable_syscore_resume:
	pm_debug_get_starttime (PM_TIME_DEV_SYSCORE_RESUME, hal_gettime_ns());
	pm_syscore_resume(mode);
	pm_debug_record_time (PM_TIME_DEV_SYSCORE_RESUME, hal_gettime_ns());
	pm_record_stage(PM_TEST_RECORDING_ENTER | 0x300, 1);

lable_dev_resume_noirq:
	pm_virt_log(0, PM_VIRT_LOG_LATE_STAGE);
