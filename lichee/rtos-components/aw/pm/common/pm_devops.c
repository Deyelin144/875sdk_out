...
int pm_devops_prepared(suspend_mode_t mode)
{
	...
	pm_debug_get_starttime (PM_TIME_DEV_PREPARED, starttime);
	...
	pm_debug_record_time (PM_TIME_DEV_PREPARED, hal_gettime_ns());
...
int pm_devops_suspend(suspend_mode_t mode)
{
	int ret;
	uint64_t starttime = hal_gettime_ns();
	pm_debug_get_starttime (PM_TIME_DEV_SUSPEND, starttime);
	pm_devops_type_t type = PM_DEVOPS_TYPE_SUSPEND;

	ret = pm_devops_call(type, mode);
	pm_debug_record_time (PM_TIME_DEV_SUSPEND, hal_gettime_ns());
	pm_devops_type_showtime(starttime, type, ret, NULL);
...
int pm_devops_suspend_late(suspend_mode_t mode)
{
	int ret;
	uint64_t starttime = hal_gettime_ns();
	pm_debug_get_starttime (PM_TIME_DEV_SUSPEND_LATE, starttime);
	pm_devops_type_t type = PM_DEVOPS_TYPE_SUSPEND_LATE;

	ret = pm_devops_call(type, mode);
	pm_debug_record_time (PM_TIME_DEV_SUSPEND_LATE, hal_gettime_ns());
	pm_devops_type_showtime(starttime, type, ret, NULL);
...
int pm_devops_suspend_noirq(suspend_mode_t mode)
{
	int ret;
	uint64_t starttime = hal_gettime_ns();
	pm_debug_get_starttime (PM_TIME_DEV_SUSPEND_NOIRQ, starttime);
	pm_devops_type_t type = PM_DEVOPS_TYPE_SUSPEND_NOIRQ;

	ret = pm_devops_call(type, mode);
	pm_debug_record_time (PM_TIME_DEV_SUSPEND_NOIRQ, hal_gettime_ns());
	pm_devops_type_showtime(starttime, type, ret, NULL);
...
{
	uint64_t starttime = hal_gettime_ns();
	pm_debug_get_starttime (PM_TIME_DEV_RESUME_NOIRQ, starttime);
	pm_devops_type_t type = PM_DEVOPS_TYPE_RESUME_NOIRQ;

	pm_devops_call(type, mode);
	pm_debug_record_time (PM_TIME_DEV_RESUME_NOIRQ, hal_gettime_ns());
...
{
	uint64_t starttime = hal_gettime_ns();
	pm_debug_get_starttime (PM_TIME_DEV_RESUME_EARLY, starttime);
	pm_devops_type_t type = PM_DEVOPS_TYPE_RESUME_EARLY;

	pm_devops_call(type, mode);
	pm_debug_record_time (PM_TIME_DEV_RESUME_EARLY, hal_gettime_ns());
	pm_devops_type_showtime(starttime, type, 0, NULL);
}
...
{
	uint64_t starttime = hal_gettime_ns();
	pm_debug_get_starttime (PM_TIME_DEV_RESUME, starttime);
	pm_devops_type_t type = PM_DEVOPS_TYPE_RESUME;

	pm_devops_call(type, mode);
	pm_debug_record_time (PM_TIME_DEV_RESUME, hal_gettime_ns());
	pm_devops_type_showtime(starttime, type, 0, NULL);
}
...
{
	uint64_t starttime = hal_gettime_ns();
	pm_debug_get_starttime (PM_TIME_DEV_COMPLETE, starttime);
	...
	pm_devops_call(type, mode);
	pm_debug_record_time (PM_TIME_DEV_COMPLETE, hal_gettime_ns());
	pm_devops_type_showtime(starttime, type, 0, NULL);
}
