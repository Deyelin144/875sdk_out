...
static uint32_t		 ignore_wakelock = 0;
...
// fix 注释pm计数定时器的实现, 解决概率出现的线程阻塞和优先级翻转问题, 例如SPI刷屏，刷屏会调用这个MUTEX，并等待发送完成这时候定时器回调来了，但是因为有这个MUTEX，阻塞掉了
...
void pm_wakelocks_showall(void)
{
    ...
    list_for_each(list, head) {
        ...
        pm_log("%d: %s ref:%d exp:%u\n", i++, wl->name, wl->ref, wl->expires);
    }
    printf("\n");
    xSemaphoreGive(wl_mutex);
}

static int pm_wakelock_list_info(int argc, char **argv)
{
	pm_wakelocks_showall();

	return 0;
}
FINSH_FUNCTION_EXPORT_CMD(pm_wakelock_list_info, pm_list_wakelock, pm tools)
