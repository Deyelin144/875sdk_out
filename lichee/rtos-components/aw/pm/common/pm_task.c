...
int pm_task_register(TaskHandle_t xHandle, pm_task_type_t type)
{
	...
	if (!xHandle || !pm_task_type_notapp(type) || (eTaskGetState(xHandle) >= eDeleted)) {
		pm_invalid();
		return -EINVAL;
	}
...
int pm_task_freeze(pm_task_type_t type)
{
	...
	case PM_TASK_TYPE_WLAN:
		pm_debug_get_starttime (PM_TIME_FREEZE_SYS, hal_gettime_ns());
		for (i = 0; i < PM_TASK_MAX; i++) {
			ptr = &pm_task_array[i];
		...
		pm_debug_record_time (PM_TIME_FREEZE_SYS, hal_gettime_ns());
		break;
	case PM_TASK_TYPE_APP:
		pm_debug_get_starttime (PM_TIME_FREEZE_APP, hal_gettime_ns());
...
#endif
		pm_debug_record_time (PM_TIME_FREEZE_APP, hal_gettime_ns());

		break;
	case PM_TASK_TYPE_FREEZE_AT_ONCE:
		pm_debug_get_starttime (PM_TIME_FREEZE_ATONCE, hal_gettime_ns());
		for (i = 0; i < PM_TASK_MAX; i++) {
			ptr = &pm_task_array[i];
			if (ptr->status && (ptr->type == type)) {
		...
		pm_debug_record_time (PM_TIME_FREEZE_ATONCE, hal_gettime_ns());
		break;
	default:
		pm_invalid();
		ret = -EINVAL;
...
int pm_task_restore(pm_task_type_t type)
{
	...
	case PM_TASK_TYPE_BT:
	case PM_TASK_TYPE_WLAN:
		pm_debug_get_starttime (PM_TIME_RESTORE_SYS, hal_gettime_ns());
		for (i = 0; i < PM_TASK_MAX; i++) {
			ptr = &pm_task_array[i];
			if (ptr->status && (ptr->type == type)) {
		...
		pm_debug_record_time (PM_TIME_RESTORE_SYS, hal_gettime_ns());
		break;
	case PM_TASK_TYPE_APP:
		pm_debug_get_starttime (PM_TIME_RESTORE_APP, hal_gettime_ns());
#ifndef CONFIG_ARCH_DSP
		...
		pm_debug_record_time (PM_TIME_RESTORE_APP, hal_gettime_ns());

		break;
	case PM_TASK_TYPE_FREEZE_AT_ONCE:
		pm_debug_get_starttime (PM_TIME_RESTORE_ATONCE, hal_gettime_ns());
		for (i = 0; i < PM_TASK_MAX; i++) {
			ptr = &pm_task_array[i];
			if (ptr->status && (ptr->type == type)) {
				pm_dbg("Task %s(%p) restore.\n",\
		...
		pm_debug_record_time (PM_TIME_RESTORE_ATONCE, hal_gettime_ns());
		break;
	default:
		pm_invalid();
		ret = -EINVAL;
