...
// #define DEBUG_RECORD_ADB
#if defined(DEBUG_RECORD_ADB)
#include "adb_forward.h"
#define DUMP_PORT 23333
#endif
...
struct dsp_controller_t {
	...
	audio_eos_send_t audio_eos_send;
	void *ring;
	...
	int send_eos;
	int reset_vad;
	int force_raw_data_send;
};

#if defined(ENABLE_LICENSE)
static char g_license[64] = {"0992505020020000a4e76bef342d39379eb0c027e853d86d"};
static char g_client_id[64] = {"TestYunXiGuDemo1"};
extern int uvoice_activate(const char *pc_license, const char *pc_client_id);
extern int uvoice_parse_data(char* tmpRbuf, int size);
#endif
...
static int rpd_data_recv_cb(rpdata_t *rpd, void *data, unsigned int data_len)
{
	...
	const int magic = 77777;
//	int bDupFrame = 0;
	char newdata[4 * 3 + 320 * 4] = {0};

    // printf("word_id %d, vad_flag %d, confidence %f\n", *(int *)((char *)data),*(int *)((char *)data + 4), *(float *)((char *)data + 8));
	if (!hdl)
		return 0;

#if defined(ENABLE_LICENSE)
	memcpy(newdata, (char *)data, 4 * 3 + 320 * 4);
	// if (*(int *)data == magic)
	// {
	// 	*(int *)data = 0;
		//if (uvoice_parse_data((char *)data + 4, (int)data_len - 4) < 0)
        // printf("uvoice_parse_data %d\n", 0);
		if (uvoice_parse_data((char *)newdata, 4 * 3 + 320 * 4) < 0)
		{
			return 0;
		}
        // printf("uvoice_parse_data %d\n", 1);
...
//`	if( 0==bDupFrame ){
		vad_flag = *(short *)((char *)newdata + 0);
		word_id = *(short *)((char *)newdata + 4);
//	}
#else
	vad_flag = *(int *)((char *)data + 0);
	word_id = *(int *)((char *)data + 4);
#endif
	dsp_data = (unsigned char *)((char *)newdata + RPD_DATA_RAW_DATA_OFFSET);
	dsp_output_data = (unsigned char *)((char *)newdata + RPD_DATA_OUT_DATA_OFFSET);
#if defined(DEBUG_RECORD_ADB)
	static char raw[320 * 4];
	int size = 320*4;
	{
		int index;
		short* data_s = (short*)dsp_data;
		int short_size = 160;
		short *ch0 = data_s + short_size * 0;
		short *ch1 = data_s + short_size * 1;
		short *ch2 = data_s + short_size * 2;
		short *ch3 = data_s + short_size * 3;
		short *p = (short *)raw;
		...
		//  printf("++++++++++raw++size+++++++++++++++++\n");
		// save_ch3_to_file_dsp("/data/ch4.pcm",raw, size);
		adb_forward_send(DUMP_PORT, raw, size);
	}
#endif

    // printf("word_id %d, vad_flag %d\n", word_id, vad_flag);
    // printf("vad_flag %d, word_id %d, confidence %f\n", *(int *)((char *)data),*(int *)((char *)data + 4), *(float *)((char *)data + 8));
	// printf("data:%p, dsp_output_data:%p, vad_flag:%d, word_id:%d, confidence:%f\n", data, dsp_output_data, *(int *)((char *)data),*(int *)((char *)data + 4), *(float *)((char *)data + 8));

	confidence = *(float *)((char *)newdata + RPD_DATA_CONFIDENCE_OFFSET);
	// if (word_id > 0)
	// {
    //     printf("vad_flag %d, word_id %d, confidence %f\n", *(int *)((char *)data),*(int *)((char *)data + 4), *(float *)((char *)data + 8));
	// }
	...
	if (hdl->last_enable_vad != hdl->enable_vad) {
		...
		{
			hdl->send_eos = 1;
			if (hdl->audio_eos_send)
			{
				hdl->audio_eos_send(hdl->priv);
			}
		}
	}
	...
	if (hdl->audio_raw_data_send && (hdl->force_raw_data_send || hdl->enable_vad))
	...
	} else if (hdl->audio_data_send && !hdl->enable_vad) {
		hdl->audio_data_send(hdl->priv, dsp_output_data, dsp_output_data_len);
	}
    ...
    // diff eos_time
    if (word_id == 1) {
        dsp_controller_set_vad_eos(hdl, 1500);
    }
...
static int rpd_vad_init(struct dsp_controller_t *hdl)
{
    ...
    printf("rpd_vad_init, rpdata: len: %d", RPD_VAD_TOTAL_LEN);
	hdl->rpd_vad = rpdata_create(RPD_VAD_DIR, RPD_VAD_TYPE, RPD_VAD_NAME, RPD_VAD_TOTAL_LEN);
	if (!hdl->rpd_vad) {
		printf("rpdata_connect failed!\n");
		goto err;
	}
...
static int rpd_vad_set_vad_enable(struct dsp_controller_t *hdl, int val)
{
	...
	*vad_enable = val;
	...
	return rpdata_send(hdl->rpd_vad, 0, RPD_WAKE_UP_SEND_SIZE);
}
...
// diff
int dsp_controller_set_vad_eos(void *_hdl, int val){
	...
	*(vad_enable + 2) = 1;
	...
	return rpdata_send(hdl->rpd_vad, 0, RPD_VAD_TOTAL_LEN);
}

// diff
int rpd_vad_set_sdk(void *_hdl, int val)
{
	struct dsp_controller_t *hdl = (struct dsp_controller_t *)_hdl;
	unsigned int* vad_enable = (unsigned int *)((char *)hdl->rpd_vad_buffer + RPD_VAD_ENABLE_OFFSET);
...
struct dsp_controller_t *s_hdl = NULL;
int cmd_rpd_vad_set_sdk(int argc, char ** argv)
{
	if (argc != 2) {
		printf("usage: rpd_vad_set_sdk <val>\n");
		return -1;
	}
	int val = atoi(argv[1]);
	if (!s_hdl) {
		printf("dsp_controller_create first!\n");
		return -1;
	}
	return rpd_vad_set_sdk(s_hdl, val);
}

FINSH_FUNCTION_EXPORT_CMD(cmd_rpd_vad_set_sdk, rpd_vad_set_sdk, asrdemo);

static int cnt_set = 0;
...
static void dsp_controller_thread(void *arg)
{
	...
	while(hdl->run) {
		...
		cnt_set++;
		// if (cnt_set % 100 ==0){
		//     rpd_vad_set_sdk(hdl, (1 + cnt_set / 200) % 2);
        //     printf("STOP or START sdk %d\n", (1 + cnt_set / 200) % 2);
		// }
...
#if defined(DEBUG_RECORD_ADB)
    adb_forward_end(DUMP_PORT);
    adb_forward_destroy(DUMP_PORT);
#endif
}
...
#if defined(ENABLE_LICENSE)
	if (uvoice_activate(g_license, g_client_id) < 0)
	{
        printf("%d\n", 1);
		printf("uvoice_activate error %d\n", 1);
        fflush(stdout);
		return -1;
	}
#endif
...
int dsp_controller_reset_vad(void *_hdl)
{
	struct dsp_controller_t *hdl = (struct dsp_controller_t *)_hdl;
	hdl->reset_vad = 1;
...
#if defined(ENABLE_LICENSE)
void dsp_controller_set_activate_info(const char *license, const char *client_id)
{
	if (license && (strlen(license) < sizeof(g_license) - 1))
	{
		strcpy(g_license, license);
	}
	if (client_id && (strlen(client_id) < sizeof(g_client_id) - 1))
	{
