...
typedef struct _mfbd_group_struct
{
    ...
    void (*btn_value_report)(mfbd_btn_index_t btn_index, mfbd_btn_code_t btn_value);
...
typedef struct _mfbd_normal_btn_struct
{
    ...
    mfbd_nbtn_info_t          *btn_info;        /* a pointer to mfbd_nbtn_info_t. */
    mfbd_btn_count_t                  filter_count;     /* filter time count when button state changed. */
...
static mfbd_nbtn_info_t NAME##_info = { \
        FILTER_TIME, \
        REPEAT_TIME, \
        LONG_TIME, \
        BTN_DOWN_CODE, \
