...
#if DBUG
#define drv_extio_aw9523b_debug(fmt, ...) drv_logd(fmt, ##__VA_ARGS__);
#else
#define drv_extio_aw9523b_debug(fmt, ...)
#endif

#if INFO
#define drv_extio_aw9523b_info(fmt, ...) drv_logi(fmt, ##__VA_ARGS__);
#else
#define drv_extio_aw9523b_info(fmt, ...)
#endif

#if WARN
#define drv_extio_aw9523b_warn(fmt, ...) drv_logw(fmt, ##__VA_ARGS__);
#else
#define drv_extio_aw9523b_warn(fmt, ...)
#endif

#if EROR
#define drv_extio_aw9523b_error(fmt, ...) drv_loge(fmt, ##__VA_ARGS__);
#else
#define drv_extio_aw9523b_error(fmt, ...)
#endif
...
static aw9523b_pin_map_t s_pin_map[DEV_EXTIO_PIN_NUM] = {
    ...
    [DEV_EXTIO_PIN_FULL_DET]        = {"full_det",   5,  DEV_EXTIO_GPIO_DIR_INPUT,  DEV_EXTIO_GPIO_LEVEL_HIGH,  false, 1}, // P0_5
    ...
    [DEV_EXTIO_PIN_MOTOR1]          = {"motor1",     8,  DEV_EXTIO_GPIO_DIR_OUTPUT, DEV_EXTIO_GPIO_LEVEL_LOW, false, 0}, // P1_0
    [DEV_EXTIO_PIN_MOTOR1]          = {"motor2",     9,  DEV_EXTIO_GPIO_DIR_OUTPUT, DEV_EXTIO_GPIO_LEVEL_LOW, false, 0}, // P1_1
    [DEV_EXTIO_PIN_MOTOR1]          = {"motor3",     10, DEV_EXTIO_GPIO_DIR_OUTPUT, DEV_EXTIO_GPIO_LEVEL_LOW, false, 0}, // P1_2
    [DEV_EXTIO_PIN_MOTOR1]          = {"motor4",     11, DEV_EXTIO_GPIO_DIR_OUTPUT, DEV_EXTIO_GPIO_LEVEL_LOW, false, 0}, // P1_3
    [DEV_EXTIO_PIN_EXTVCC_EN]       = {"extvcc_en",  12, DEV_EXTIO_GPIO_DIR_OUTPUT, DEV_EXTIO_GPIO_LEVEL_HIGH, false, 1}, // P1_4
    [DEV_EXTIO_PIN_TP_RESET]        = {"tp_reset",   13, DEV_EXTIO_GPIO_DIR_INPUT, DEV_EXTIO_GPIO_LEVEL_LOW,  false, 0}, // P1_5,Tp的复位和屏幕的复位是同一个引脚，使用屏幕复位引脚的配置就可以，防止干扰设置成输入
    [DEV_EXTIO_PIN_LCD_RESET]       = {"lcd_reset",  14, DEV_EXTIO_GPIO_DIR_OUTPUT, DEV_EXTIO_GPIO_LEVEL_LOW,  false, 1}, // P1_6
    [DEV_EXTIO_PIN_CAM_PWDN]        = {"cam_pwdn",   15, DEV_EXTIO_GPIO_DIR_OUTPUT, DEV_EXTIO_GPIO_LEVEL_HIGH,  false, 1}, // P1_7
};
 ...
 * @return 当前的中断标志位
 */
uint16_t aw9523b_port_interrupt_clear(void)
{
    int ret = -1;
    uint8_t reg_val[2] = {0};
...
static int aw9523b_read_chipid(void)
{
    ...
    if (0 != ret) {
        drv_extio_aw9523b_error("read chipid fail.");
        return -1;
    }
    drv_extio_aw9523b_debug("chip id: 0x%x", reg_val);
...
{
    // TODO
    drv_extio_iface_irq_init(aw9523b_irq_cb);
    return 0;
}
...
static int drv_extio_aw9523b_init(dev_extio_notify_t cb)
{
    ...
    XR_OS_MSleep(50);
    drv_extio_iface_irq_init((drv_extio_iface_irq_cb_t)aw9523b_irq_cb);
    XR_OS_MSleep(50);

    if (0 != aw9523b_read_chipid()) {
        drv_extio_aw9523b_error("aw9523b init fail.");
    ...
    for (int i = 0; i < sizeof(s_pin_map) / sizeof(s_pin_map[0]); i++) {
        if (s_pin_map[i].used) {
            drv_extio_aw9523b_gpio_init(s_pin_map[i].index, s_pin_map[i].direction, s_pin_map[i].interrupt_flag);
    ...
    drv_extio_aw9523b_info("aw9523b init success.");
...
void drv_extio_aw9523b_ops_register(drv_extio_ops_t* ops)
{
    if (NULL != ops) {
        ...
        drv_extio_aw9523b_error("ops is NULL.");
    }
}

#if AW9523B_REG_PRINT_ENABLE
#include <console.h>
static void cmd_extio_aw9523b_print(int argc, char **argv)
{
    int ret = 0;
    uint8_t reg_val[2] = {0};

    ret = drv_extio_iface_i2c_recv_with_addr(&s_extio_aw9523b_param, AW9523B_REG_P0_OUTPUT, reg_val, 2);
    if (0 == ret) {
        printf("Output P0: 0x%.2x, Output P1: 0x%.2x.\n", reg_val[0], reg_val[1]);
    }

    ret = drv_extio_iface_i2c_recv_with_addr(&s_extio_aw9523b_param, AW9523B_REG_P0_CONFIG, reg_val, 2);
    if (0 == ret) {
        printf("Config P0: 0x%.2x, Config P1: 0x%.2x.\n", reg_val[0], reg_val[1]);
    }

    ret = drv_extio_iface_i2c_recv_with_addr(&s_extio_aw9523b_param, AW9523B_REG_GCR_REG, reg_val, 1);
    if (0 == ret) {
        printf("GCR: 0x%.2x.\n", reg_val[0]);
    }

    ret = drv_extio_iface_i2c_recv_with_addr(&s_extio_aw9523b_param, AW9523B_REG_P0INT_MSK, reg_val, 2);
    if (0 == ret) {
        printf("IRQ Enable P0: 0x%.2x, Config P1: 0x%.2x.\n", reg_val[0], reg_val[1]);
    }
    printf("Input state: 0x%.2x.\n", aw9523b_port_interrupt_clear());
}
FINSH_FUNCTION_EXPORT_CMD(cmd_extio_aw9523b_print, extio_aw9523b_print, extio aw9523b reg print);
#endif
