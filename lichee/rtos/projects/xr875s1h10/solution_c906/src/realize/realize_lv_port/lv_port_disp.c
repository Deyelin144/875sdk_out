...
static void lv_monitor_cb(struct _lv_disp_drv_t * disp_drv, uint32_t time, uint32_t px)
{
    if (time >= 100) {
        realize_unit_log_error("UI flush time too long, time = %d, px = %d", time, px);
    }
}
...
void lv_port_disp_init(void)
{
    ...
    disp_drv.monitor_cb     = lv_monitor_cb;
    disp_drv.wait_cb     = lv_wait_cb;
    disp_drv.sw_rotate    = 1;
    disp_drv.antialiasing = 1;
    // disp_drv.rotated= 3;//先通过lvgl将竖屏变为横屏
