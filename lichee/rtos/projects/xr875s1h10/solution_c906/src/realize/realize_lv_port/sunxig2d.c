...
int sunxifb_g2d_convert(uintptr_t src_buf, uint32_t src_w, uint32_t src_h, g2d_data_fmt src_format,
                        uintptr_t dst_buf, uint32_t dst_w, uint32_t dst_h, g2d_data_fmt dst_format)
{
    g2d_blt_h info;
    memset(&info, 0, sizeof(g2d_blt_h));
    ...
    info.src_image_h.width = src_w;
    info.src_image_h.height = src_h;
    info.src_image_h.laddr[0] = (uint32_t)(uintptr_t)src_buf;
    // 如果涉及YUV格式转换，需要注意设置YUV的地址，仅按照当前需求封装,该函数不具备通用性!!!!
    if (src_format == G2D_FORMAT_YUV420_PLANAR) {
        info.src_image_h.laddr[1] = (int)(info.src_image_h.laddr[0] + src_w * src_h);
        info.src_image_h.laddr[2] = (int)(info.src_image_h.laddr[0] + src_w * src_h * 5 / 4);
    } else {
        info.src_image_h.laddr[1] = (uintptr_t) 0;
        info.src_image_h.laddr[2] = (uintptr_t) 0;
    }
    ...
    info.src_image_h.format = src_format;
    ...
    info.dst_image_h.format = dst_format;
    ...
    info.dst_image_h.clip_rect.w = dst_w;
    info.dst_image_h.clip_rect.h = dst_h;
    ...
    info.dst_image_h.align[1] = info.dst_image_h.align[0];
    info.dst_image_h.align[2] = info.dst_image_h.align[0];
    info.dst_image_h.laddr[0] = (uint32_t)(uintptr_t)dst_buf;
    info.dst_image_h.laddr[1] = (uintptr_t) 0;
    info.dst_image_h.laddr[2] = (uintptr_t) 0;
    info.dst_image_h.use_phy_addr = 1;
