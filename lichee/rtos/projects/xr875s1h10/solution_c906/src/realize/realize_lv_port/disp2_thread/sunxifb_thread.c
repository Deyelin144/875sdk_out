 ...
 *  STATIC VARIABLES
 **********************/
static uint32_t disp_w;
static uint32_t disp_h;
...
void sunxifb_init(uint32_t rotated)
{
    ...
    sunxifb_get_sizes(&disp_w, &disp_h);
    buf_len = disp_h * disp_w * LV_COLOR_DEPTH / 8;
...
extern int extra_layer_init(int len, int w, int h);
extern void extra_layer_exit(void);
extern int extra_layer_set_data(void *final_buf, void *data, int width, int height, int is_extra);

int sunxifb_extra_layer_init()
{
    int ret = -1;

    if (buf_len == 0 || disp_w == 0 || disp_h == 0) {
        return -1;
    }

    ret = extra_layer_init(buf_len, disp_w, disp_h);
    if (ret != 0) {
        printf("extra layer init fail\n");
    }
...
void sunxifb_extra_layer_deinit()
{
    extra_layer_exit();
}

// 自定义函数，用于视频播放额外图层设置数据
int sunxifb_extra_layer_set_buf(char *data, int width, int height)
{
    int ret = -1;
    ...
    ret = extra_layer_set_data(refresh_buff + buf_len, data, width, height, 1);
...
void sunxifb_flush(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    printf("lvgl use %dus, disp use %dus\n", (cur_time - old_time)/1000, (disp_time - cur_time)/1000);
    old_time = disp_time;
#endif
...
static int sunxifb_area_update(char *dst_buff, struct lv_queue_context *new_area)
{
    ...
    // if (lv_area_get_size(area) >= sunxifb_g2d_get_limit(SUNXI_G2D_LIMIT_BLIT)) {
        ...
        if (new_area->last_area != 0) {
            ...
            extra_layer_set_data(dst_buff + smem_len, dst_buff + smem_len, fbp_h, fbp_w, 0);
        }
