...
static int pal_suspend_late(struct pm_device *dev, suspend_mode_t mode);
...
extern int32_t palBtPmIsAllowSleep(uint32_t margin_time_us);
#endif
...
static int pal_suspend_late(struct pm_device *dev, suspend_mode_t mode)
{
	int ret = 0;
	uint64_t timeout;
	uint64_t starttime = hal_gettime_ns() / 1000000;

	do {
#if defined CONFIG_ARCH_ARM_CORTEX_M33
		ret = palBtPmIsAllowSleep(80000);
#endif
		timeout = hal_gettime_ns() / 1000000 - starttime;
		hal_msleep(20);
	} while (!ret && (!pm_suspend_assert()) && (timeout <= 20000));

	if (timeout > 20000 || pm_suspend_assert()) {
		return -1;
	}
...
{
#if defined CONFIG_ARCH_ARM_CORTEX_M33
	return btc_suspend_noirq((suspend_mode_t)mode);
#endif
}
...
{
#ifdef CONFIG_BT_HOST_RUN_IN_M33
	return 1;
#endif
