...
{
	sunxi_input_event(dev, INPUT_EVENT_SYN, INPUT_SYNC_MT_REPORT, 0);
}
...
static int tlsc6x_update_data(struct tlsc6x_drv_data *parm)
{
	...
	for (i = 0; i < TS_MAX_FINGER; i++) {
		...
		if ((buf[6 * i + 3] & 0x40) == 0x0) {
			input_report_abs_t(data->input_dev, INPUT_ABS_MT_TRACKING_ID,
					buf[6 * i + 5] >> 4);
			if (y == 1500) {
				if (x == 40) {
				...
				input_report_abs_t(data->input_dev,
						 INPUT_ABS_MT_POSITION_X, x);
				input_report_abs_t(data->input_dev,
						 INPUT_ABS_MT_POSITION_Y, y);

				input_report_abs_t(data->input_dev,
						 INPUT_ABS_MT_PRESSURE, 15);
				input_report_abs_t(data->input_dev,
						 INPUT_ABS_MT_TOUCH_MAJOR, tlsc_size);
				input_report_key_t(data->input_dev, INPUT_BTN_TOUCH, 1);
			}
	...
	if (event->touch_point == 0) {
		if (y == 1500) {
			if (x == 40) {
				input_report_key_t(data->input_dev, INPUT_KEY_MENU, 0);
			}
			if (x == 80) {
				input_report_key_t(data->input_dev, INPUT_KEY_HOMEPAGE,
						 0);
			}
			if (x == 120) {
				input_report_key_t(data->input_dev, INPUT_KEY_BACK, 0);
			}
		}
		input_report_key_t(data->input_dev, INPUT_BTN_TOUCH, 0);
	}
...
int tlsc6x_init(void)
{
	...
	input_set_capability(input_dev, INPUT_EVENT_ABS, INPUT_ABS_MT_TOUCH_MAJOR);
	input_set_capability(input_dev, INPUT_EVENT_ABS, INPUT_ABS_MT_WIDTH_MAJOR);
	input_set_capability(input_dev, INPUT_EVENT_ABS, INPUT_ABS_MT_TRACKING_ID);
	input_set_capability(input_dev, INPUT_EVENT_KEY, INPUT_KEY_MENU);
	input_set_capability(input_dev, INPUT_EVENT_KEY, INPUT_BTN_TOUCH);
	input_set_capability(input_dev, INPUT_EVENT_KEY, INPUT_KEY_BACK);
	input_set_capability(input_dev, INPUT_EVENT_KEY, INPUT_KEY_HOMEPAGE);
