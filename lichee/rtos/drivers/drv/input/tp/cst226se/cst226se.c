...
{
#if 1
    input_report_abs_t(ts->input_dev, INPUT_ABS_MT_TRACKING_ID, id);
    input_report_abs_t(ts->input_dev, INPUT_ABS_MT_POSITION_X, y);
    input_report_abs_t(ts->input_dev, INPUT_ABS_MT_POSITION_Y, x);

    input_report_abs_t(ts->input_dev, INPUT_ABS_MT_PRESSURE, 15);
    input_report_abs_t(ts->input_dev, INPUT_ABS_MT_TOUCH_MAJOR, w);
    input_report_key_t(ts->input_dev, INPUT_BTN_TOUCH, 1);
...
static void cst2xx_touch_up(struct hyn_ts* ts, int id)
{
	...
	return;
#if 1
    input_report_key_t(ts->input_dev, INPUT_BTN_TOUCH, 0);
    input_sync_t(ts->input_dev);
#else
    cst226se_wp = 0;
#endif
