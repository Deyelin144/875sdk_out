...
CONFIG_ARCH_START_ADDRESS_DECIMAL = $(shell printf %d $(CONFIG_ARCH_START_ADDRESS))
CONFIG_ARCH_MEM_LENGTH_DECIMAL = $(shell printf %d $(CONFIG_ARCH_MEM_LENGTH))

KASAN_SHADOW_LENGTH = $(shell expr $(CONFIG_ARCH_MEM_LENGTH_DECIMAL) / 8)
KASAN_SHADOW_START_ADDRESS = $(shell expr $(CONFIG_ARCH_START_ADDRESS_DECIMAL) + $(CONFIG_ARCH_MEM_LENGTH_DECIMAL) - $(KASAN_SHADOW_LENGTH))
KASAN_SHADOW_OFFSET_DECIMAL  = $(shell expr $(KASAN_SHADOW_START_ADDRESS) - $(CONFIG_ARCH_START_ADDRESS_DECIMAL) / 8)
KASAN_SHADOW_OFFSET = $(shell printf 0x%x $(KASAN_SHADOW_OFFSET_DECIMAL))
endif
...
INCLUDES += -I $(BASE)/components/common/aw/trace_event/include
INCLUDES += -I $(BASE)/components/common/aw/ftrace/include
INCLUDES += -I $(BASE)/components/common/aw/xradio/include
...
ifeq ($(CONFIG_ARCH_DSP), y)
   WRAP_FUNCS += sprintf vsprintf
endif
ifeq ($(CONFIG_ARCH_RISCV_C906), y)
  WRAP_FUNCS += sprintf vsprintf snprintf vsnprintf
endif
ifneq ($(strip $(WRAP_FUNCS)),)
  LDFLAGS += $(foreach func, $(WRAP_FUNCS), -Wl,--wrap=$(func))
endif
