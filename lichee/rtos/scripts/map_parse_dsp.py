...
map_parse_version = "1.0.0"
print "================================="
print "Usage: "
print "  map_parse_version: " + map_parse_version
print "  map_parse_dsp.py xxx.map"
print "=================================\n"
...
with open(map_file, 'r') as f:
    ...
    mem_config_text = '\n'
    mem_config_text_res = re.findall('Name             Origin          Length          Used            Unused          Attributes\n([\s\S]+)', s)
    if (len(mem_config_text_res)) == 0:
        mem_config_text += re.findall('Name             Origin          Length          Used            Unused          Attributes\n([\s\S]+)', s)[0]
    else:
        mem_config_text += re.findall('Name             Origin          Length          Used            Unused          Attributes\n([\s\S]+)', s)[0]
    ...
    flash_config_text = re.findall(r"FLASH\s+(0x\w+)\s+(0x\w+)\s+(0x\w+)\s+(0x\w+)",mem_config_text)
    ...
    psram_config_text = re.findall(r"extra_mem_10_seg\s+(0x\w+)\s+(0x\w+)\s+(0x\w+)\s+(0x\w+)",mem_config_text)
    ...
    modules = list(set(item[0] for item in re.findall('0x\w+[\t ]+0x\w+[\t ]+.+?([\w\-\+\/]+\.[ao])(\([^\(\)]+\.o(bj)?\))?', mem_map)))
    ...
    for module in modules:
        ...
        if(module == '*fill*'):
            ...
            sections = map(lambda arg : {'address':int(arg[0], 16), 'size':int(arg[1], 16)}, re.findall('(0x\w+)[ \t]+(0x\w+)[ \t]+.+'+module+'(\(.+\.o\))?', mem_map))
        ...
        map_lines.append('| %-40s |%-8d | %-8d | %s |'%(module.split('/')[-1],flash_size, psram_size, module))

print '\n                            MEMORY MAP                            '
print '|===========================================================================|'
print '| %-40s | %-8s | %-8s |'%('MODULE', 'FLASH', 'HPSRAM')
print '|===========================================================================|'
for line in map_lines:
    print line
print '|===========================================================================|'
print '| %-40s | %-8d | %-8d |'%('TOTAL (bytes)', total_flash, total_psram)
print '| %-40s | %-8d | %-8d |'%('MEM LIMIT', limit_flash, limit_psram)
print '| %-40s | %-8d | %-8d |'%('MEM LEFT', limit_flash-total_flash, limit_psram-total_psram)
print '|===========================================================================|'
