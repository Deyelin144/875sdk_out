...
/* In the XIP scheme, disable the ability to check variables in flash. */
#if CONFIG_XIP

#define KASAN_CHECK_ADDR_FILTER(addr) \
        ...
        unsigned long ret_ip)
{
    ...
    if (addr < CONFIG_ARCH_START_ADDRESS )
    {
        return;
    }
    if (addr >= (CONFIG_ARCH_START_ADDRESS + CONFIG_ARCH_MEM_LENGTH))
    {
        return;
    }
#else
