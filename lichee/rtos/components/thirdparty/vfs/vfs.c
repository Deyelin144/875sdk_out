...
int vfs_open(struct _reent *r, const char  *path, int flags, int mode)
{
    ...
    trace_event(EV_FS, "vfs_open", ARG_PTR(path), ARG_UINT(flags), ARG_UINT(mode));
	fullpath = vfs_workdir_get_path(NULL, path);
    if (fullpath == NULL) {
        trace_event(EV_FS, "vfs_open", RET_ARG(-ENOMEM), ARG_STR(fullpath));
        return -ENOMEM;
    }
    ...
    if (vfs == NULL) {
        ...
        vfs_workdir_put_path(fullpath);
        trace_event(EV_FS, "vfs_open", RET_ARG(-ENOENT), ARG_STR(vfs));
        return -1;
    }
    ...
    if (!strlen(path_within_vfs)) {
        _errno_r(((struct _reent *)r)) = ENOENT;
        vfs_workdir_put_path(fullpath);
        trace_event(EV_FS, "vfs_open", RET_ARG(-ENOENT), ARG_STR(path_within_vfs));
        return -1;
    }
    ...
    if (fd_within_vfs >= 0) {
        ...
        for (i = 0; i < MAX_FDS; ++i) {
            if (s_fd_table[i].vfs_index == -1) {
                ...
                vfs_tb_unlock();
                vfs_workdir_put_path(fullpath);
                trace_event(EV_FS, "vfs_open", RET_ARG(i + STDIO_FD_OFF));
        ...
        vfs_workdir_put_path(fullpath);
        trace_event(EV_FS, "vfs_open", RET_ARG(-ENOENT));
        return -1;
    }

    _errno_r(((struct _reent *)r)) = ENOENT;
    vfs_workdir_put_path(fullpath);
    trace_event(EV_FS, "vfs_open", RET_ARG(-ENOENT), ARG_STR(fullpath));
    return -1;
}
...
ssize_t vfs_write(struct _reent *r, int fd, const void  *data, size_t size)
{
    ...
    trace_event(EV_FS, "vfs_write", ARG_PTR(vfs), ARG_INT(fd), ARG_PTR(data), ARG_ULONG(size));
    if (vfs == NULL || local_fd < 0) {
        _errno_r(((struct _reent *)r)) = EBADF;
        return -1;
    }
    CHECK_AND_CALL(ret, r, vfs, write, local_fd, data, size);
    trace_event(EV_FS, "vfs_write", RET_ARG(ret));
    if (ret == -ECANCELED) {
...
off_t vfs_lseek(struct _reent *r, int fd, off_t size, int mode)
{
    ...
    trace_event(EV_FS, "vfs_lseek", ARG_PTR(vfs), ARG_INT(fd), ARG_PTR(size), ARG_ULONG(mode));
    if (vfs == NULL || local_fd < 0) {
        _errno_r(((struct _reent *)r)) = EBADF;
        return -1;
    }
    ...
    trace_event(EV_FS, "vfs_lseek", RET_ARG(ret));
    return ret;
}
...
ssize_t vfs_read(struct _reent *r, int fd, void  *dst, size_t size)
{
    ...
    trace_event(EV_FS, "vfs_read", ARG_PTR(vfs), ARG_INT(fd), ARG_PTR(dst), ARG_ULONG(size));
    if (vfs == NULL || local_fd < 0) {
        _errno_r(((struct _reent *)r)) = EBADF;
        return -1;
    }
    ...
    trace_event(EV_FS, "vfs_read", RET_ARG(ret));
    if (ret == -ECANCELED) {
	_errno_r(((struct _reent *)r)) = ECANCELED;
	return -1;
    }
...
int vfs_close(struct _reent *r, int fd)
{
    ...
    trace_event(EV_FS, "vfs_close", ARG_PTR(vfs), ARG_INT(fd));
    if (vfs == NULL || local_fd < 0) {
        _errno_r(((struct _reent *)r)) = EBADF;
        return -1;
    }
    ...
    vfs_tb_unlock();
    trace_event(EV_FS, "vfs_close", RET_ARG(ret));
    return ret;
}
...
int vfs_fstat(struct _reent *r, int fd, struct stat  *st)
{
    ...
    trace_event(EV_FS, "vfs_fstat", ARG_PTR(vfs), ARG_INT(fd));
    if (vfs == NULL || local_fd < 0) {
        _errno_r(((struct _reent *)r)) = EBADF;
        return -1;
    }
    ...
    trace_event(EV_FS, "vfs_fstat", RET_ARG(ret));
    return ret;
}
...
int vfs_stat(struct _reent *r, const char  *path, struct stat  *st)
{
    ...
    trace_event(EV_FS, "vfs_fstat", ARG_PTR(path));
    fullpath = vfs_workdir_get_path(NULL, path);
    if (fullpath == NULL) {
        return -ENOMEM;
    }
...
int vfs_link(struct _reent *r, const char *n1, const char *n2)
{
    ...
    trace_event(EV_FS, "vfs_link", ARG_PTR(n1), ARG_PTR(n2));
	...
	vfs_workdir_put_path(fulln1);
    vfs_workdir_put_path(fulln2);
    trace_event(EV_FS, "vfs_link", RET_ARG(ret));
...
int vfs_unlink(struct _reent *r, const char *path)
{
    ...
    trace_event(EV_FS, "vfs_unlink", ARG_PTR(path));
    fullpath = vfs_workdir_get_path(NULL, path);
    if (fullpath == NULL) {
        return -ENOMEM;
    }
    ...
    vfs_workdir_put_path(fullpath);
    trace_event(EV_FS, "vfs_unlink", RET_ARG(ret));
    return ret;
}
...
int vfs_rename(struct _reent *r, const char *src, const char *dst)
{
    ...
    trace_event(EV_FS, "vfs_rename", ARG_PTR(src), ARG_PTR(dst));
    ...
    vfs_workdir_put_path(fullsrc);
    vfs_workdir_put_path(fulldst);
    trace_event(EV_FS, "vfs_rename", RET_ARG(ret));
...
struct dirent *readdir(DIR *pdir)
{
    ...
    trace_event(EV_FS, "readdir", ARG_PTR(pdir));
    if (pdir == (DIR *)&g_rdir)
    ...
    trace_event(EV_FS, "readdir", RET_ARG(ret));
    return ret;
}
...
int readdir_r(DIR *pdir, struct dirent *entry, struct dirent **out_dirent)
{
    ...
    trace_event(EV_FS, "readdir_r", ARG_PTR(pdir), ARG_PTR(entry));
    vfs = get_vfs_for_index(pdir->dd_vfs_idx);
    if (vfs == NULL) {
        errno = EBADF;
        return -1;
    ...
    trace_event(EV_FS, "readdir_r", RET_ARG(ret));
    return ret;
}
...
long telldir(DIR *pdir)
{
    ...
    trace_event(EV_FS, "telldir", ARG_PTR(pdir));
    vfs = get_vfs_for_index(pdir->dd_vfs_idx);
    if (vfs == NULL) {
        errno = EBADF;
        return -1;
    ...
    trace_event(EV_FS, "telldir", RET_ARG(ret));
    return ret;
}
...
void seekdir(DIR *pdir, long loc)
{
    ...
    trace_event(EV_FS, "seekdir", ARG_PTR(vfs), ARG_PTR(pdir));
    if (vfs == NULL) {
        errno = EBADF;
        return;
    }
...
int closedir(DIR *pdir)
{
    ...
    trace_event(EV_FS, "closedir", ARG_PTR(pdir));
    if (pdir == (DIR *)&g_rdir)
    ...
    trace_event(EV_FS, "closedir", RET_ARG(ret));
    return ret;
}
...
int mkdir(const char *name, mode_t mode)
{
    ...
    trace_event(EV_FS, "mkdir", ARG_PTR(name), ARG_ULONG(mode));
    fullname = vfs_workdir_get_path(NULL, name);
    if (fullname == NULL) {
        return -ENOMEM;
    }
    ...
    vfs_workdir_put_path(fullname);
    trace_event(EV_FS, "mkdir", RET_ARG(ret));
    return ret;
}
...
int rmdir(const char *name)
{
    ...
    trace_event(EV_FS, "rmdir", ARG_PTR(name));
    fullname = vfs_workdir_get_path(NULL, name);
    if (fullname == NULL) {
        return -ENOMEM;
    }
    ...
    vfs_workdir_put_path(fullname);
    trace_event(EV_FS, "rmdir", RET_ARG(ret));
    return ret;
}
...
int fcntl(int fd, int cmd, ...)
{
    ...
    trace_event(EV_FS, "fcntl", ARG_INT(fd), ARG_UINT(cmd));
    if (vfs == NULL || local_fd < 0) {
        errno = EBADF;
        return -1;
    }
    ...
    va_end(args);
    trace_event(EV_FS, "fcntl", ARG_INT(fd), RET_ARG(ret));
    return ret;
}
...
int ioctl(int fd, int cmd, ...)
{
    ...
    trace_event(EV_FS, "ioctl", ARG_INT(fd), ARG_UINT(cmd));
    if (vfs == NULL || local_fd < 0) {
        errno = EBADF;
        return -1;
    }
    ...
    va_end(args);
    trace_event(EV_FS, "ioctl", RET_ARG(ret));
    if (ret == -ECANCELED) {
        _errno_r(((struct _reent *)r)) = ECANCELED;
	return -1;
    }
...
int fsync(int fd)
{
    ...
    trace_event(EV_FS, "fsync", ARG_INT(fd));
    if (vfs == NULL || local_fd < 0) {
        errno = EBADF;
        return -1;
    }
    ...
    trace_event(EV_FS, "fsync", RET_ARG(ret));
    return ret;
}
...
int access(const char *path, int amode)
{
    ...
    trace_event(EV_FS, "access", ARG_STR(path), ARG_UINT(amode));
    fullpath = vfs_workdir_get_path(NULL, path);
    if (fullpath == NULL) {
        return -ENOMEM;
    }
    ...
    vfs_workdir_put_path(fullpath);
    trace_event(EV_FS, "access", RET_ARG(ret));
    return ret;
}
...
int truncate(const char *path, off_t length)
{
    ...
    trace_event(EV_FS, "truncate", ARG_STR(path), ARG_ULONG(length));
    fullpath = vfs_workdir_get_path(NULL, path);
    if (fullpath == NULL) {
        return -ENOMEM;
    }
    ...
    trace_event(EV_FS, "truncate", RET_ARG(ret));
    return ret;
}
...
int statfs(const char *path, struct statfs *buf)
{
    ...
    trace_event(EV_FS, "statfs", ARG_STR(path));
    fullpath = vfs_workdir_get_path(NULL, path);
    if (fullpath == NULL) {
        return -ENOMEM;
    }
    ...
    vfs_workdir_put_path(fullpath);
    trace_event(EV_FS, "statfs", RET_ARG(ret));
    return ret;
}
...
int fstatfs(int fd, struct statfs *buf)
{
    ...
    trace_event(EV_FS, "fstatfs", ARG_INT(fd));
    if (vfs == NULL || local_fd < 0) {
        _errno_r(((struct _reent *)r)) = EBADF;
        return -1;
    }
    ...
    trace_event(EV_FS, "fstatfs", RET_ARG(ret));
    return ret;
}
