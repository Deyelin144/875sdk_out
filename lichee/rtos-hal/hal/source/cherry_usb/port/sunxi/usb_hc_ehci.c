...
#endif
static void ehci_recheck_qh(struct ehci_qh_hw *qh_check);
void *hal_malloc_coherent(size_t size);
void hal_free_coherent(void *addr);

struct ehci_hcd g_ehci_hcd;

USB_NOCACHE_RAM_SECTION struct ehci_qh_hw ehci_qh_pool[CONFIG_USB_EHCI_QH_NUM] __attribute__((aligned(CACHE_LINE_SIZE)));
USB_NOCACHE_RAM_SECTION struct ehci_qtd_hw ehci_qtd_pool[CONFIG_USB_EHCI_QTD_NUM] __attribute__((aligned(CACHE_LINE_SIZE)));

/* The head of the asynchronous queue */
USB_NOCACHE_RAM_SECTION struct ehci_qh_hw g_async_qh_head __attribute__((aligned(CACHE_LINE_SIZE)));
/* The head of the periodic queue */
USB_NOCACHE_RAM_SECTION struct ehci_qh_hw g_periodic_qh_head[EHCI_PERIOIDIC_QH_NUM] __attribute__((aligned(CACHE_LINE_SIZE)));
...
static inline void ehci_qh_add_head(struct ehci_qh_hw *head, struct ehci_qh_hw *n)
{
    ...
    usb_ehci_dcache_clean((unsigned long)&n->hw, sizeof(struct ehci_qh));

    head->hw.hlp = QH_HLP_QH(n);
    usb_ehci_dcache_clean((unsigned long)&head->hw, sizeof(struct ehci_qh));
}
...
static inline void ehci_qh_remove(struct ehci_qh_hw *head, struct ehci_qh_hw *n)
{
    ...
    if (tmp) {
        tmp->hw.hlp = n->hw.hlp;
        usb_ehci_dcache_clean((unsigned long)&tmp->hw, sizeof(struct ehci_qh));
    }
}
...
static void ehci_qtd_bpl_fill(struct ehci_qtd_hw *qtd, uint32_t bufaddr, size_t buflen)
{
    ...
    usb_ehci_dcache_clean((unsigned long)bufaddr, buflen);
...
static struct ehci_qh_hw *ehci_control_urb_init(struct usbh_urb *urb, struct usb_setup_packet *setup, uint8_t *buffer, uint32_t buflen)
{
    ...
    /* flush dcache */
    usb_ehci_dcache_clean((unsigned long)&qtd_setup->hw, sizeof(struct ehci_qtd));
    usb_ehci_dcache_clean((unsigned long)&qtd_data->hw, sizeof(struct ehci_qtd));
    usb_ehci_dcache_clean((unsigned long)&qtd_status->hw, sizeof(struct ehci_qtd));
...
static struct ehci_qh_hw *ehci_bulk_urb_init(struct usbh_urb *urb, uint8_t *buffer, uint32_t buflen)
{
    ...
    while (buflen >= 0) {
        ...
        if (prev_qtd) {
            ...
            usb_ehci_dcache_clean((unsigned long)&prev_qtd->hw, sizeof(struct ehci_qtd));
        } else {
            first_qtd = qtd;
        }
        usb_ehci_dcache_clean((unsigned long)&qtd->hw, sizeof(struct ehci_qtd));
        prev_qtd = qtd;
...
static void ehci_check_qh(struct ehci_qh_hw *qhead, struct ehci_qh_hw *qh)
{
    ...
    while (qtd) {
        usb_ehci_dcache_invalidate((unsigned long)&qtd->hw, sizeof(struct ehci_qtd));
    ...
    if ((token & QTD_TOKEN_STATUS_ERRORS) == 0) {
        usb_ehci_dcache_invalidate((unsigned long)&qh->hw, sizeof(struct ehci_qh));
...
static void ehci_recheck_qh(struct ehci_qh_hw *qh_check)
{
    ...
    size_t flags;
    flags = usb_osal_enter_critical_section();
    qh = EHCI_ADDR2QH(&g_async_qh_head);
    do {
        if (qh == qh_check) {
            ehci_kill_qh(&g_async_qh_head, qh_check);
        }
...
int usb_hc_init(void)
{
    ...
    usb_ehci_dcache_clean((unsigned long)&g_async_qh_head.hw, sizeof(struct ehci_qh));
    ...
    for (int i = EHCI_PERIOIDIC_QH_NUM - 1; i >= 0; i--) {
        ...
        usb_ehci_dcache_clean((unsigned long)&g_periodic_qh_head[i].hw, sizeof(struct ehci_qh_hw));
        ...
        for (uint32_t j = interval - 1; j < CONFIG_USB_EHCI_FRAME_LIST_SIZE; j += interval) {
            if (g_framelist[j] == 0) {
                ...
                usb_ehci_dcache_clean((unsigned long)&qh->hw, sizeof(struct ehci_qh));
            }
        }
        usb_ehci_dcache_clean((unsigned long)g_framelist,
			CONFIG_USB_EHCI_FRAME_LIST_SIZE * sizeof(uint32_t));
    }
...
int usbh_submit_urb(struct usbh_urb *urb)
{
    ...
    int temp_flag = 0;
    uint8_t *align_buff = NULL;
    struct usb_setup_packet *setup_buff = NULL;
    ...
    /*  An interrupt will be triggered immediately after starting the transmission, 
        which may cause the time to be cleared before judgment.*/
    if (urb->timeout > 0)
        temp_flag = 1;
    else
        temp_flag = 0;

    align_buff = (uint8_t *)hal_malloc_coherent(urb->transfer_buffer_length);
    if (align_buff == NULL) {
        USB_LOG_ERR("align_buff malloc failed\r\n");
        return -USB_ERR_NOMEM;
    }
    memcpy(align_buff, urb->transfer_buffer, urb->transfer_buffer_length);

    switch (USB_GET_ENDPOINT_TYPE(urb->ep->bmAttributes)) {
        case USB_ENDPOINT_TYPE_CONTROL:
            setup_buff = (struct usb_setup_packet *)hal_malloc_coherent(sizeof(struct usb_setup_packet));
            if (setup_buff == NULL) {
                USB_LOG_ERR("align_buff malloc failed\r\n");
                return -USB_ERR_NOMEM;
            }
            memcpy(setup_buff, urb->setup, sizeof(struct usb_setup_packet));
            usb_ehci_dcache_clean((unsigned long)setup_buff, sizeof(struct usb_setup_packet));
            qh = ehci_control_urb_init(urb, setup_buff, align_buff, urb->transfer_buffer_length);
            if (qh == NULL) {
                hal_free_coherent(align_buff);
                hal_free_coherent(setup_buff);
                return -USB_ERR_NOMEM;
            }
            urb->hcpriv = qh;
            break;
        case USB_ENDPOINT_TYPE_BULK:
            qh = ehci_bulk_urb_init(urb, align_buff, urb->transfer_buffer_length);
            if (qh == NULL) {
                hal_free_coherent(align_buff);
                return -USB_ERR_NOMEM;
            }
        ...
        case USB_ENDPOINT_TYPE_INTERRUPT:
            qh = ehci_intr_urb_init(urb, align_buff, urb->transfer_buffer_length);
            if (qh == NULL) {
                hal_free_coherent(align_buff);
                return -USB_ERR_NOMEM;
            }
    ...
    if (temp_flag > 0) {
        ...
        if (ret < 0) {
            printf("wait urb transfer, timeout!\n");
            goto errout_timeout;
        }
        urb->timeout = 0;
        ret = urb->errorcode;
    ...
    usb_ehci_dcache_invalidate((unsigned long)align_buff, urb->transfer_buffer_length);
    memcpy(urb->transfer_buffer, align_buff, urb->transfer_buffer_length);
    hal_free_coherent(align_buff);
    if (setup_buff != NULL) {
        usb_ehci_dcache_invalidate((unsigned long)setup_buff, sizeof(struct usb_setup_packet));
        memcpy(urb->setup, setup_buff, sizeof(struct usb_setup_packet));
        hal_free_coherent(setup_buff);
    }
    if (urb->complete) {
        if (urb->errorcode < 0) {
