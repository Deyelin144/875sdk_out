...
#define EHCI_ADDR2QH(x)  ((struct ehci_qh_hw *)((uintptr_t)(unsigned long)(x) & ~0x1F))
...
struct ehci_qh_hw {
    struct ehci_qh hw __attribute__((aligned(CACHE_LINE_SIZE)));
    uint32_t first_qtd __attribute__((aligned(CACHE_LINE_SIZE)));
    struct usbh_urb *urb;
...
struct ehci_qtd_hw {
    struct ehci_qtd hw __attribute__((aligned(CACHE_LINE_SIZE)));
    struct usbh_urb *urb __attribute__((aligned(CACHE_LINE_SIZE)));
    uint32_t total_len;
};

struct ehci_itd_hw {
    struct ehci_itd hw __attribute__((aligned(CACHE_LINE_SIZE)));
    struct usbh_urb *urb;
