...
bool adb_main_status = 0;
void cherryadb_init(uint32_t reg_base)
{

	if (adb_main_status) {
		USB_LOG_ERR("adbd service allread runing.\n");
		return;
	}
	adb_main_status = 1;
	usbd_desc_register(adb_descriptor);
	usbd_add_interface(usbd_adb_init_intf(&intf0, WINUSB_IN_EP, WINUSB_OUT_EP));
	usbd_initialize();
}
FINSH_FUNCTION_EXPORT_CMD(cherryadb_init, cherryadb_init, cherry usb adb init);

void cherryadb_deinit(uint32_t reg_base)
{

	if (!adb_main_status) {
		USB_LOG_ERR("adbd service allread exit.\n");
		return;
	}
	usbd_deinitialize();
	adb_main_status = 0;
}
FINSH_FUNCTION_EXPORT_CMD(cherryadb_deinit, cherryadb_deinit, cherry usb adb deinit);
