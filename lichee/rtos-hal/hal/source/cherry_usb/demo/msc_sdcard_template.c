...
{
    *block_size = BLOCK_SIZE; // SD卡固定为512
    uint64_t temp_num = 0;
    ff_sdmmc_ioctl(0, 1, &temp_num);
    *block_num = (uint32_t)temp_num;
}
int usbd_msc_sector_read(uint32_t sector, uint8_t *buffer, uint32_t length)
{
    ff_sdmmc_read(0, buffer, sector, (length / 512));
    return 0;
}

int usbd_msc_sector_write(uint32_t sector, uint8_t *buffer, uint32_t length)
{
    ff_sdmmc_write(0, buffer, sector, (length / 512));
    return 0;
}
...
FINSH_FUNCTION_EXPORT_CMD(msc_ram_init, usb_msc_init, usb devices msc init);
