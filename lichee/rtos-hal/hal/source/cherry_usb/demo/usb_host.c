...
#define OPERATE_INTERFACE_NAME "/dev/ttyACM2"
...
static void usbh_cdc_acm_thread(void *argument)
{
    ...
    uint8_t *acm_tx_buffer = NULL;

    // clang-format off
find_class:
    // clang-format on
    while ((cdc_acm_class = (struct usbh_cdc_acm *)usbh_find_class_instance(OPERATE_INTERFACE_NAME)) == NULL) {
        goto delete;
    }

    USB_LOG_DBG("argument:%s\r\n", argument);
    acm_tx_buffer = usb_malloc(strlen(argument));
    if (acm_tx_buffer == NULL)
        goto delete;
    memset(acm_tx_buffer, 0, strlen(argument));
    memcpy(acm_tx_buffer, argument, strlen(argument));
    usbh_bulk_urb_fill(&cdc_acm_class->bulkout_urb, cdc_acm_class->hport, cdc_acm_class->bulkout, acm_tx_buffer, strlen(argument), 3000, NULL, NULL);
    ret = usbh_submit_urb(&cdc_acm_class->bulkout_urb);
    usb_free(acm_tx_buffer);
    if (ret < 0) {
        USB_LOG_RAW("bulk out error,ret:%d\r\n", ret);
        goto find_class;
    } else {
        USB_LOG_DBG("send over:%d\n", cdc_acm_class->bulkout_urb.actual_length);
    }
    // clang-format off
delete:
...
void usbh_cdc_acm_rx_thread(void)
{
    int ret;
    struct usbh_cdc_acm *cdc_acm_class;

    USB_LOG_INFO("Create cdc acm rx thread %s\r\n", OPERATE_INTERFACE_NAME);
    // clang-format off
    // clang-format off
    find_class:
    // clang-format on
    ...
    while (1) {
        usbh_bulk_urb_fill(&cdc_acm_class->bulkin_urb, cdc_acm_class->hport, cdc_acm_class->bulkin, cdc_buffer, cdc_acm_class->bulkin->wMaxPacketSize, USB_OSAL_WAITING_FOREVER, usbh_cdc_acm_callback, cdc_acm_class);
...
#endif //__RTTHREAD__
#endif //TEST_USBH_CDC_ECM || TEST_USBH_RNDIS

#if TEST_USBH_CDC_ECM
#include "usbh_cdc_ecm.h"

struct netif g_cdc_ecm_netif;
static usb_osal_thread_t cherry_ecm_thread;
...
void usbh_cdc_ecm_run(struct usbh_cdc_ecm *cdc_ecm_class)
{
    ...
    cherry_ecm_thread = usb_osal_thread_create("usbh_cdc_ecm_rx", 6144, CONFIG_USBHOST_PSC_PRIO + 1, usbh_cdc_ecm_rx_thread, netif);
#if LWIP_DHCP
    dhcp_start(netif);
...
void usbh_cdc_ecm_stop(struct usbh_cdc_ecm *cdc_ecm_class)
{
    ...
    netif_remove(netif);
#endif
    usb_osal_thread_delete(cherry_ecm_thread);
}
#endif //TEST_USBH_CDC_ECM
...
#endif
#if TEST_USBH_CDC_ACM
static usb_osal_thread_t usbh_acm_rx_thread;
#endif
void usbh_cdc_acm_run(struct usbh_cdc_acm *cdc_acm_class)
{
#if TEST_USBH_CDC_ACM
    usb_osal_thread_create("usbh_cdc", 4096, CONFIG_USBHOST_PSC_PRIO + 1, usbh_cdc_acm_thread, "AT\r\n");
    if(strstr(cdc_acm_class->hport->config.intf[cdc_acm_class->ctrl_intf].devname,OPERATE_INTERFACE_NAME))
        usbh_acm_rx_thread = usb_osal_thread_create("usbh_cdc_acm_rx", 4096, CONFIG_USBHOST_PSC_PRIO + 1, usbh_cdc_acm_rx_thread, NULL);
#endif
}

void usbh_cdc_at_run(int argc, char *argv[])
{
    strcat(argv[1], "\r\n");  //AT COMMAND must end with \r\n, console can not identify.
    USB_LOG_DBG("argv[1]:%s\n", argv[1]);
#if TEST_USBH_CDC_ACM
    usb_osal_thread_create("usbh_cdc", 4096, CONFIG_USBHOST_PSC_PRIO + 1, usbh_cdc_acm_thread, argv[1]);
#endif
}
FINSH_FUNCTION_EXPORT_CMD(usbh_cdc_at_run, usb_at_run, usb host at run test);
...
#if TEST_USBH_CDC_ACM
    if(strstr(cdc_acm_class->hport->config.intf[cdc_acm_class->ctrl_intf].devname,OPERATE_INTERFACE_NAME))
        usb_osal_thread_delete(usbh_acm_rx_thread);
#endif
