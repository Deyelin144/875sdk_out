...
int cmd_hid_init(int argc, char ** argv)
{
    if (is_init == 0) {
        hid_mouse_init();
        is_init = 1;
        ...
        printf("usb is already.");
    }
}

int cmd_hid_test(int argc, char ** argv)
{
    if (is_init == 0) {
        printf("hid in not init.\n");
        return -1;
    }
    hid_mouse_test();
}

FINSH_FUNCTION_EXPORT_CMD(cmd_hid_test, hid, Console hid Command);
FINSH_FUNCTION_EXPORT_CMD(cmd_hid_init, hid_init, Console hid init Command);