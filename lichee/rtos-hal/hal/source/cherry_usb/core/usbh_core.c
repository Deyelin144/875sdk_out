...
{
    for (int i = 0; i < sizeof(usbh_class_type_info)/sizeof(struct usbh_class_info); i++) {
        if ((usbh_class_type_info[i].match_flags & (USB_CLASS_MATCH_VENDOR | USB_CLASS_MATCH_PRODUCT | USB_CLASS_MATCH_INTF_CLASS | USB_CLASS_MATCH_INTF_SUBCLASS | USB_CLASS_MATCH_INTF_PROTOCOL)) ==
            ...
            if (usbh_class_type_info[i].vid == vid && usbh_class_type_info[i].pid == pid &&
                usbh_class_type_info[i].class == class && usbh_class_type_info[i].subclass == subclass && usbh_class_type_info[i].protocol == protocol) {
                return usbh_class_type_info[i].class_driver;
            }
        } else if ((usbh_class_type_info[i].match_flags & (USB_CLASS_MATCH_INTF_CLASS | USB_CLASS_MATCH_INTF_SUBCLASS | USB_CLASS_MATCH_INTF_PROTOCOL)) ==
                   (USB_CLASS_MATCH_INTF_CLASS | USB_CLASS_MATCH_INTF_SUBCLASS | USB_CLASS_MATCH_INTF_PROTOCOL)) {
            if (usbh_class_type_info[i].class == class && usbh_class_type_info[i].subclass == subclass && usbh_class_type_info[i].protocol == protocol) {
                return usbh_class_type_info[i].class_driver;
            }
        } else if ((usbh_class_type_info[i].match_flags & (USB_CLASS_MATCH_VENDOR | USB_CLASS_MATCH_PRODUCT | USB_CLASS_MATCH_INTF_CLASS)) ==
                   (USB_CLASS_MATCH_VENDOR | USB_CLASS_MATCH_PRODUCT | USB_CLASS_MATCH_INTF_CLASS)) {
            if (usbh_class_type_info[i].vid == vid && usbh_class_type_info[i].pid == pid && usbh_class_type_info[i].class == class) {
                return usbh_class_type_info[i].class_driver;
            }
        } else if ((usbh_class_type_info[i].match_flags & (USB_CLASS_MATCH_INTF_CLASS | USB_CLASS_MATCH_INTF_SUBCLASS)) == (USB_CLASS_MATCH_INTF_CLASS | USB_CLASS_MATCH_INTF_SUBCLASS)) {
            if (usbh_class_type_info[i].class == class && usbh_class_type_info[i].subclass == subclass) {
                return usbh_class_type_info[i].class_driver;
            }
        } else if ((usbh_class_type_info[i].match_flags & (USB_CLASS_MATCH_INTF_CLASS)) == USB_CLASS_MATCH_INTF_CLASS) {
            if (usbh_class_type_info[i].class == class) {
                return usbh_class_type_info[i].class_driver;
            }
        }
    }
...
static int parse_config_descriptor(struct usbh_hubport *hport, struct usb_configuration_descriptor *desc, uint16_t length)
{
    ...
    if (desc->bLength != USB_SIZEOF_CONFIG_DESC) {
        ...
        while (p[DESC_bLength] && (desc_len <= length)) {
            switch (p[DESC_bDescriptorType]) {
                case USB_DESCRIPTOR_TYPE_INTERFACE:
                    ...
                    if (cur_iface > max_iface_number) {
                        max_iface_number = cur_iface;
                    }
                    ...
                    USB_LOG_INFO("Interface Class:0x%02x, SubClass:0x%02x\n",
        ...
        hport->config.config_desc.bNumInterfaces = max_iface_number + 1;
    }
    return 0;
}
...
int usbh_enumerate(struct usbh_hubport *hport)
{
    ...
    USB_LOG_INFO("Device Class:%04x, SubClass:%04x\n",
                 ((struct usb_device_descriptor *)ep0_request_buffer)->bDeviceClass,
                 ((struct usb_device_descriptor *)ep0_request_buffer)->bDeviceSubClass);

    string_offset = ((struct usb_device_descriptor *)ep0_request_buffer)->iManufacturer - 1;
    ...
    ret = usbh_get_string_desc(hport, USB_STRING_MFC_INDEX + string_offset, string_buffer);
    if (ret < 0) {
    ...
    /* Get Product string */
    memset(string_buffer, 0, 128);
    ret = usbh_get_string_desc(hport, USB_STRING_PRODUCT_INDEX + string_offset, string_buffer);
    if (ret < 0) {
    ...
    memset(string_buffer, 0, 128);
    ret = usbh_get_string_desc(hport, USB_STRING_SERIAL_INDEX + string_offset, string_buffer);
    if (ret < 0) {
    ...
    for (uint8_t i = 0; i < hport->config.config_desc.bNumInterfaces; i++) {
        ...
        if (class_driver == NULL) {
            USB_LOG_ERR("%d do not support Class:0x%02x,Subclass:0x%02x,Protocl:0x%02x\r\n",
                        i,
                        intf_desc->bInterfaceClass,
                        intf_desc->bInterfaceSubClass,
                        intf_desc->bInterfaceProtocol);
        ...
        USB_LOG_INFO("USB%d Loading %s class driver\r\n", i, class_driver->driver_name);
...
FINSH_FUNCTION_EXPORT_CMD(usbh_initialize, usbh_init, usb host init test);

int usbh_deinitialize(void)
{
    return usbh_hub_deinitialize();
}
FINSH_FUNCTION_EXPORT_CMD(usbh_deinitialize, usbh_deinit, usb host deinit test);
    ...
    if (strcmp(argv[1], "-t") == 0) {
        usb_slist_for_each(i, &hub_class_head)
        {
            ...
            for (uint8_t port = 0; port < hub->hub_desc.bNbrPorts; port++) {
                ...
                if (hport->connected) {
                    for (uint8_t i = 0; i < hport->config.config_desc.bNumInterfaces; i++) {
                         if(hport->config.intf[i].class_driver != NULL) {
                             if (hport->config.intf[i].class_driver->driver_name) {
                                 USB_LOG_RAW("    |__Port %u,Port addr:0x%02x,If %u,ClassDriver=%s,name:%s\r\n",
                                     hport->port,
                                     hport->dev_addr,
                                     i,
                                     hport->config.intf[i].class_driver->driver_name,
                                     strlen(hport->config.intf[i].devname)?hport->config.intf[i].devname:"NULL");
                             }
                        }
                   }
                }
...
FINSH_FUNCTION_EXPORT_CMD(lsusb, lsusb, list usb devices test);
