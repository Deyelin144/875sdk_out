...
usb_osal_thread_t cherry_hub_thread;
...
#ifdef CONFIG_COMPONENTS_PM
static int sunxi_usb_hc_suspend(struct pm_device *dev, suspend_mode_t mode)
{
    usb_slist_t *i;
...
static int sunxi_usb_hc_resume(struct pm_device *dev, suspend_mode_t mode)
{
    USB_LOG_WRN("%s\r\n", __func__);
    usb_hc_init();
	return 0;
...
struct pm_devops sunxi_usb_hc_pm_ops = {
	.suspend = sunxi_usb_hc_suspend,
	.resume = sunxi_usb_hc_resume,
};
struct pm_device sunxi_usb_pm_dev = {
	.ops = &sunxi_usb_hc_pm_ops,
	.name = "cherry-usb",
};
#endif
...
int usbh_hub_initialize(void)
{
    ...
    cherry_hub_thread = usb_osal_thread_create("usbh_hub", CONFIG_USBHOST_PSC_STACKSIZE, CONFIG_USBHOST_PSC_PRIO, usbh_hub_thread, NULL);
    if (cherry_hub_thread == NULL) {
        return -1;
    }

#ifdef CONFIG_COMPONENTS_PM
    pm_devops_register(&sunxi_usb_pm_dev);
#endif
    return 0;
}
...
int usbh_hub_deinitialize(void)
{
    ...
    usb_osal_thread_delete(cherry_hub_thread);

#ifdef CONFIG_COMPONENTS_PM
    pm_devops_unregister(&sunxi_usb_pm_dev);
#endif
