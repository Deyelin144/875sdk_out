...
#ifdef CONFIG_USB_EC800M_CN  /* ec800m-cn do not limit package length. */
#define CONFIG_USBHOST_CDC_ECM_ETH_MAX_SEGSZE 1642U
#else
...
static int usbh_cdc_ecm_connect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    while (p[DESC_bLength]) {
        USB_LOG_DBG("%s-%d p[DESC_bLength]: 0x%x\r\n", __func__, __LINE__, p[DESC_bLength]);
        USB_LOG_DBG("%s-%d p[DESC_bDescriptorType]: 0x%x\r\n", __func__, __LINE__, p[DESC_bDescriptorType]);
        switch (p[DESC_bDescriptorType]) {
            case USB_DESCRIPTOR_TYPE_CONFIGURATION:
                USB_LOG_DBG("%s-%d configation descriptor\r\n", __func__, __LINE__);
                break;
            case USB_DESCRIPTOR_TYPE_INTERFACE:
                USB_LOG_DBG("%s-%d interface descriptor\r\n", __func__, __LINE__);
                cur_iface = p[INTF_DESC_bInterfaceNumber];
                //cur_alt_setting = p[INTF_DESC_bAlternateSetting];
                break;
            case CDC_CS_INTERFACE:
                USB_LOG_DBG("%s-%d CS interface descriptor\r\n", __func__, __LINE__);
