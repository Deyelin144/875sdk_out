...
unsigned int DiskRead(struct devfs_node *node, uint32_t blk, uint32_t n, const void *pBuffer)
{
    ...
    msc_class = (struct usbh_msc *)(node->private);

    if (msc_class == NULL) {
        USB_LOG_ERR("DiskRead error, msc_class is NULL\n");
        return 0;
    }

    if ((blk+n) > msc_class->blocknum) {
        USB_LOG_ERR("DiskRead error, block(%s, %s) is adjacence max capacity(%x)\n",
                blk, n, msc_class->blocknum);
        return 0;
    }

    USB_LOG_DBG("read p:0x%x blk:%d size:%d\n", pBuffer, blk, n);

    cnt = usbh_msc_scsi_read10(msc_class, blk, pBuffer, n);

    USB_LOG_DBG("read return cnt:%d\n", cnt);

    if (cnt < 0) {
        USB_LOG_ERR("DiskRead failed, want(%d) != read(%d)\n", n, cnt);
        return 0;
    }
...
unsigned int DiskWrite(struct devfs_node *node, uint32_t blk, uint32_t n, const void *pBuffer)
{
    ...
    if (msc_class == NULL) {
        USB_LOG_ERR("DiskWrite error, msc_class is NULL\n");
        return 0;
    }

    if ((blk+n) > msc_class->blocknum) {
        USB_LOG_ERR("DiskWirte error, block(%s, %s) is adjacence max capacity(%x)\n",
                blk, n, msc_class->blocknum);
        return 0;
    }

    USB_LOG_DBG("write blk:%d size:%d\n", blk, n);

    cnt = usbh_msc_scsi_write10(msc_class, blk, pBuffer, n);

    USB_LOG_DBG("write return cnt:%d\n", cnt);

    if (cnt < 0) {
        USB_LOG_ERR("DiskWirte failed, want(%d) != write(%d)\n", n, cnt);
        return 0;
    }
...
static int32_t DiskIoctl(struct devfs_node *node, int cmd, void *args[2])
{
    ...
    if (msc_class == NULL) {
        USB_LOG_ERR("DiskIoctl error, msc_class is NULL\n");
        return 0;
    }

    USB_LOG_INFO("ioctl cmd: %d\n", cmd);

    switch (cmd) {
    case DEV_CMD_GET_INFO: {
        if (!pBuffer) {
            USB_LOG_ERR("usb host msc, pBuffer = NULL\n");
            return -1;
        }
        ((__dev_blkinfo_t *)pBuffer)->hiddennum = 0;
        ((__dev_blkinfo_t *)pBuffer)->headnum = 0;
        ((__dev_blkinfo_t *)pBuffer)->secnum = 0;
        ((__dev_blkinfo_t *)pBuffer)->partsize = msc_class->blocknum;
        ((__dev_blkinfo_t *)pBuffer)->secsize = msc_class->blocksize;
        USB_LOG_INFO("usbh msc: DEV_CMD_GET_INFO, capacity = %dM, sector = %d\n",
                msc_class->blocknum >> 11, msc_class->blocksize);
    } break;
    ...
    case DEV_CDROM_MULTISESSION:
        return -1;
    default:
        USB_LOG_ERR("%s, cmd = 0x%x, not support\n", __func__, cmd);
        return -1;
    }
...
static int usbh_msc_connect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    /* init devfs node*/
    char *name = NULL;
    name = usb_malloc(CONFIG_USBHOST_DEV_NAMELEN);
    if (!name) {
        USB_LOG_ERR("Malloc name failed\n");
        return -1;
    }
    snprintf(name, CONFIG_USBHOST_DEV_NAMELEN, "sd%c", msc_class->sdchar);
    msc_class->dev_node.open = DiskOpen;
    msc_class->dev_node.close = DiskClose;
    msc_class->dev_node.read = (void *)DiskRead;
    msc_class->dev_node.write = (void *)DiskWrite;
    msc_class->dev_node.ioctl = (void *)DiskIoctl;
    msc_class->dev_node.name = name;
    msc_class->dev_node.alias = name;
    msc_class->dev_node.private = msc_class;
    msc_class->dev_node.size = 1;

    USB_LOG_INFO("FreeRTOS create devfsnode, name:%s alias:%s\n", msc_class->dev_node.name, msc_class->dev_node.alias);
    /* create devfs node*/
    msc_class->dev_node.size = (uint64_t)msc_class->blocknum * msc_class->blocksize;
    ret = devfs_add_node(&msc_class->dev_node);
    if (ret) {
        USB_LOG_ERR("Create FreeRTOS devfs node failed, return %d\n", ret);
        goto err_handle;
    } else {
        free(name);
    }

    usb_msc_elmfat_mount(msc_class->dev_node.name, "/usb_msc");
#endif
    ...
    /* no need to run test case. */
    /* usbh_msc_run(msc_class); */
    return ret;

err_handle:
    USB_LOG_ERR("usbh msc connect failed![%d]\n", ret);
...
#ifdef CONFIG_KERNEL_FREERTOS
        USB_LOG_INFO("FreeRTOS delete fs node - /usb_msc\n");
        usb_msc_elmfat_unmount("/usb_msc", &msc_class->dev_node);
        devfs_del_node(&msc_class->dev_node);
#endif
