...
void sunxi_sound_rx_sync_remove(rx_sync_domain_t domain)
{
	...
	__cpsr = hal_spin_lock_irqsave(&rx_sync_lock);
	rx_sync_data.total_count[domain]--;
	snd_debug("domain=%d, total_count=%d\n",
		      domain, rx_sync_data.total_count[domain]);
			   ...
			   void *data, void (*rx_enable)(void *data, bool enable))
{
	...
	if (!rx_enable || !data) {
		snd_debug("card dev or data err.\n");
		return;
	}
...
void sunxi_sound_rx_sync_shutdown(rx_sync_domain_t domain, int id)
{
	...
	snd_debug("domian=%d, id=%d, enabled_count=%d\n",
		      domain, id, rx_sync_data.enabled_count[domain]);

	hal_spin_lock_irqsave(&rx_sync_lock);
	rx_sync_data.total_ctl_count[domain]--;

	snd_debug("domian=%d, id=%d, reset rx_sync_data\n", domain, id);
...
void sunxi_sound_rx_sync_control(rx_sync_domain_t domain, int id, bool enable)
{
	...
	snd_debug("domain=%d, id=%d, enable=%d, enabled_count=%d\n",
	...
	if (enable) {
		...
		if (rx_sync_data.enabled_count[domain] == rx_sync_data.total_ctl_count[domain]) {
			...
			snd_debug("domain=%d, id=%d, last_enabled_dev=0x%p\n",
