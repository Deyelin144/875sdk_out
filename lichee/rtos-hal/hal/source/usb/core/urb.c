...
int usb_submit_urb(struct urb *urb, unsigned mem_flags)
{
	...
	if (unlikely(urb->hcpriv || !urb->complete)) {
		hal_log_err("ERR: usb_submit_urb: urb->hcpriv = %p,\
			    urb->complete = %p input error ",
	...
	if (unlikely(!dev || (dev->state < USB_STATE_DEFAULT) || (!dev->bus) || (dev->devnum < 0))) {
		hal_log_err("ERR: usb_submit_urb: dev had ben destroied");
		return -ENODEV;
	}

	if (unlikely(dev->state == USB_STATE_SUSPENDED)) {
		hal_log_err("ERR: usb_submit_urb: dev had ben destroied");
		return -EHOSTUNREACH;
	}
	 ...
	 */
	if (unlikely(temp == PIPE_ISOCHRONOUS)) {
		int n, len;
	...
	if (unlikely(urb->transfer_buffer_length < 0)) {
