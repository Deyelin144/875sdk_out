...
static int mscUsbTransport(__mscDev_t *mscDev, unsigned int TimeOut)
{
	...
	if (unlikely(mscDev == NULL)) {
		hal_log_err("ERR: mscUSBTransport: input error, mscDev = %p", mscDev);
		return -EINVAL;
	}

	if (unlikely(mscDev->state != MSC_DEV_ONLINE)) {
	...
	/* create timer */
	if (TimeOut && mscDev->TimerHdle == NULL) {
...
int mscBoTransport(__mscDev_t *mscDev, __ScsiCmnd_t *ScsiCmnd)
{
	...
	ret = mscSendBulkReq(mscDev, mscDev->BulkOut, CBW, sizeof(__CBW_t), NULL, TimeOut);
	if (unlikely(ret != USB_STOR_XFER_GOOD)) {
	...
	if (likely(ScsiCmnd->DataTransferLength)) {
		...
		ret = mscSendBulkReq(mscDev, Pipe, ScsiCmnd->buffer, ScsiCmnd->DataTransferLength, &ActLen, TimeOut);
		/* 有些设备在read/write命令的数据传输过程中，会出现ep stall。
		   出现这种情况，仅仅clear feature是不够的，还得需要命令重传。
		  */
		if (unlikely(ret == USB_STOR_XFER_STALLED)) {
			switch (((__u8 *)(ScsiCmnd->cmnd.CommandBlock))[0])
			{
				case SCSI_READ6:
				case SCSI_READ10:
				case SCSI_READ16:
				...
				case SCSI_WRITE16:
				{
					hal_log_err("ERR: Command(%x) execute failed, for ep stall, need retry", ((__u8 *)(ScsiCmnd->cmnd.CommandBlock))[0]);
					mscDev->ResetRecovery(mscDev);
					ret = USB_STOR_TRANSPORT_ERROR;
					goto TransportDone;
				}
			}
		} else if (unlikely(ret == USB_STOR_XFER_ERROR)) {
...
RETRY_CSW:
	...
	ret = mscSendBulkReq(mscDev, mscDev->BulkIn, CSW, sizeof(__CSW_t), &ActLen, TimeOut);
