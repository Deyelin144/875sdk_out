...
__mscLun_t *mscLun_alloc(void)
{
	...
	if (unlikely(mscLun->Lock == NULL)) {
		hal_log_err("ERR: UsbBlkDevAllocInit: create lock failed");
		hal_free(mscLun);
		return NULL;
	}

	mscLun->read_sem = hal_sem_create(0);
	if (unlikely(mscLun->read_sem == NULL)) {
		hal_log_err("ERR: UsbBlkDevAllocInit: create lock failed");
	...
	mscLun->write_sem = hal_sem_create(0);
	if (unlikely(mscLun->write_sem == NULL)) {
		hal_log_err("ERR: UsbBlkDevAllocInit: create lock failed");
		hal_sem_delete(mscLun->Lock);
		hal_sem_delete(mscLun->read_sem);
		hal_free(mscLun);
		return NULL;
	}
...
void mscLun_free(__mscLun_t *mscLun)
{
	...
	if (mscLun->write_sem) {
		hal_sem_delete(mscLun->write_sem);
		mscLun->write_sem = NULL;
	} else {
		hal_log_err("ERR: BlkDev write_sem had already delete");
	}

	if (mscLun->read_sem) {
		hal_sem_delete(mscLun->read_sem);
		mscLun->read_sem = NULL;
	} else {
		hal_log_err("ERR: BlkDev read_sem had already delete");
	}
