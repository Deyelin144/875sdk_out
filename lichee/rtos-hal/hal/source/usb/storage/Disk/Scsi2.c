...
{
	unsigned char alloc_sem = 0;
	unsigned int ret = 0;

	if (unlikely(mscLun == NULL || ScsiCmnd == NULL)) {
		hal_log_err("ERR: ScsiDoCommand: input error, mscLun = %p, ScsiCmd == %p",
			    mscLun, ScsiCmnd);
		return USB_STATUS_BAD_ARGUMENTS;
	}

	if (unlikely(ScsiCmnd->complete == NULL)) {
	...
	if (unlikely(ret != USB_ERR_SUCCESS)) {
...
int ScsiInquery(__mscLun_t *mscLun, void *buffer, unsigned int buffer_len)
{
	...
	if (unlikely(mscLun == NULL || buffer == NULL)) {
...
int ScsiReadCapacity(__mscLun_t *mscLun)
{
	...
	if (likely(ret == USB_STATUS_SUCCESS)) {
...
static int ScsiGetSenseData(__mscLun_t *mscLun)
{
	...
	if (unlikely(ret != USB_STATUS_SUCCESS)) {
	     ...
	     unsigned int buffer_len)
{
	...
	/* initialize transport command */
	ScsiCmnd.complete = mscLun->read_sem;
	ScsiCmnd.cmnd.data_direction    = DATA_FROM_DEVICE;
	ScsiCmnd.cmnd.Timeout           = SCSI_MEDIA_OP_TIMEOUT;
	ScsiCmnd.cmnd.dwLun             = mscLun->LunNo;
	ScsiCmnd.cmnd.CommandBlock      = CB;
	      ...
	      unsigned int buffer_len)
{
	...
	/* initialize transport command */
	ScsiCmnd.complete = mscLun->write_sem;
