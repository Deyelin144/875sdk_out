...
static unsigned int __DiskWrite(const void *pBuffer, unsigned int blk, unsigned int n, void *hDev)
{
	...
	if (unlikely(mscLun->WriteProtect)) {
	...
	*/
	...
	{
		case 10:
			ret = ScsiWrite(mscLun, 10, blk, n, (void *)pBuffer, (n * (mscLun->disk_info.sector_size)));
		break;
		case 16:
			ret = ScsiWrite(mscLun, 16, blk, n, (void *)pBuffer, (n * (mscLun->disk_info.sector_size)));
		break;
		default:
			ret = ScsiWrite(mscLun, 6, blk, n, (void *)pBuffer, (n * (mscLun->disk_info.sector_size)));
		break;
	}
	hal_sem_post(mscLun->Lock);
	// set_usbh_disk_status(USB_STORAGE_DEVICE_STATUS_IDLE);
