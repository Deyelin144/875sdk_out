			...
			if ((nctri->nci->support_toggle_only == 0) &&
				...
				for (i = 0; i < 3; i++) {
					...
					ret = _change_all_nand_parameter(
							...
							nctri->nci->frequency);
					if (ret) {
						PHY_ERR("1 _change nand parameter error when use clk %dMhz!\n",
								nctri->nci->frequency);
						return ret;
					}
					...
					if (ret) {
						PHY_ERR("1 change clk to cfg(%d, %d) failed!\n",
								nctri->nci->frequency,
								nctri->nci->frequency *
								2);
						return ERR_NO_47;
...
int update_1st_nctri_interface(struct _nand_controller_info *nctri)
{
	...
	if ((ret < 0) || (ret == 1)) {
		if ((ddr_type == SDR) && (sclk0 < 50)) {
			PHY_DBG("interface change ddr_type:%x sclk0:%d sclk1:%d sdr_edo_bak:%x ddr_edo_bak:%x ddr_delay_bak:%x !\n",
			...
			PHY_DBG("interface  not change ddr_type:%x sclk0:%d sclk1:%d sdr_edo:%x ddr_edo:%x ddr_delay:%x !\n",
					ddr_type_bak, sclk0_bak, sclk1_bak, sdr_edo_bak,
					ddr_edo_bak, ddr_delay_bak);
			ret = start_update_nctri_interface(
					nctri, 1, ddr_type_bak, ddr_type_bak, sclk0_bak,
					sclk1_bak, sdr_edo_bak, ddr_edo_bak,
					ddr_delay_bak);
		}
	} else {
		PHY_DBG("interface change ddr_type:%x sclk0:%d sclk1:%d sdr_edo_bak:%x ddr_edo_bak:%x ddr_delay_bak:%x !\n",
				...
				ddr_delay_bak);
		ret = start_update_nctri_interface(nctri, 0, ddr_type,
				ddr_type_bak, sclk0, sclk1,
				sdr_edo_bak, ddr_edo_bak,
				ddr_delay_bak);
	}
...
int update_other_nctri_interface(struct _nand_controller_info *nctri)
{
	...
	PHY_ERR("%s: timing_change %d, sclk0_bak %d, sclk0 %d!\n", __func__,
...
s32 _update_ext_accessfreq_para(struct _nand_controller_info *nctri)
{
	...
	if ((id_number_ctl & 0x01) != 0) {//bit 0, set freq para
		...
		if (para == 0xffffffff) {//get script fail
			PHY_ERR("_update_ext accessfreq_para: wrong freq para, 0x%x\n",
					para);
			return ERR_NO_43;
		}
		...
		if (((para & 0xffffff) == nctri->nci->npi->id_number) ||
			...
			if ((script_frequence > 10) &&
				...
				PHY_ERR("_update_ext accessfreq_para: update freq from script, %d\n",
						script_frequence);
			} else {
			...
			PHY_ERR("_update_ext accessfreq_para: wrong id number, 0x%x/0x%x\n",
		...
		if ((script_frequence > 10) && (script_frequence < 100)) {
			...
			PHY_ERR("_update_ext accessfreq_para: update freq high, %d\n",
					script_frequence);
		} else {
			PHY_ERR("_update_ext accessfreq_para: wrong freq, %d\n",
					script_frequence);
...
int set_nand_script_frequence(void)
{
	...
	if (SUPPORT_UPDATE_EXTERNAL_ACCESS_FREQ) {
		while (nctri != NULL) {
			...
			if (ret != 0) {
				PHY_ERR("update ext accessfreq para fail, using internal frequency!\n");
			}
			nctri = nctri->next;
		}
	}
...
int update_nctri(struct _nand_controller_info *nctri)
{
	...
	while (nctri != NULL) {
		...
		if (rb_connect == 0xff) {
			PHY_ERR("update_nctri: build nand rb connect fail, chip_cnt=%d, chip_connect=0x%x\n",
