 ...
 *Parameter    :
 *Return       : 0:ok  -1:fail
 *Note         :for hynix 26nm nand
 *****************************************************************************/
int m3_read_boot0_page(struct _nand_chip_info *nci,
 ...
 *Note         :for hynix 26nm nand
 *****************************************************************************/
int m3_write_boot0_page(struct _nand_chip_info *nci,
			struct _nand_physic_op_par *npo)
{
	struct boot_ndfc_cfg cfg;
 ...
 *Note         :Ã¿¸öpageÖ»Ð´16K
 *****************************************************************************/
int m3_read_boot0_one_nonsecure(unsigned char *buf, unsigned int len,
				unsigned int counter)
{
	...
	PHY_ERR("m3_read_boot0_one_nonsecure!\n");
...
error:
	PHY_ERR("m3_read_boot0_one_nonsecure error!");
	nand_free_temp_buf(ptr, 8 * 1024);

	return -1;
}

int m3_read_boot0_one_secure(unsigned char *buf, unsigned int len,
			     unsigned int counter)
{
	__u32 j, k, m, err_flag = 0, count, block;
	__u8 oob_buf[64];
...
int m3_read_boot0_one(unsigned char *buf, unsigned int len,
		      unsigned int counter)
{
	if (NAND_IS_Secure_sys() == 0)
		return (m3_read_boot0_one_nonsecure(buf, len, counter));
	else if (NAND_IS_Secure_sys() == 1)
		return (m3_read_boot0_one_secure(buf, len, counter));
	return 0;
}
 ...
 *Note         :
 *****************************************************************************/
int m3_write_boot0_one_nonsecure(unsigned char *buf, unsigned int len,
				 unsigned int counter)
{
	int ret;
	...
	PHY_ERR("m3_write_boot0_one_nonsecure\n");
...
int m3_write_boot0_one_secure(unsigned char *buf, unsigned int len,
			      unsigned int counter)
{
	...
	PHY_ERR("burn_boot0_lsb_enable_pagetab mode!\n");
 ...
 *Note         :
 *****************************************************************************/
int m3_write_boot0_one(unsigned char *buf, unsigned int len,
		       unsigned int counter)
{
	if (NAND_IS_Secure_sys() == 0)
		return (m3_write_boot0_one_nonsecure(buf, len, counter));
	else if (NAND_IS_Secure_sys() == 1)
		return (m3_write_boot0_one_secure(buf, len, counter));
	return 0;
}
