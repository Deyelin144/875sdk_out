...
{
#ifdef CONFIG_SUNXI_BOOT_NAND_SELECT
	int nand_val;

	if (get_boot_storage_type() == STORAGE_NAND) {
		nand_val = readl(SID_EFUSE + EFUSE_SYSTEM);
		//select media field0 bit 5:2 should be 0100 for nand
		nand_val = (nand_val >> 2) & 0x7;
		if (nand_val == 0x4) {
			printf("already burn efuse for nand! skip.\n");
		} else {
			if (uni_burn_key(EFUSE_SYSTEM, (0x1 << 4))) {
				printf("burn efuse for nand failed.\n");
				return -1;
			} else {
				printf("burn efuse for nand ok.\n");
			}
		}
	}
#endif
