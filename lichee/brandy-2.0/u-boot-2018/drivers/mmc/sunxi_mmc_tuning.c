...
static int sunxi_tuning_method_0(struct mmc *mmc, int retry_times)
{
	...
	rcv_pattern = memalign(CONFIG_SYS_CACHELINE_SIZE, TUNING_LEN * 512);
	if (rcv_pattern == NULL) {
		MMCINFO("request memory for rcv_pat fail\n");
		return -1;
	}

	std_pattern = memalign(CONFIG_SYS_CACHELINE_SIZE, TUNING_LEN * 512);
	if (std_pattern == NULL) {
		MMCINFO("request memory for rcv_pat fail\n");
		return -1;
	}
...
int sunxi_execute_tuning(struct mmc *mmc, int speed_mode)
{
	...
	if (!IS_SD(mmc)) {
		...
		if ((speed_mode >= HSDDR52_DDR50) && (speed_mode < MAX_SPD_MD_NUM)) {
