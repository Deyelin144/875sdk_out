...
#define SPILCD_DBI_PORT 1
#define SPILCD_BL_PIN 0x22
// #define SPILCD_RESET_PIN 0x06
#define SPILCD_WIDTH 320
#define SPILCD_HEIGHT 240

#define MCU_WAKEUP_PIN 0x0b

#define LCD_COMMON_DLY_FLAG 0x80

extern unsigned char uboot_img[];

typedef struct {
    uint8_t data_size; /**< 锟斤拷围锟斤拷 0-0x7f, 锟斤拷锟斤拷锟斤拷锟斤拷要锟街成讹拷锟斤拷锟斤拷锟斤拷 */
    union {
        uint8_t cmd;
        uint8_t dly;
    };
...
static uint8_t s_st7789v3_reg_param[] = {
    // data_size cmd/dly data
...
extern int dbi_init(int x, int y);
extern int disp_lcd_cmd_write_by_dbi(uint32_t port, uint8_t *buf, uint32_t size);
extern int disp_lcd_para_write_by_dbi(uint32_t port, uint8_t *buf, uint32_t size);
extern int disp_lcd_dbi_pixel_cfg(uint32_t port, uint8_t *buf, uint32_t size);

int lcd_common_reg_init(uint32_t port, uint8_t *array, uint32_t array_size)
{
    int ret = 0;
    ...
    for (uint32_t i = 0; i < array_size;) {
        ...
        if (LCD_COMMON_DLY_FLAG == data_size) {
            udelay((((lcd_common_reg_para_t *)&reg_param[i])->dly) * 1000);
            data_size = 0;
        } else {
            disp_lcd_cmd_write_by_dbi(port, &(((lcd_common_reg_para_t *)&reg_param[i])->cmd), 1);
            for (uint16_t j = 0; j < data_size; j++) {
                disp_lcd_para_write_by_dbi(port, &(((lcd_common_reg_para_t *)&reg_param[i])->data[j]), 1);
            }
        }
        i += 2 + data_size;
        data_size = 0;
...
void spilcd_init_bl(void)
{
	sunxi_gpio_set_cfgpin(SPILCD_BL_PIN, SUNXI_GPIO_DISABLE);
	sunxi_gpio_set_pull(SPILCD_BL_PIN, SUNXI_GPIO_PULL_UP);
	gpio_direction_output(SPILCD_BL_PIN, 1);
	gpio_set_value(SPILCD_BL_PIN, 0);
}

void spilcd_reset(void)
{
	// gpio_direction_output(SPILCD_RESET_PIN, 1);
	// gpio_set_value(SPILCD_RESET_PIN, 1);
	// udelay(20 * 1000);
	// gpio_set_value(SPILCD_RESET_PIN, 0);
	// udelay(20 * 1000);
	// gpio_set_value(SPILCD_RESET_PIN, 1);
}

static void address(unsigned int port, int x, int y, int width, int height)
{
    // set coloum address
	unsigned char spilcd_cmd[1];
	unsigned char spilcd_para[4];

	spilcd_cmd[0] = 0x2a;
	disp_lcd_cmd_write_by_dbi(port, spilcd_cmd, 1);

	spilcd_para[0] = (unsigned char)((x >> 8) & 0xff);
	spilcd_para[1] = (unsigned char)(x & 0xff);
	spilcd_para[2] = (unsigned char)((width >> 8) & 0xff);
	spilcd_para[3] = (unsigned char)(width & 0xff);
	disp_lcd_para_write_by_dbi(port, spilcd_para, 4);

	spilcd_cmd[0] = 0x2b;
	disp_lcd_cmd_write_by_dbi(port, spilcd_cmd, 1);

	spilcd_para[0] = (unsigned char)((y >> 8) & 0xff);
	spilcd_para[1] = (unsigned char)(y & 0xff);
	spilcd_para[2] = (unsigned char)((height >> 8) & 0xff);
	spilcd_para[3] = (unsigned char)(height & 0xff);
	disp_lcd_para_write_by_dbi(port, spilcd_para, 4);
}

// static void address(unsigned int port, uint16_t xStart, uint16_t yStart, uint16_t xlong, uint16_t ylong)
// {
// 	unsigned char spilcd_cmd[1];
...
// 	x_data[0] = xStart >> 8;
// 	x_data[1] = xStart & 0x00FF;
// 	x_data[2] = xEnd >> 8;
// 	x_data[3] = xEnd & 0x00FF;
// 	spilcd_cmd[0] = 0x2a;
// 	disp_lcd_cmd_write_by_dbi(port, spilcd_cmd, 1);
// 	disp_lcd_para_write_by_dbi(port, x_data, 4);
	
// 	y_data[0] = yStart >> 8;
// 	y_data[1] = yStart & 0x00FF;
// 	y_data[2] = yEnd >> 8;
// 	y_data[3] = yEnd & 0x00FF;
// 	spilcd_cmd[0] = 0x2b;
// 	disp_lcd_cmd_write_by_dbi(port, spilcd_cmd, 1);
// 	disp_lcd_para_write_by_dbi(port, y_data, 4);
// }

static int lcd_set_addr_win(unsigned int port, int x, int y, int width, int height)
{
	unsigned char spilcd_cmd[1];

    address(port, x, y, width, height);
...
int spilcd_init(void)
{
	spilcd_init_bl();
	// 延时200ms，等mcu那边把lcd的复位脚初始化
	udelay(200 * 1000);
	// spilcd_reset();

	lcd_common_reg_init(SPILCD_DBI_PORT, s_st7789v3_reg_param, sizeof(s_st7789v3_reg_param));

	// 直接刷一屏黑色, 避免128软复位时, 屏幕还保留上一帧的数据，看起来会闪一下
	uint8_t *buffer = (uint8_t *)malloc(153600);          // 240*320*2 字节
	if (buffer != NULL) {
		memset(buffer, 0x00, 153600);
		
		lcd_set_addr_win(SPILCD_DBI_PORT, 0, 0, SPILCD_HEIGHT - 1, SPILCD_WIDTH - 1);
		disp_lcd_dbi_pixel_cfg(SPILCD_DBI_PORT, (uint8_t *)buffer, 153600);
		free(buffer);
		buffer = NULL;
		// udelay(2000 * 1000);
	}
	...
	lcd_set_addr_win(SPILCD_DBI_PORT, 0, 0, SPILCD_HEIGHT-1, SPILCD_WIDTH-1);
	disp_lcd_dbi_pixel_cfg(SPILCD_DBI_PORT, uboot_img, 153600);

	gpio_set_value(SPILCD_BL_PIN, 1);
	return 0;
}

#define GPIO_REG_DEBUG (0)
...
int initr_sunxi_display(void)
{
	...
	// 定义PMC相关寄存器
	#define PM_PMC_REG_BASE       0x40051400
	#define PM_RTC_IO_WAKE_EN     (PM_PMC_REG_BASE + 0x110)
	#define PM_RTC_IO_HOLD_CTRL   (PM_PMC_REG_BASE + 0x11c)
	
	// 获取WUPIO0相关寄存器状态
	volatile uint32_t *pm_rtc_io_hold_ctrl_reg = (volatile uint32_t *)PM_RTC_IO_HOLD_CTRL;
	uint32_t hold_ctrl_val = *pm_rtc_io_hold_ctrl_reg;
	
	// 检查WUPIO0是否被置为1, 如果是的话表示是rtc wakeup,  需要唤醒mcu
	if (hold_ctrl_val & (0x1 << 0)) {
		printf("RTC WAKEUP, need to wakeup mcu (reg: 0x%08x)\n", hold_ctrl_val);
		
		// 清除pa11的IO状态保持
		*pm_rtc_io_hold_ctrl_reg = hold_ctrl_val & ~(0x1 << 0);
		printf("clear bit0 - reg: 0x%08x\n", *pm_rtc_io_hold_ctrl_reg);
		
#if GPIO_REG_DEBUG
		// 定义GPIO基地址和寄存器偏移
		#define GPIO_BASE      0x4004A400
		#define PA_CFG0        (GPIO_BASE + 0x0000)
		#define PA_CFG1        (GPIO_BASE + 0x0004)
		#define PA_DATA        (GPIO_BASE + 0x0010)   
		// 读取并打印PA11配置寄存器的初始值
		volatile uint32_t *pa_cfg1_reg = (volatile uint32_t *)PA_CFG1;
		uint32_t pa_cfg1_val = *pa_cfg1_reg;
		printf("PA11初始配置 - PA_CFG1寄存器值: 0x%08x\n", pa_cfg1_val);
#endif  
		// 先deinit掉IO口
		sunxi_gpio_set_cfgpin(MCU_WAKEUP_PIN, SUNXI_GPIO_DISABLE);

#if GPIO_REG_DEBUG
		// 打印deinit后的寄存器值
		pa_cfg1_val = *pa_cfg1_reg;
		printf("PA11禁用后 - PA_CFG1寄存器值: 0x%08x\n", pa_cfg1_val);
#endif  
		
		// 然后重新初始化成输出
		gpio_direction_output(MCU_WAKEUP_PIN, 1);
		
#if GPIO_REG_DEBUG
		// 打印设置为输出后的寄存器值
		pa_cfg1_val = *pa_cfg1_reg;
		printf("PA11设置为输出后 - PA_CFG1寄存器值: 0x%08x\n", pa_cfg1_val);
#endif

		// 拉高PA11, 唤醒mcu
		gpio_set_value(MCU_WAKEUP_PIN, 1);
#if GPIO_REG_DEBUG
		// 读取并打印数据寄存器值
		volatile uint32_t *pa_data_reg = (volatile uint32_t *)PA_DATA;
		uint32_t pa_data_val = *pa_data_reg;
		printf("PA_DATA寄存器值: 0x%08x\n", pa_data_val);
#endif
	}

	// printf("-------dbi init\n");
	dbi_init(SPILCD_HEIGHT, SPILCD_WIDTH);
	spilcd_init();

	// printf("-------------dbi init finish\n");
	return ret;
}
